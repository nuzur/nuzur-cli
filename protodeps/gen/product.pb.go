// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.35.0
// source: product.proto

package gen

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	gen "github.com/nuzur/nem/idl/gen"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CheckoutSessionType int32

const (
	CheckoutSessionType_CHECKOUT_SESSION_TYPE_INVALID CheckoutSessionType = 0
	CheckoutSessionType_CHECKOUT_SESSION_TYPE_MONTHLY CheckoutSessionType = 1
	CheckoutSessionType_CHECKOUT_SESSION_TYPE_YEARLY  CheckoutSessionType = 2
)

// Enum value maps for CheckoutSessionType.
var (
	CheckoutSessionType_name = map[int32]string{
		0: "CHECKOUT_SESSION_TYPE_INVALID",
		1: "CHECKOUT_SESSION_TYPE_MONTHLY",
		2: "CHECKOUT_SESSION_TYPE_YEARLY",
	}
	CheckoutSessionType_value = map[string]int32{
		"CHECKOUT_SESSION_TYPE_INVALID": 0,
		"CHECKOUT_SESSION_TYPE_MONTHLY": 1,
		"CHECKOUT_SESSION_TYPE_YEARLY":  2,
	}
)

func (x CheckoutSessionType) Enum() *CheckoutSessionType {
	p := new(CheckoutSessionType)
	*p = x
	return p
}

func (x CheckoutSessionType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CheckoutSessionType) Descriptor() protoreflect.EnumDescriptor {
	return file_product_proto_enumTypes[0].Descriptor()
}

func (CheckoutSessionType) Type() protoreflect.EnumType {
	return &file_product_proto_enumTypes[0]
}

func (x CheckoutSessionType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CheckoutSessionType.Descriptor instead.
func (CheckoutSessionType) EnumDescriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{0}
}

type ProContext int32

const (
	ProContext_PRO_CONTEXT_INVALID      ProContext = 0
	ProContext_PRO_CONTEXT_EDITOR       ProContext = 1
	ProContext_PRO_CONTEXT_DATA_MANAGER ProContext = 2
)

// Enum value maps for ProContext.
var (
	ProContext_name = map[int32]string{
		0: "PRO_CONTEXT_INVALID",
		1: "PRO_CONTEXT_EDITOR",
		2: "PRO_CONTEXT_DATA_MANAGER",
	}
	ProContext_value = map[string]int32{
		"PRO_CONTEXT_INVALID":      0,
		"PRO_CONTEXT_EDITOR":       1,
		"PRO_CONTEXT_DATA_MANAGER": 2,
	}
)

func (x ProContext) Enum() *ProContext {
	p := new(ProContext)
	*p = x
	return p
}

func (x ProContext) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ProContext) Descriptor() protoreflect.EnumDescriptor {
	return file_product_proto_enumTypes[1].Descriptor()
}

func (ProContext) Type() protoreflect.EnumType {
	return &file_product_proto_enumTypes[1]
}

func (x ProContext) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ProContext.Descriptor instead.
func (ProContext) EnumDescriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{1}
}

// user start
type GetUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserUuid      string                 `protobuf:"bytes,1,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRequest) Reset() {
	*x = GetUserRequest{}
	mi := &file_product_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRequest) ProtoMessage() {}

func (x *GetUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRequest.ProtoReflect.Descriptor instead.
func (*GetUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{0}
}

func (x *GetUserRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

type GetTokenUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTokenUserRequest) Reset() {
	*x = GetTokenUserRequest{}
	mi := &file_product_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTokenUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTokenUserRequest) ProtoMessage() {}

func (x *GetTokenUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTokenUserRequest.ProtoReflect.Descriptor instead.
func (*GetTokenUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{1}
}

type GetUserByEmailRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Email         string                 `protobuf:"bytes,1,opt,name=email,proto3" json:"email,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserByEmailRequest) Reset() {
	*x = GetUserByEmailRequest{}
	mi := &file_product_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserByEmailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserByEmailRequest) ProtoMessage() {}

func (x *GetUserByEmailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserByEmailRequest.ProtoReflect.Descriptor instead.
func (*GetUserByEmailRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{2}
}

func (x *GetUserByEmailRequest) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

type GetUserRoleForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRoleForProjectRequest) Reset() {
	*x = GetUserRoleForProjectRequest{}
	mi := &file_product_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRoleForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRoleForProjectRequest) ProtoMessage() {}

func (x *GetUserRoleForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRoleForProjectRequest.ProtoReflect.Descriptor instead.
func (*GetUserRoleForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{3}
}

func (x *GetUserRoleForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type GetUserRoleForProjectResponse struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	Role                    gen.UserProjectRole    `protobuf:"varint,1,opt,name=role,proto3,enum=nem.UserProjectRole" json:"role,omitempty"`                                               // the role of the user in the project
	ReviewRequiredStructure bool                   `protobuf:"varint,2,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"` // if true, the user needs to review structure changes
	ReviewRequiredData      bool                   `protobuf:"varint,3,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`                // if true, the user needs to review data changes
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *GetUserRoleForProjectResponse) Reset() {
	*x = GetUserRoleForProjectResponse{}
	mi := &file_product_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRoleForProjectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRoleForProjectResponse) ProtoMessage() {}

func (x *GetUserRoleForProjectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRoleForProjectResponse.ProtoReflect.Descriptor instead.
func (*GetUserRoleForProjectResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{4}
}

func (x *GetUserRoleForProjectResponse) GetRole() gen.UserProjectRole {
	if x != nil {
		return x.Role
	}
	return gen.UserProjectRole(0)
}

func (x *GetUserRoleForProjectResponse) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *GetUserRoleForProjectResponse) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type GetUserRoleForTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserRoleForTeamRequest) Reset() {
	*x = GetUserRoleForTeamRequest{}
	mi := &file_product_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRoleForTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRoleForTeamRequest) ProtoMessage() {}

func (x *GetUserRoleForTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRoleForTeamRequest.ProtoReflect.Descriptor instead.
func (*GetUserRoleForTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{5}
}

func (x *GetUserRoleForTeamRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type GetUserRoleForTeamResponse struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	Role                    gen.UserTeamRole       `protobuf:"varint,1,opt,name=role,proto3,enum=nem.UserTeamRole" json:"role,omitempty"`                                                  // the role of the user in the team
	ReviewRequiredStructure bool                   `protobuf:"varint,2,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"` // if true, the user needs to review structure changes
	ReviewRequiredData      bool                   `protobuf:"varint,3,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`                // if true, the user needs to review data changes
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *GetUserRoleForTeamResponse) Reset() {
	*x = GetUserRoleForTeamResponse{}
	mi := &file_product_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserRoleForTeamResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserRoleForTeamResponse) ProtoMessage() {}

func (x *GetUserRoleForTeamResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserRoleForTeamResponse.ProtoReflect.Descriptor instead.
func (*GetUserRoleForTeamResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{6}
}

func (x *GetUserRoleForTeamResponse) GetRole() gen.UserTeamRole {
	if x != nil {
		return x.Role
	}
	return gen.UserTeamRole(0)
}

func (x *GetUserRoleForTeamResponse) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *GetUserRoleForTeamResponse) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type UpdateUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	User          *gen.User              `protobuf:"bytes,1,opt,name=user,proto3" json:"user,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateUserRequest) Reset() {
	*x = UpdateUserRequest{}
	mi := &file_product_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateUserRequest) ProtoMessage() {}

func (x *UpdateUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateUserRequest.ProtoReflect.Descriptor instead.
func (*UpdateUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{7}
}

func (x *UpdateUserRequest) GetUser() *gen.User {
	if x != nil {
		return x.User
	}
	return nil
}

// user relations
type ListUsersForTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	PageSize      int32                  `protobuf:"varint,2,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,3,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,4,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListUsersForTeamRequest) Reset() {
	*x = ListUsersForTeamRequest{}
	mi := &file_product_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUsersForTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUsersForTeamRequest) ProtoMessage() {}

func (x *ListUsersForTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUsersForTeamRequest.ProtoReflect.Descriptor instead.
func (*ListUsersForTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{8}
}

func (x *ListUsersForTeamRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *ListUsersForTeamRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListUsersForTeamRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListUsersForTeamRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

type ListUsersForTeamResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Users         []*gen.User            `protobuf:"bytes,1,rep,name=users,proto3" json:"users,omitempty"`
	UserTeams     []*gen.UserTeam        `protobuf:"bytes,2,rep,name=user_teams,json=userTeams,proto3" json:"user_teams,omitempty"`
	NextPageToken string                 `protobuf:"bytes,3,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListUsersForTeamResponse) Reset() {
	*x = ListUsersForTeamResponse{}
	mi := &file_product_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUsersForTeamResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUsersForTeamResponse) ProtoMessage() {}

func (x *ListUsersForTeamResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUsersForTeamResponse.ProtoReflect.Descriptor instead.
func (*ListUsersForTeamResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{9}
}

func (x *ListUsersForTeamResponse) GetUsers() []*gen.User {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *ListUsersForTeamResponse) GetUserTeams() []*gen.UserTeam {
	if x != nil {
		return x.UserTeams
	}
	return nil
}

func (x *ListUsersForTeamResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type ListUsersForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	PageSize      int32                  `protobuf:"varint,2,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,3,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,4,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListUsersForProjectRequest) Reset() {
	*x = ListUsersForProjectRequest{}
	mi := &file_product_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUsersForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUsersForProjectRequest) ProtoMessage() {}

func (x *ListUsersForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUsersForProjectRequest.ProtoReflect.Descriptor instead.
func (*ListUsersForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{10}
}

func (x *ListUsersForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ListUsersForProjectRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListUsersForProjectRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListUsersForProjectRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

type ListUsersForProjectResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Users         []*gen.User            `protobuf:"bytes,1,rep,name=users,proto3" json:"users,omitempty"`
	UserProjects  []*gen.UserProject     `protobuf:"bytes,2,rep,name=user_projects,json=userProjects,proto3" json:"user_projects,omitempty"`
	NextPageToken string                 `protobuf:"bytes,3,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListUsersForProjectResponse) Reset() {
	*x = ListUsersForProjectResponse{}
	mi := &file_product_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUsersForProjectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUsersForProjectResponse) ProtoMessage() {}

func (x *ListUsersForProjectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUsersForProjectResponse.ProtoReflect.Descriptor instead.
func (*ListUsersForProjectResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{11}
}

func (x *ListUsersForProjectResponse) GetUsers() []*gen.User {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *ListUsersForProjectResponse) GetUserProjects() []*gen.UserProject {
	if x != nil {
		return x.UserProjects
	}
	return nil
}

func (x *ListUsersForProjectResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type AddUserToProjectRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid             string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	UserUuid                string                 `protobuf:"bytes,2,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`
	UserEmail               string                 `protobuf:"bytes,3,opt,name=user_email,json=userEmail,proto3" json:"user_email,omitempty"` // optional, if user_uuid is not provided
	Role                    gen.UserProjectRole    `protobuf:"varint,4,opt,name=role,proto3,enum=nem.UserProjectRole" json:"role,omitempty"`
	ReviewRequiredStructure bool                   `protobuf:"varint,5,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"`
	ReviewRequiredData      bool                   `protobuf:"varint,6,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *AddUserToProjectRequest) Reset() {
	*x = AddUserToProjectRequest{}
	mi := &file_product_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddUserToProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddUserToProjectRequest) ProtoMessage() {}

func (x *AddUserToProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddUserToProjectRequest.ProtoReflect.Descriptor instead.
func (*AddUserToProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{12}
}

func (x *AddUserToProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *AddUserToProjectRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

func (x *AddUserToProjectRequest) GetUserEmail() string {
	if x != nil {
		return x.UserEmail
	}
	return ""
}

func (x *AddUserToProjectRequest) GetRole() gen.UserProjectRole {
	if x != nil {
		return x.Role
	}
	return gen.UserProjectRole(0)
}

func (x *AddUserToProjectRequest) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *AddUserToProjectRequest) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type UpdateUserProjectRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	Uuid                    string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	UserUuid                string                 `protobuf:"bytes,2,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`
	UserEmail               string                 `protobuf:"bytes,3,opt,name=user_email,json=userEmail,proto3" json:"user_email,omitempty"`
	Role                    gen.UserProjectRole    `protobuf:"varint,4,opt,name=role,proto3,enum=nem.UserProjectRole" json:"role,omitempty"`
	ReviewRequiredStructure bool                   `protobuf:"varint,5,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"`
	ReviewRequiredData      bool                   `protobuf:"varint,6,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *UpdateUserProjectRequest) Reset() {
	*x = UpdateUserProjectRequest{}
	mi := &file_product_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateUserProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateUserProjectRequest) ProtoMessage() {}

func (x *UpdateUserProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateUserProjectRequest.ProtoReflect.Descriptor instead.
func (*UpdateUserProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{13}
}

func (x *UpdateUserProjectRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *UpdateUserProjectRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

func (x *UpdateUserProjectRequest) GetUserEmail() string {
	if x != nil {
		return x.UserEmail
	}
	return ""
}

func (x *UpdateUserProjectRequest) GetRole() gen.UserProjectRole {
	if x != nil {
		return x.Role
	}
	return gen.UserProjectRole(0)
}

func (x *UpdateUserProjectRequest) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *UpdateUserProjectRequest) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type RemoveUserFromProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Uuid          string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	ProjectUuid   string                 `protobuf:"bytes,2,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveUserFromProjectRequest) Reset() {
	*x = RemoveUserFromProjectRequest{}
	mi := &file_product_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveUserFromProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveUserFromProjectRequest) ProtoMessage() {}

func (x *RemoveUserFromProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveUserFromProjectRequest.ProtoReflect.Descriptor instead.
func (*RemoveUserFromProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{14}
}

func (x *RemoveUserFromProjectRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *RemoveUserFromProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type AddUserToTeamRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid                string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	UserUuid                string                 `protobuf:"bytes,2,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`
	UserEmail               string                 `protobuf:"bytes,3,opt,name=user_email,json=userEmail,proto3" json:"user_email,omitempty"` // optional, if user_uuid is not provided
	Role                    gen.UserTeamRole       `protobuf:"varint,4,opt,name=role,proto3,enum=nem.UserTeamRole" json:"role,omitempty"`
	ReviewRequiredStructure bool                   `protobuf:"varint,5,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"`
	ReviewRequiredData      bool                   `protobuf:"varint,6,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *AddUserToTeamRequest) Reset() {
	*x = AddUserToTeamRequest{}
	mi := &file_product_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddUserToTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddUserToTeamRequest) ProtoMessage() {}

func (x *AddUserToTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddUserToTeamRequest.ProtoReflect.Descriptor instead.
func (*AddUserToTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{15}
}

func (x *AddUserToTeamRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *AddUserToTeamRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

func (x *AddUserToTeamRequest) GetUserEmail() string {
	if x != nil {
		return x.UserEmail
	}
	return ""
}

func (x *AddUserToTeamRequest) GetRole() gen.UserTeamRole {
	if x != nil {
		return x.Role
	}
	return gen.UserTeamRole(0)
}

func (x *AddUserToTeamRequest) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *AddUserToTeamRequest) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type UpdateUserTeamRequest struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	Uuid                    string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	UserUuid                string                 `protobuf:"bytes,2,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"`
	UserEmail               string                 `protobuf:"bytes,3,opt,name=user_email,json=userEmail,proto3" json:"user_email,omitempty"`
	Role                    gen.UserTeamRole       `protobuf:"varint,4,opt,name=role,proto3,enum=nem.UserTeamRole" json:"role,omitempty"`
	ReviewRequiredStructure bool                   `protobuf:"varint,5,opt,name=review_required_structure,json=reviewRequiredStructure,proto3" json:"review_required_structure,omitempty"`
	ReviewRequiredData      bool                   `protobuf:"varint,6,opt,name=review_required_data,json=reviewRequiredData,proto3" json:"review_required_data,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *UpdateUserTeamRequest) Reset() {
	*x = UpdateUserTeamRequest{}
	mi := &file_product_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateUserTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateUserTeamRequest) ProtoMessage() {}

func (x *UpdateUserTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateUserTeamRequest.ProtoReflect.Descriptor instead.
func (*UpdateUserTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{16}
}

func (x *UpdateUserTeamRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *UpdateUserTeamRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

func (x *UpdateUserTeamRequest) GetUserEmail() string {
	if x != nil {
		return x.UserEmail
	}
	return ""
}

func (x *UpdateUserTeamRequest) GetRole() gen.UserTeamRole {
	if x != nil {
		return x.Role
	}
	return gen.UserTeamRole(0)
}

func (x *UpdateUserTeamRequest) GetReviewRequiredStructure() bool {
	if x != nil {
		return x.ReviewRequiredStructure
	}
	return false
}

func (x *UpdateUserTeamRequest) GetReviewRequiredData() bool {
	if x != nil {
		return x.ReviewRequiredData
	}
	return false
}

type RemoveUserFromTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Uuid          string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	TeamUuid      string                 `protobuf:"bytes,2,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveUserFromTeamRequest) Reset() {
	*x = RemoveUserFromTeamRequest{}
	mi := &file_product_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveUserFromTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveUserFromTeamRequest) ProtoMessage() {}

func (x *RemoveUserFromTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveUserFromTeamRequest.ProtoReflect.Descriptor instead.
func (*RemoveUserFromTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{17}
}

func (x *RemoveUserFromTeamRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *RemoveUserFromTeamRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

// team start
type ListTeamsForUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	MinimalInfo   bool                   `protobuf:"varint,4,opt,name=minimal_info,json=minimalInfo,proto3" json:"minimal_info,omitempty"` // if true, only return team UUIDs, versions, names, status, and created_at
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTeamsForUserRequest) Reset() {
	*x = ListTeamsForUserRequest{}
	mi := &file_product_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTeamsForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTeamsForUserRequest) ProtoMessage() {}

func (x *ListTeamsForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTeamsForUserRequest.ProtoReflect.Descriptor instead.
func (*ListTeamsForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{18}
}

func (x *ListTeamsForUserRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListTeamsForUserRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListTeamsForUserRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListTeamsForUserRequest) GetMinimalInfo() bool {
	if x != nil {
		return x.MinimalInfo
	}
	return false
}

type ListTeamsForUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Teams         []*gen.Team            `protobuf:"bytes,1,rep,name=teams,proto3" json:"teams,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTeamsForUserResponse) Reset() {
	*x = ListTeamsForUserResponse{}
	mi := &file_product_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTeamsForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTeamsForUserResponse) ProtoMessage() {}

func (x *ListTeamsForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTeamsForUserResponse.ProtoReflect.Descriptor instead.
func (*ListTeamsForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{19}
}

func (x *ListTeamsForUserResponse) GetTeams() []*gen.Team {
	if x != nil {
		return x.Teams
	}
	return nil
}

func (x *ListTeamsForUserResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type ListTeamsForAdminUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	MinimalInfo   bool                   `protobuf:"varint,4,opt,name=minimal_info,json=minimalInfo,proto3" json:"minimal_info,omitempty"` // if true, only return team UUIDs, versions, names, status, and created_at
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTeamsForAdminUserRequest) Reset() {
	*x = ListTeamsForAdminUserRequest{}
	mi := &file_product_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTeamsForAdminUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTeamsForAdminUserRequest) ProtoMessage() {}

func (x *ListTeamsForAdminUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTeamsForAdminUserRequest.ProtoReflect.Descriptor instead.
func (*ListTeamsForAdminUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{20}
}

func (x *ListTeamsForAdminUserRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListTeamsForAdminUserRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListTeamsForAdminUserRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListTeamsForAdminUserRequest) GetMinimalInfo() bool {
	if x != nil {
		return x.MinimalInfo
	}
	return false
}

type ListTeamsForAdminUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Teams         []*gen.Team            `protobuf:"bytes,1,rep,name=teams,proto3" json:"teams,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTeamsForAdminUserResponse) Reset() {
	*x = ListTeamsForAdminUserResponse{}
	mi := &file_product_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTeamsForAdminUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTeamsForAdminUserResponse) ProtoMessage() {}

func (x *ListTeamsForAdminUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTeamsForAdminUserResponse.ProtoReflect.Descriptor instead.
func (*ListTeamsForAdminUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{21}
}

func (x *ListTeamsForAdminUserResponse) GetTeams() []*gen.Team {
	if x != nil {
		return x.Teams
	}
	return nil
}

func (x *ListTeamsForAdminUserResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetTeamForUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTeamForUserRequest) Reset() {
	*x = GetTeamForUserRequest{}
	mi := &file_product_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTeamForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTeamForUserRequest) ProtoMessage() {}

func (x *GetTeamForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTeamForUserRequest.ProtoReflect.Descriptor instead.
func (*GetTeamForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{22}
}

func (x *GetTeamForUserRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type CreateTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Team          *gen.Team              `protobuf:"bytes,1,opt,name=team,proto3" json:"team,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateTeamRequest) Reset() {
	*x = CreateTeamRequest{}
	mi := &file_product_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateTeamRequest) ProtoMessage() {}

func (x *CreateTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateTeamRequest.ProtoReflect.Descriptor instead.
func (*CreateTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{23}
}

func (x *CreateTeamRequest) GetTeam() *gen.Team {
	if x != nil {
		return x.Team
	}
	return nil
}

type UpdateTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Team          *gen.Team              `protobuf:"bytes,1,opt,name=team,proto3" json:"team,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateTeamRequest) Reset() {
	*x = UpdateTeamRequest{}
	mi := &file_product_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateTeamRequest) ProtoMessage() {}

func (x *UpdateTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateTeamRequest.ProtoReflect.Descriptor instead.
func (*UpdateTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{24}
}

func (x *UpdateTeamRequest) GetTeam() *gen.Team {
	if x != nil {
		return x.Team
	}
	return nil
}

func (x *UpdateTeamRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

// connection start
type GetConnectionWithSecretRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ConnectionUuid string                 `protobuf:"bytes,1,opt,name=connection_uuid,json=connectionUuid,proto3" json:"connection_uuid,omitempty"`
	TeamUuid       string                 `protobuf:"bytes,2,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetConnectionWithSecretRequest) Reset() {
	*x = GetConnectionWithSecretRequest{}
	mi := &file_product_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetConnectionWithSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetConnectionWithSecretRequest) ProtoMessage() {}

func (x *GetConnectionWithSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetConnectionWithSecretRequest.ProtoReflect.Descriptor instead.
func (*GetConnectionWithSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{25}
}

func (x *GetConnectionWithSecretRequest) GetConnectionUuid() string {
	if x != nil {
		return x.ConnectionUuid
	}
	return ""
}

func (x *GetConnectionWithSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type CreateConnectionSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	Connection    *gen.Connection        `protobuf:"bytes,2,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateConnectionSecretRequest) Reset() {
	*x = CreateConnectionSecretRequest{}
	mi := &file_product_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateConnectionSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateConnectionSecretRequest) ProtoMessage() {}

func (x *CreateConnectionSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateConnectionSecretRequest.ProtoReflect.Descriptor instead.
func (*CreateConnectionSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{26}
}

func (x *CreateConnectionSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *CreateConnectionSecretRequest) GetConnection() *gen.Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type UpdateConnectionSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	Connection    *gen.Connection        `protobuf:"bytes,2,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateConnectionSecretRequest) Reset() {
	*x = UpdateConnectionSecretRequest{}
	mi := &file_product_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateConnectionSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateConnectionSecretRequest) ProtoMessage() {}

func (x *UpdateConnectionSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateConnectionSecretRequest.ProtoReflect.Descriptor instead.
func (*UpdateConnectionSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{27}
}

func (x *UpdateConnectionSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *UpdateConnectionSecretRequest) GetConnection() *gen.Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type DeleteConnectionSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	Connection    *gen.Connection        `protobuf:"bytes,2,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteConnectionSecretRequest) Reset() {
	*x = DeleteConnectionSecretRequest{}
	mi := &file_product_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteConnectionSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteConnectionSecretRequest) ProtoMessage() {}

func (x *DeleteConnectionSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteConnectionSecretRequest.ProtoReflect.Descriptor instead.
func (*DeleteConnectionSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{28}
}

func (x *DeleteConnectionSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *DeleteConnectionSecretRequest) GetConnection() *gen.Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

// project start
type ListProjectsForUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	TeamUuid      string                 `protobuf:"bytes,4,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListProjectsForUserRequest) Reset() {
	*x = ListProjectsForUserRequest{}
	mi := &file_product_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectsForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectsForUserRequest) ProtoMessage() {}

func (x *ListProjectsForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectsForUserRequest.ProtoReflect.Descriptor instead.
func (*ListProjectsForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{29}
}

func (x *ListProjectsForUserRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListProjectsForUserRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListProjectsForUserRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListProjectsForUserRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type ListProjectsForUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Projects      []*gen.Project         `protobuf:"bytes,1,rep,name=projects,proto3" json:"projects,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListProjectsForUserResponse) Reset() {
	*x = ListProjectsForUserResponse{}
	mi := &file_product_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectsForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectsForUserResponse) ProtoMessage() {}

func (x *ListProjectsForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectsForUserResponse.ProtoReflect.Descriptor instead.
func (*ListProjectsForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{30}
}

func (x *ListProjectsForUserResponse) GetProjects() []*gen.Project {
	if x != nil {
		return x.Projects
	}
	return nil
}

func (x *ListProjectsForUserResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetProjectForUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetProjectForUserRequest) Reset() {
	*x = GetProjectForUserRequest{}
	mi := &file_product_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectForUserRequest) ProtoMessage() {}

func (x *GetProjectForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectForUserRequest.ProtoReflect.Descriptor instead.
func (*GetProjectForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{31}
}

func (x *GetProjectForUserRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type CreateProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Project       *gen.Project           `protobuf:"bytes,1,opt,name=project,proto3" json:"project,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateProjectRequest) Reset() {
	*x = CreateProjectRequest{}
	mi := &file_product_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateProjectRequest) ProtoMessage() {}

func (x *CreateProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateProjectRequest.ProtoReflect.Descriptor instead.
func (*CreateProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{32}
}

func (x *CreateProjectRequest) GetProject() *gen.Project {
	if x != nil {
		return x.Project
	}
	return nil
}

type UpdateProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Project       *gen.Project           `protobuf:"bytes,1,opt,name=project,proto3" json:"project,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateProjectRequest) Reset() {
	*x = UpdateProjectRequest{}
	mi := &file_product_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateProjectRequest) ProtoMessage() {}

func (x *UpdateProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateProjectRequest.ProtoReflect.Descriptor instead.
func (*UpdateProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{33}
}

func (x *UpdateProjectRequest) GetProject() *gen.Project {
	if x != nil {
		return x.Project
	}
	return nil
}

func (x *UpdateProjectRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type GetProjectNameRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetProjectNameRequest) Reset() {
	*x = GetProjectNameRequest{}
	mi := &file_product_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectNameRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectNameRequest) ProtoMessage() {}

func (x *GetProjectNameRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectNameRequest.ProtoReflect.Descriptor instead.
func (*GetProjectNameRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{34}
}

func (x *GetProjectNameRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type GetProjectNameResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectName   string                 `protobuf:"bytes,1,opt,name=project_name,json=projectName,proto3" json:"project_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetProjectNameResponse) Reset() {
	*x = GetProjectNameResponse{}
	mi := &file_product_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectNameResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectNameResponse) ProtoMessage() {}

func (x *GetProjectNameResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectNameResponse.ProtoReflect.Descriptor instead.
func (*GetProjectNameResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{35}
}

func (x *GetProjectNameResponse) GetProjectName() string {
	if x != nil {
		return x.ProjectName
	}
	return ""
}

// project version start
type ListProjectVersionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	ProjectUuid   string                 `protobuf:"bytes,3,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListProjectVersionsRequest) Reset() {
	*x = ListProjectVersionsRequest{}
	mi := &file_product_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectVersionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectVersionsRequest) ProtoMessage() {}

func (x *ListProjectVersionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectVersionsRequest.ProtoReflect.Descriptor instead.
func (*ListProjectVersionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{36}
}

func (x *ListProjectVersionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListProjectVersionsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListProjectVersionsRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type ListProjectVersionsResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersions []*gen.ProjectVersion  `protobuf:"bytes,1,rep,name=project_versions,json=projectVersions,proto3" json:"project_versions,omitempty"`
	NextPageToken   string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ListProjectVersionsResponse) Reset() {
	*x = ListProjectVersionsResponse{}
	mi := &file_product_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectVersionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectVersionsResponse) ProtoMessage() {}

func (x *ListProjectVersionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectVersionsResponse.ProtoReflect.Descriptor instead.
func (*ListProjectVersionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{37}
}

func (x *ListProjectVersionsResponse) GetProjectVersions() []*gen.ProjectVersion {
	if x != nil {
		return x.ProjectVersions
	}
	return nil
}

func (x *ListProjectVersionsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type ListProjectVersionsForUserRequest struct {
	state             protoimpl.MessageState         `protogen:"open.v1"`
	PageSize          int32                          `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken         string                         `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	ProjectUuid       string                         `protobuf:"bytes,3,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ReviewStatus      gen.ProjectVersionReviewStatus `protobuf:"varint,4,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ProjectVersionReviewStatus" json:"review_status,omitempty"`
	ExcludeJsonFields bool                           `protobuf:"varint,5,opt,name=exclude_json_fields,json=excludeJsonFields,proto3" json:"exclude_json_fields,omitempty"` // if true, will exclude large JSON fields to reduce payload size
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListProjectVersionsForUserRequest) Reset() {
	*x = ListProjectVersionsForUserRequest{}
	mi := &file_product_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectVersionsForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectVersionsForUserRequest) ProtoMessage() {}

func (x *ListProjectVersionsForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectVersionsForUserRequest.ProtoReflect.Descriptor instead.
func (*ListProjectVersionsForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{38}
}

func (x *ListProjectVersionsForUserRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListProjectVersionsForUserRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListProjectVersionsForUserRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ListProjectVersionsForUserRequest) GetReviewStatus() gen.ProjectVersionReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ProjectVersionReviewStatus(0)
}

func (x *ListProjectVersionsForUserRequest) GetExcludeJsonFields() bool {
	if x != nil {
		return x.ExcludeJsonFields
	}
	return false
}

type ListProjectVersionsForUserResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersions []*gen.ProjectVersion  `protobuf:"bytes,1,rep,name=project_versions,json=projectVersions,proto3" json:"project_versions,omitempty"`
	NextPageToken   string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ListProjectVersionsForUserResponse) Reset() {
	*x = ListProjectVersionsForUserResponse{}
	mi := &file_product_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListProjectVersionsForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListProjectVersionsForUserResponse) ProtoMessage() {}

func (x *ListProjectVersionsForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListProjectVersionsForUserResponse.ProtoReflect.Descriptor instead.
func (*ListProjectVersionsForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{39}
}

func (x *ListProjectVersionsForUserResponse) GetProjectVersions() []*gen.ProjectVersion {
	if x != nil {
		return x.ProjectVersions
	}
	return nil
}

func (x *ListProjectVersionsForUserResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetProjectVersionForUserRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetProjectVersionForUserRequest) Reset() {
	*x = GetProjectVersionForUserRequest{}
	mi := &file_product_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectVersionForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectVersionForUserRequest) ProtoMessage() {}

func (x *GetProjectVersionForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectVersionForUserRequest.ProtoReflect.Descriptor instead.
func (*GetProjectVersionForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{40}
}

func (x *GetProjectVersionForUserRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type GetLatestProjectVersionRequest struct {
	state         protoimpl.MessageState         `protogen:"open.v1"`
	ProjectUuid   string                         `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ReviewStatus  gen.ProjectVersionReviewStatus `protobuf:"varint,2,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ProjectVersionReviewStatus" json:"review_status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLatestProjectVersionRequest) Reset() {
	*x = GetLatestProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLatestProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLatestProjectVersionRequest) ProtoMessage() {}

func (x *GetLatestProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLatestProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*GetLatestProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{41}
}

func (x *GetLatestProjectVersionRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetLatestProjectVersionRequest) GetReviewStatus() gen.ProjectVersionReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ProjectVersionReviewStatus(0)
}

type GetLatestProjectVersionForUserRequest struct {
	state         protoimpl.MessageState         `protogen:"open.v1"`
	ProjectUuid   string                         `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ReviewStatus  gen.ProjectVersionReviewStatus `protobuf:"varint,2,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ProjectVersionReviewStatus" json:"review_status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLatestProjectVersionForUserRequest) Reset() {
	*x = GetLatestProjectVersionForUserRequest{}
	mi := &file_product_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLatestProjectVersionForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLatestProjectVersionForUserRequest) ProtoMessage() {}

func (x *GetLatestProjectVersionForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLatestProjectVersionForUserRequest.ProtoReflect.Descriptor instead.
func (*GetLatestProjectVersionForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{42}
}

func (x *GetLatestProjectVersionForUserRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetLatestProjectVersionForUserRequest) GetReviewStatus() gen.ProjectVersionReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ProjectVersionReviewStatus(0)
}

type GetLatestProjectVersionUUIDForUserRequest struct {
	state         protoimpl.MessageState         `protogen:"open.v1"`
	ProjectUuid   string                         `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ReviewStatus  gen.ProjectVersionReviewStatus `protobuf:"varint,2,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ProjectVersionReviewStatus" json:"review_status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLatestProjectVersionUUIDForUserRequest) Reset() {
	*x = GetLatestProjectVersionUUIDForUserRequest{}
	mi := &file_product_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLatestProjectVersionUUIDForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLatestProjectVersionUUIDForUserRequest) ProtoMessage() {}

func (x *GetLatestProjectVersionUUIDForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLatestProjectVersionUUIDForUserRequest.ProtoReflect.Descriptor instead.
func (*GetLatestProjectVersionUUIDForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{43}
}

func (x *GetLatestProjectVersionUUIDForUserRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetLatestProjectVersionUUIDForUserRequest) GetReviewStatus() gen.ProjectVersionReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ProjectVersionReviewStatus(0)
}

type GetLatestProjectVersionUUIDForUserResponse struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetLatestProjectVersionUUIDForUserResponse) Reset() {
	*x = GetLatestProjectVersionUUIDForUserResponse{}
	mi := &file_product_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLatestProjectVersionUUIDForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLatestProjectVersionUUIDForUserResponse) ProtoMessage() {}

func (x *GetLatestProjectVersionUUIDForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLatestProjectVersionUUIDForUserResponse.ProtoReflect.Descriptor instead.
func (*GetLatestProjectVersionUUIDForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{44}
}

func (x *GetLatestProjectVersionUUIDForUserResponse) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type GetProjectVersionVersionRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetProjectVersionVersionRequest) Reset() {
	*x = GetProjectVersionVersionRequest{}
	mi := &file_product_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectVersionVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectVersionVersionRequest) ProtoMessage() {}

func (x *GetProjectVersionVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectVersionVersionRequest.ProtoReflect.Descriptor instead.
func (*GetProjectVersionVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{45}
}

func (x *GetProjectVersionVersionRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type GetProjectVersionVersionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       int64                  `protobuf:"varint,1,opt,name=version,proto3" json:"version,omitempty"` // timestamp
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetProjectVersionVersionResponse) Reset() {
	*x = GetProjectVersionVersionResponse{}
	mi := &file_product_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectVersionVersionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectVersionVersionResponse) ProtoMessage() {}

func (x *GetProjectVersionVersionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectVersionVersionResponse.ProtoReflect.Descriptor instead.
func (*GetProjectVersionVersionResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{46}
}

func (x *GetProjectVersionVersionResponse) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

type GetProjectVersionIdentifierRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetProjectVersionIdentifierRequest) Reset() {
	*x = GetProjectVersionIdentifierRequest{}
	mi := &file_product_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectVersionIdentifierRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectVersionIdentifierRequest) ProtoMessage() {}

func (x *GetProjectVersionIdentifierRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectVersionIdentifierRequest.ProtoReflect.Descriptor instead.
func (*GetProjectVersionIdentifierRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{47}
}

func (x *GetProjectVersionIdentifierRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type GetProjectVersionIdentifierResponse struct {
	state                    protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionIdentifier string                 `protobuf:"bytes,1,opt,name=project_version_identifier,json=projectVersionIdentifier,proto3" json:"project_version_identifier,omitempty"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *GetProjectVersionIdentifierResponse) Reset() {
	*x = GetProjectVersionIdentifierResponse{}
	mi := &file_product_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetProjectVersionIdentifierResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetProjectVersionIdentifierResponse) ProtoMessage() {}

func (x *GetProjectVersionIdentifierResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetProjectVersionIdentifierResponse.ProtoReflect.Descriptor instead.
func (*GetProjectVersionIdentifierResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{48}
}

func (x *GetProjectVersionIdentifierResponse) GetProjectVersionIdentifier() string {
	if x != nil {
		return x.ProjectVersionIdentifier
	}
	return ""
}

type CreateProjectVersionRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersion *gen.ProjectVersion    `protobuf:"bytes,1,opt,name=project_version,json=projectVersion,proto3" json:"project_version,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CreateProjectVersionRequest) Reset() {
	*x = CreateProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateProjectVersionRequest) ProtoMessage() {}

func (x *CreateProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*CreateProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{49}
}

func (x *CreateProjectVersionRequest) GetProjectVersion() *gen.ProjectVersion {
	if x != nil {
		return x.ProjectVersion
	}
	return nil
}

type CreateProjectVersionFromJSONRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionJson string                 `protobuf:"bytes,1,opt,name=project_version_json,json=projectVersionJson,proto3" json:"project_version_json,omitempty"` // the JSON representation of the project version
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *CreateProjectVersionFromJSONRequest) Reset() {
	*x = CreateProjectVersionFromJSONRequest{}
	mi := &file_product_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateProjectVersionFromJSONRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateProjectVersionFromJSONRequest) ProtoMessage() {}

func (x *CreateProjectVersionFromJSONRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateProjectVersionFromJSONRequest.ProtoReflect.Descriptor instead.
func (*CreateProjectVersionFromJSONRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{50}
}

func (x *CreateProjectVersionFromJSONRequest) GetProjectVersionJson() string {
	if x != nil {
		return x.ProjectVersionJson
	}
	return ""
}

type UpdateProjectVersionRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersion *gen.ProjectVersion    `protobuf:"bytes,1,opt,name=project_version,json=projectVersion,proto3" json:"project_version,omitempty"`
	UpdateMask     *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UpdateProjectVersionRequest) Reset() {
	*x = UpdateProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateProjectVersionRequest) ProtoMessage() {}

func (x *UpdateProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*UpdateProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{51}
}

func (x *UpdateProjectVersionRequest) GetProjectVersion() *gen.ProjectVersion {
	if x != nil {
		return x.ProjectVersion
	}
	return nil
}

func (x *UpdateProjectVersionRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type CreateDraftProjectVersionForExistingProjectRequest struct {
	state                     protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid               string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	Identifier                string                 `protobuf:"bytes,2,opt,name=identifier,proto3" json:"identifier,omitempty"`
	BasedOnProjectVersionUuid string                 `protobuf:"bytes,3,opt,name=based_on_project_version_uuid,json=basedOnProjectVersionUuid,proto3" json:"based_on_project_version_uuid,omitempty"`
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *CreateDraftProjectVersionForExistingProjectRequest) Reset() {
	*x = CreateDraftProjectVersionForExistingProjectRequest{}
	mi := &file_product_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateDraftProjectVersionForExistingProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateDraftProjectVersionForExistingProjectRequest) ProtoMessage() {}

func (x *CreateDraftProjectVersionForExistingProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateDraftProjectVersionForExistingProjectRequest.ProtoReflect.Descriptor instead.
func (*CreateDraftProjectVersionForExistingProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{52}
}

func (x *CreateDraftProjectVersionForExistingProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *CreateDraftProjectVersionForExistingProjectRequest) GetIdentifier() string {
	if x != nil {
		return x.Identifier
	}
	return ""
}

func (x *CreateDraftProjectVersionForExistingProjectRequest) GetBasedOnProjectVersionUuid() string {
	if x != nil {
		return x.BasedOnProjectVersionUuid
	}
	return ""
}

type DiscardDraftProjectVersionRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *DiscardDraftProjectVersionRequest) Reset() {
	*x = DiscardDraftProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DiscardDraftProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DiscardDraftProjectVersionRequest) ProtoMessage() {}

func (x *DiscardDraftProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DiscardDraftProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*DiscardDraftProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{53}
}

func (x *DiscardDraftProjectVersionRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type SendProjectVersionForReviewRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ReviewDescription  string                 `protobuf:"bytes,2,opt,name=review_description,json=reviewDescription,proto3" json:"review_description,omitempty"`
	AiGenerated        bool                   `protobuf:"varint,3,opt,name=ai_generated,json=aiGenerated,proto3" json:"ai_generated,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *SendProjectVersionForReviewRequest) Reset() {
	*x = SendProjectVersionForReviewRequest{}
	mi := &file_product_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendProjectVersionForReviewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendProjectVersionForReviewRequest) ProtoMessage() {}

func (x *SendProjectVersionForReviewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendProjectVersionForReviewRequest.ProtoReflect.Descriptor instead.
func (*SendProjectVersionForReviewRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{54}
}

func (x *SendProjectVersionForReviewRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *SendProjectVersionForReviewRequest) GetReviewDescription() string {
	if x != nil {
		return x.ReviewDescription
	}
	return ""
}

func (x *SendProjectVersionForReviewRequest) GetAiGenerated() bool {
	if x != nil {
		return x.AiGenerated
	}
	return false
}

type WithdrawFromReviewProjectVersionRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *WithdrawFromReviewProjectVersionRequest) Reset() {
	*x = WithdrawFromReviewProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WithdrawFromReviewProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WithdrawFromReviewProjectVersionRequest) ProtoMessage() {}

func (x *WithdrawFromReviewProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WithdrawFromReviewProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*WithdrawFromReviewProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{55}
}

func (x *WithdrawFromReviewProjectVersionRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type ReviewProjectVersionRequest struct {
	state              protoimpl.MessageState         `protogen:"open.v1"`
	ProjectVersionUuid string                         `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ChangeRequestUuid  string                         `protobuf:"bytes,2,opt,name=change_request_uuid,json=changeRequestUuid,proto3" json:"change_request_uuid,omitempty"`
	ReviewStatus       gen.ProjectVersionReviewStatus `protobuf:"varint,3,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ProjectVersionReviewStatus" json:"review_status,omitempty"` // APPROVED, REJECTED
	ReviewComment      string                         `protobuf:"bytes,4,opt,name=review_comment,json=reviewComment,proto3" json:"review_comment,omitempty"`                                   // optional
	PublishChanges     bool                           `protobuf:"varint,5,opt,name=publish_changes,json=publishChanges,proto3" json:"publish_changes,omitempty"`                               // the changes will be published if approved
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *ReviewProjectVersionRequest) Reset() {
	*x = ReviewProjectVersionRequest{}
	mi := &file_product_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReviewProjectVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReviewProjectVersionRequest) ProtoMessage() {}

func (x *ReviewProjectVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReviewProjectVersionRequest.ProtoReflect.Descriptor instead.
func (*ReviewProjectVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{56}
}

func (x *ReviewProjectVersionRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *ReviewProjectVersionRequest) GetChangeRequestUuid() string {
	if x != nil {
		return x.ChangeRequestUuid
	}
	return ""
}

func (x *ReviewProjectVersionRequest) GetReviewStatus() gen.ProjectVersionReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ProjectVersionReviewStatus(0)
}

func (x *ReviewProjectVersionRequest) GetReviewComment() string {
	if x != nil {
		return x.ReviewComment
	}
	return ""
}

func (x *ReviewProjectVersionRequest) GetPublishChanges() bool {
	if x != nil {
		return x.PublishChanges
	}
	return false
}

// files start
type UploadProjectVersionSnapshotRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid        string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ImageData          []byte                 `protobuf:"bytes,3,opt,name=image_data,json=imageData,proto3" json:"image_data,omitempty"` // base64 encoded image data
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *UploadProjectVersionSnapshotRequest) Reset() {
	*x = UploadProjectVersionSnapshotRequest{}
	mi := &file_product_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadProjectVersionSnapshotRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadProjectVersionSnapshotRequest) ProtoMessage() {}

func (x *UploadProjectVersionSnapshotRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadProjectVersionSnapshotRequest.ProtoReflect.Descriptor instead.
func (*UploadProjectVersionSnapshotRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{57}
}

func (x *UploadProjectVersionSnapshotRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *UploadProjectVersionSnapshotRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *UploadProjectVersionSnapshotRequest) GetImageData() []byte {
	if x != nil {
		return x.ImageData
	}
	return nil
}

type UploadProjectVersionSnapshotResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadProjectVersionSnapshotResponse) Reset() {
	*x = UploadProjectVersionSnapshotResponse{}
	mi := &file_product_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadProjectVersionSnapshotResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadProjectVersionSnapshotResponse) ProtoMessage() {}

func (x *UploadProjectVersionSnapshotResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadProjectVersionSnapshotResponse.ProtoReflect.Descriptor instead.
func (*UploadProjectVersionSnapshotResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{58}
}

func (x *UploadProjectVersionSnapshotResponse) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type GetSignedFileURLRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSignedFileURLRequest) Reset() {
	*x = GetSignedFileURLRequest{}
	mi := &file_product_proto_msgTypes[59]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSignedFileURLRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSignedFileURLRequest) ProtoMessage() {}

func (x *GetSignedFileURLRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[59]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSignedFileURLRequest.ProtoReflect.Descriptor instead.
func (*GetSignedFileURLRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{59}
}

func (x *GetSignedFileURLRequest) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type GetSignedFileURLResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSignedFileURLResponse) Reset() {
	*x = GetSignedFileURLResponse{}
	mi := &file_product_proto_msgTypes[60]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSignedFileURLResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSignedFileURLResponse) ProtoMessage() {}

func (x *GetSignedFileURLResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[60]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSignedFileURLResponse.ProtoReflect.Descriptor instead.
func (*GetSignedFileURLResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{60}
}

func (x *GetSignedFileURLResponse) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type UploadExtensionExecutionFileRequest struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid          string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid   string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ExtensionUuid        string                 `protobuf:"bytes,3,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	ExtensionVersionUuid string                 `protobuf:"bytes,4,opt,name=extension_version_uuid,json=extensionVersionUuid,proto3" json:"extension_version_uuid,omitempty"`
	ExecutionUuid        string                 `protobuf:"bytes,5,opt,name=execution_uuid,json=executionUuid,proto3" json:"execution_uuid,omitempty"`
	Data                 []byte                 `protobuf:"bytes,6,opt,name=data,proto3" json:"data,omitempty"`
	FileExtension        string                 `protobuf:"bytes,7,opt,name=file_extension,json=fileExtension,proto3" json:"file_extension,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *UploadExtensionExecutionFileRequest) Reset() {
	*x = UploadExtensionExecutionFileRequest{}
	mi := &file_product_proto_msgTypes[61]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadExtensionExecutionFileRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadExtensionExecutionFileRequest) ProtoMessage() {}

func (x *UploadExtensionExecutionFileRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[61]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadExtensionExecutionFileRequest.ProtoReflect.Descriptor instead.
func (*UploadExtensionExecutionFileRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{61}
}

func (x *UploadExtensionExecutionFileRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *UploadExtensionExecutionFileRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *UploadExtensionExecutionFileRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

func (x *UploadExtensionExecutionFileRequest) GetExtensionVersionUuid() string {
	if x != nil {
		return x.ExtensionVersionUuid
	}
	return ""
}

func (x *UploadExtensionExecutionFileRequest) GetExecutionUuid() string {
	if x != nil {
		return x.ExecutionUuid
	}
	return ""
}

func (x *UploadExtensionExecutionFileRequest) GetData() []byte {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *UploadExtensionExecutionFileRequest) GetFileExtension() string {
	if x != nil {
		return x.FileExtension
	}
	return ""
}

type UploadExtensionExecutionFileResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filepath      string                 `protobuf:"bytes,1,opt,name=filepath,proto3" json:"filepath,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadExtensionExecutionFileResponse) Reset() {
	*x = UploadExtensionExecutionFileResponse{}
	mi := &file_product_proto_msgTypes[62]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadExtensionExecutionFileResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadExtensionExecutionFileResponse) ProtoMessage() {}

func (x *UploadExtensionExecutionFileResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[62]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadExtensionExecutionFileResponse.ProtoReflect.Descriptor instead.
func (*UploadExtensionExecutionFileResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{62}
}

func (x *UploadExtensionExecutionFileResponse) GetFilepath() string {
	if x != nil {
		return x.Filepath
	}
	return ""
}

type GetExtensionExecutionFileRequest struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid          string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid   string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ExtensionUuid        string                 `protobuf:"bytes,3,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	ExtensionVersionUuid string                 `protobuf:"bytes,4,opt,name=extension_version_uuid,json=extensionVersionUuid,proto3" json:"extension_version_uuid,omitempty"`
	ExecutionUuid        string                 `protobuf:"bytes,5,opt,name=execution_uuid,json=executionUuid,proto3" json:"execution_uuid,omitempty"`
	FileExtension        string                 `protobuf:"bytes,6,opt,name=file_extension,json=fileExtension,proto3" json:"file_extension,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *GetExtensionExecutionFileRequest) Reset() {
	*x = GetExtensionExecutionFileRequest{}
	mi := &file_product_proto_msgTypes[63]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExtensionExecutionFileRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExtensionExecutionFileRequest) ProtoMessage() {}

func (x *GetExtensionExecutionFileRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[63]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExtensionExecutionFileRequest.ProtoReflect.Descriptor instead.
func (*GetExtensionExecutionFileRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{63}
}

func (x *GetExtensionExecutionFileRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetExtensionExecutionFileRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *GetExtensionExecutionFileRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

func (x *GetExtensionExecutionFileRequest) GetExtensionVersionUuid() string {
	if x != nil {
		return x.ExtensionVersionUuid
	}
	return ""
}

func (x *GetExtensionExecutionFileRequest) GetExecutionUuid() string {
	if x != nil {
		return x.ExecutionUuid
	}
	return ""
}

func (x *GetExtensionExecutionFileRequest) GetFileExtension() string {
	if x != nil {
		return x.FileExtension
	}
	return ""
}

type GetExtensionExecutionFileResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filepath      string                 `protobuf:"bytes,1,opt,name=filepath,proto3" json:"filepath,omitempty"`
	SignedUrl     string                 `protobuf:"bytes,2,opt,name=signed_url,json=signedUrl,proto3" json:"signed_url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExtensionExecutionFileResponse) Reset() {
	*x = GetExtensionExecutionFileResponse{}
	mi := &file_product_proto_msgTypes[64]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExtensionExecutionFileResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExtensionExecutionFileResponse) ProtoMessage() {}

func (x *GetExtensionExecutionFileResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExtensionExecutionFileResponse.ProtoReflect.Descriptor instead.
func (*GetExtensionExecutionFileResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{64}
}

func (x *GetExtensionExecutionFileResponse) GetFilepath() string {
	if x != nil {
		return x.Filepath
	}
	return ""
}

func (x *GetExtensionExecutionFileResponse) GetSignedUrl() string {
	if x != nil {
		return x.SignedUrl
	}
	return ""
}

type UploadExtensionIconRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ExtensionUuid string                 `protobuf:"bytes,1,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	ImageData     []byte                 `protobuf:"bytes,2,opt,name=image_data,json=imageData,proto3" json:"image_data,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadExtensionIconRequest) Reset() {
	*x = UploadExtensionIconRequest{}
	mi := &file_product_proto_msgTypes[65]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadExtensionIconRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadExtensionIconRequest) ProtoMessage() {}

func (x *UploadExtensionIconRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[65]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadExtensionIconRequest.ProtoReflect.Descriptor instead.
func (*UploadExtensionIconRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{65}
}

func (x *UploadExtensionIconRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

func (x *UploadExtensionIconRequest) GetImageData() []byte {
	if x != nil {
		return x.ImageData
	}
	return nil
}

type UploadExtensionIconResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filepath      string                 `protobuf:"bytes,1,opt,name=filepath,proto3" json:"filepath,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadExtensionIconResponse) Reset() {
	*x = UploadExtensionIconResponse{}
	mi := &file_product_proto_msgTypes[66]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadExtensionIconResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadExtensionIconResponse) ProtoMessage() {}

func (x *UploadExtensionIconResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[66]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadExtensionIconResponse.ProtoReflect.Descriptor instead.
func (*UploadExtensionIconResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{66}
}

func (x *UploadExtensionIconResponse) GetFilepath() string {
	if x != nil {
		return x.Filepath
	}
	return ""
}

type UploadDataManagerMetadataRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid        string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	Metadata           string                 `protobuf:"bytes,3,opt,name=metadata,proto3" json:"metadata,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *UploadDataManagerMetadataRequest) Reset() {
	*x = UploadDataManagerMetadataRequest{}
	mi := &file_product_proto_msgTypes[67]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadDataManagerMetadataRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadDataManagerMetadataRequest) ProtoMessage() {}

func (x *UploadDataManagerMetadataRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[67]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadDataManagerMetadataRequest.ProtoReflect.Descriptor instead.
func (*UploadDataManagerMetadataRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{67}
}

func (x *UploadDataManagerMetadataRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *UploadDataManagerMetadataRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *UploadDataManagerMetadataRequest) GetMetadata() string {
	if x != nil {
		return x.Metadata
	}
	return ""
}

type UploadDataManagerMetadataResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filepath      string                 `protobuf:"bytes,1,opt,name=filepath,proto3" json:"filepath,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadDataManagerMetadataResponse) Reset() {
	*x = UploadDataManagerMetadataResponse{}
	mi := &file_product_proto_msgTypes[68]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadDataManagerMetadataResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadDataManagerMetadataResponse) ProtoMessage() {}

func (x *UploadDataManagerMetadataResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[68]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadDataManagerMetadataResponse.ProtoReflect.Descriptor instead.
func (*UploadDataManagerMetadataResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{68}
}

func (x *UploadDataManagerMetadataResponse) GetFilepath() string {
	if x != nil {
		return x.Filepath
	}
	return ""
}

type UploadRecordFieldFileRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid        string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	EntityUuid         string                 `protobuf:"bytes,3,opt,name=entity_uuid,json=entityUuid,proto3" json:"entity_uuid,omitempty"`
	FieldUuid          string                 `protobuf:"bytes,4,opt,name=field_uuid,json=fieldUuid,proto3" json:"field_uuid,omitempty"`
	FileName           string                 `protobuf:"bytes,5,opt,name=file_name,json=fileName,proto3" json:"file_name,omitempty"` // original file name with extension
	FileData           []byte                 `protobuf:"bytes,6,opt,name=file_data,json=fileData,proto3" json:"file_data,omitempty"`
	ForceOverride      bool                   `protobuf:"varint,7,opt,name=force_override,json=forceOverride,proto3" json:"force_override,omitempty"` // if true, will override existing file with same name
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *UploadRecordFieldFileRequest) Reset() {
	*x = UploadRecordFieldFileRequest{}
	mi := &file_product_proto_msgTypes[69]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadRecordFieldFileRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadRecordFieldFileRequest) ProtoMessage() {}

func (x *UploadRecordFieldFileRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[69]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadRecordFieldFileRequest.ProtoReflect.Descriptor instead.
func (*UploadRecordFieldFileRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{69}
}

func (x *UploadRecordFieldFileRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *UploadRecordFieldFileRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *UploadRecordFieldFileRequest) GetEntityUuid() string {
	if x != nil {
		return x.EntityUuid
	}
	return ""
}

func (x *UploadRecordFieldFileRequest) GetFieldUuid() string {
	if x != nil {
		return x.FieldUuid
	}
	return ""
}

func (x *UploadRecordFieldFileRequest) GetFileName() string {
	if x != nil {
		return x.FileName
	}
	return ""
}

func (x *UploadRecordFieldFileRequest) GetFileData() []byte {
	if x != nil {
		return x.FileData
	}
	return nil
}

func (x *UploadRecordFieldFileRequest) GetForceOverride() bool {
	if x != nil {
		return x.ForceOverride
	}
	return false
}

type UploadRecordFieldFileResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadRecordFieldFileResponse) Reset() {
	*x = UploadRecordFieldFileResponse{}
	mi := &file_product_proto_msgTypes[70]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadRecordFieldFileResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadRecordFieldFileResponse) ProtoMessage() {}

func (x *UploadRecordFieldFileResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[70]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadRecordFieldFileResponse.ProtoReflect.Descriptor instead.
func (*UploadRecordFieldFileResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{70}
}

func (x *UploadRecordFieldFileResponse) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type GetRecordFieldSignedFileURLRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid        string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	EntityUuid         string                 `protobuf:"bytes,3,opt,name=entity_uuid,json=entityUuid,proto3" json:"entity_uuid,omitempty"`
	FieldUuid          string                 `protobuf:"bytes,4,opt,name=field_uuid,json=fieldUuid,proto3" json:"field_uuid,omitempty"`
	FileUrl            string                 `protobuf:"bytes,5,opt,name=file_url,json=fileUrl,proto3" json:"file_url,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetRecordFieldSignedFileURLRequest) Reset() {
	*x = GetRecordFieldSignedFileURLRequest{}
	mi := &file_product_proto_msgTypes[71]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRecordFieldSignedFileURLRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRecordFieldSignedFileURLRequest) ProtoMessage() {}

func (x *GetRecordFieldSignedFileURLRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[71]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRecordFieldSignedFileURLRequest.ProtoReflect.Descriptor instead.
func (*GetRecordFieldSignedFileURLRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{71}
}

func (x *GetRecordFieldSignedFileURLRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetRecordFieldSignedFileURLRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *GetRecordFieldSignedFileURLRequest) GetEntityUuid() string {
	if x != nil {
		return x.EntityUuid
	}
	return ""
}

func (x *GetRecordFieldSignedFileURLRequest) GetFieldUuid() string {
	if x != nil {
		return x.FieldUuid
	}
	return ""
}

func (x *GetRecordFieldSignedFileURLRequest) GetFileUrl() string {
	if x != nil {
		return x.FileUrl
	}
	return ""
}

type GetRecordFieldSignedFileURLResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SignedUrl     string                 `protobuf:"bytes,1,opt,name=signed_url,json=signedUrl,proto3" json:"signed_url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRecordFieldSignedFileURLResponse) Reset() {
	*x = GetRecordFieldSignedFileURLResponse{}
	mi := &file_product_proto_msgTypes[72]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRecordFieldSignedFileURLResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRecordFieldSignedFileURLResponse) ProtoMessage() {}

func (x *GetRecordFieldSignedFileURLResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[72]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRecordFieldSignedFileURLResponse.ProtoReflect.Descriptor instead.
func (*GetRecordFieldSignedFileURLResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{72}
}

func (x *GetRecordFieldSignedFileURLResponse) GetSignedUrl() string {
	if x != nil {
		return x.SignedUrl
	}
	return ""
}

type GetRecordFileContentRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid        string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,2,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	EntityUuid         string                 `protobuf:"bytes,3,opt,name=entity_uuid,json=entityUuid,proto3" json:"entity_uuid,omitempty"`
	FieldUuid          string                 `protobuf:"bytes,4,opt,name=field_uuid,json=fieldUuid,proto3" json:"field_uuid,omitempty"`
	FileUrl            string                 `protobuf:"bytes,5,opt,name=file_url,json=fileUrl,proto3" json:"file_url,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetRecordFileContentRequest) Reset() {
	*x = GetRecordFileContentRequest{}
	mi := &file_product_proto_msgTypes[73]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRecordFileContentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRecordFileContentRequest) ProtoMessage() {}

func (x *GetRecordFileContentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[73]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRecordFileContentRequest.ProtoReflect.Descriptor instead.
func (*GetRecordFileContentRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{73}
}

func (x *GetRecordFileContentRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *GetRecordFileContentRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *GetRecordFileContentRequest) GetEntityUuid() string {
	if x != nil {
		return x.EntityUuid
	}
	return ""
}

func (x *GetRecordFileContentRequest) GetFieldUuid() string {
	if x != nil {
		return x.FieldUuid
	}
	return ""
}

func (x *GetRecordFileContentRequest) GetFileUrl() string {
	if x != nil {
		return x.FileUrl
	}
	return ""
}

type GetRecordFileContentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Content       []byte                 `protobuf:"bytes,1,opt,name=content,proto3" json:"content,omitempty"` // base64 encoded content of the file
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRecordFileContentResponse) Reset() {
	*x = GetRecordFileContentResponse{}
	mi := &file_product_proto_msgTypes[74]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRecordFileContentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRecordFileContentResponse) ProtoMessage() {}

func (x *GetRecordFileContentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[74]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRecordFileContentResponse.ProtoReflect.Descriptor instead.
func (*GetRecordFileContentResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{74}
}

func (x *GetRecordFileContentResponse) GetContent() []byte {
	if x != nil {
		return x.Content
	}
	return nil
}

type UploadUserAvatarRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ImageData     []byte                 `protobuf:"bytes,1,opt,name=image_data,json=imageData,proto3" json:"image_data,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadUserAvatarRequest) Reset() {
	*x = UploadUserAvatarRequest{}
	mi := &file_product_proto_msgTypes[75]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadUserAvatarRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadUserAvatarRequest) ProtoMessage() {}

func (x *UploadUserAvatarRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[75]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadUserAvatarRequest.ProtoReflect.Descriptor instead.
func (*UploadUserAvatarRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{75}
}

func (x *UploadUserAvatarRequest) GetImageData() []byte {
	if x != nil {
		return x.ImageData
	}
	return nil
}

type UploadUserAvatarResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Url           string                 `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UploadUserAvatarResponse) Reset() {
	*x = UploadUserAvatarResponse{}
	mi := &file_product_proto_msgTypes[76]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UploadUserAvatarResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UploadUserAvatarResponse) ProtoMessage() {}

func (x *UploadUserAvatarResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[76]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UploadUserAvatarResponse.ProtoReflect.Descriptor instead.
func (*UploadUserAvatarResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{76}
}

func (x *UploadUserAvatarResponse) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

// extension start
type ListExtensionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	OwnerUuid     string                 `protobuf:"bytes,4,opt,name=owner_uuid,json=ownerUuid,proto3" json:"owner_uuid,omitempty"`
	PublishedOnly bool                   `protobuf:"varint,5,opt,name=published_only,json=publishedOnly,proto3" json:"published_only,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListExtensionsRequest) Reset() {
	*x = ListExtensionsRequest{}
	mi := &file_product_proto_msgTypes[77]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionsRequest) ProtoMessage() {}

func (x *ListExtensionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[77]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionsRequest.ProtoReflect.Descriptor instead.
func (*ListExtensionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{77}
}

func (x *ListExtensionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListExtensionsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListExtensionsRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListExtensionsRequest) GetOwnerUuid() string {
	if x != nil {
		return x.OwnerUuid
	}
	return ""
}

func (x *ListExtensionsRequest) GetPublishedOnly() bool {
	if x != nil {
		return x.PublishedOnly
	}
	return false
}

type ListExtensionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Extensions    []*gen.Extension       `protobuf:"bytes,1,rep,name=extensions,proto3" json:"extensions,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListExtensionsResponse) Reset() {
	*x = ListExtensionsResponse{}
	mi := &file_product_proto_msgTypes[78]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionsResponse) ProtoMessage() {}

func (x *ListExtensionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[78]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionsResponse.ProtoReflect.Descriptor instead.
func (*ListExtensionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{78}
}

func (x *ListExtensionsResponse) GetExtensions() []*gen.Extension {
	if x != nil {
		return x.Extensions
	}
	return nil
}

func (x *ListExtensionsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetExtensionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ExtensionUuid string                 `protobuf:"bytes,1,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExtensionRequest) Reset() {
	*x = GetExtensionRequest{}
	mi := &file_product_proto_msgTypes[79]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExtensionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExtensionRequest) ProtoMessage() {}

func (x *GetExtensionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[79]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExtensionRequest.ProtoReflect.Descriptor instead.
func (*GetExtensionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{79}
}

func (x *GetExtensionRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

type CreateExtensionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Extension     *gen.Extension         `protobuf:"bytes,1,opt,name=extension,proto3" json:"extension,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateExtensionRequest) Reset() {
	*x = CreateExtensionRequest{}
	mi := &file_product_proto_msgTypes[80]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateExtensionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateExtensionRequest) ProtoMessage() {}

func (x *CreateExtensionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[80]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateExtensionRequest.ProtoReflect.Descriptor instead.
func (*CreateExtensionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{80}
}

func (x *CreateExtensionRequest) GetExtension() *gen.Extension {
	if x != nil {
		return x.Extension
	}
	return nil
}

type UpdateExtensionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Extension     *gen.Extension         `protobuf:"bytes,1,opt,name=extension,proto3" json:"extension,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateExtensionRequest) Reset() {
	*x = UpdateExtensionRequest{}
	mi := &file_product_proto_msgTypes[81]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateExtensionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateExtensionRequest) ProtoMessage() {}

func (x *UpdateExtensionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[81]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateExtensionRequest.ProtoReflect.Descriptor instead.
func (*UpdateExtensionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{81}
}

func (x *UpdateExtensionRequest) GetExtension() *gen.Extension {
	if x != nil {
		return x.Extension
	}
	return nil
}

func (x *UpdateExtensionRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type ListExtensionVersionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	ExtensionUuid string                 `protobuf:"bytes,4,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListExtensionVersionsRequest) Reset() {
	*x = ListExtensionVersionsRequest{}
	mi := &file_product_proto_msgTypes[82]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionVersionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionVersionsRequest) ProtoMessage() {}

func (x *ListExtensionVersionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[82]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionVersionsRequest.ProtoReflect.Descriptor instead.
func (*ListExtensionVersionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{82}
}

func (x *ListExtensionVersionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListExtensionVersionsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListExtensionVersionsRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListExtensionVersionsRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

type ListExtensionVersionsResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Versions      []*gen.ExtensionVersion `protobuf:"bytes,1,rep,name=versions,proto3" json:"versions,omitempty"`
	NextPageToken string                  `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListExtensionVersionsResponse) Reset() {
	*x = ListExtensionVersionsResponse{}
	mi := &file_product_proto_msgTypes[83]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionVersionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionVersionsResponse) ProtoMessage() {}

func (x *ListExtensionVersionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[83]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionVersionsResponse.ProtoReflect.Descriptor instead.
func (*ListExtensionVersionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{83}
}

func (x *ListExtensionVersionsResponse) GetVersions() []*gen.ExtensionVersion {
	if x != nil {
		return x.Versions
	}
	return nil
}

func (x *ListExtensionVersionsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetExtensionVersionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	VersionUuid   string                 `protobuf:"bytes,1,opt,name=version_uuid,json=versionUuid,proto3" json:"version_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExtensionVersionRequest) Reset() {
	*x = GetExtensionVersionRequest{}
	mi := &file_product_proto_msgTypes[84]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExtensionVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExtensionVersionRequest) ProtoMessage() {}

func (x *GetExtensionVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[84]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExtensionVersionRequest.ProtoReflect.Descriptor instead.
func (*GetExtensionVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{84}
}

func (x *GetExtensionVersionRequest) GetVersionUuid() string {
	if x != nil {
		return x.VersionUuid
	}
	return ""
}

type CreateExtensionVersionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       *gen.ExtensionVersion  `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateExtensionVersionRequest) Reset() {
	*x = CreateExtensionVersionRequest{}
	mi := &file_product_proto_msgTypes[85]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateExtensionVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateExtensionVersionRequest) ProtoMessage() {}

func (x *CreateExtensionVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[85]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateExtensionVersionRequest.ProtoReflect.Descriptor instead.
func (*CreateExtensionVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{85}
}

func (x *CreateExtensionVersionRequest) GetVersion() *gen.ExtensionVersion {
	if x != nil {
		return x.Version
	}
	return nil
}

type UpdateExtensionVersionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       *gen.ExtensionVersion  `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateExtensionVersionRequest) Reset() {
	*x = UpdateExtensionVersionRequest{}
	mi := &file_product_proto_msgTypes[86]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateExtensionVersionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateExtensionVersionRequest) ProtoMessage() {}

func (x *UpdateExtensionVersionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[86]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateExtensionVersionRequest.ProtoReflect.Descriptor instead.
func (*UpdateExtensionVersionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{86}
}

func (x *UpdateExtensionVersionRequest) GetVersion() *gen.ExtensionVersion {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *UpdateExtensionVersionRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type ListExtensionExecutionsRequest struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	PageSize             int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken            string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy              string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	ProjectUuid          string                 `protobuf:"bytes,4,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid   string                 `protobuf:"bytes,5,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ProjectExtensionUuid string                 `protobuf:"bytes,6,opt,name=project_extension_uuid,json=projectExtensionUuid,proto3" json:"project_extension_uuid,omitempty"`
	ExtensionUuid        string                 `protobuf:"bytes,7,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	ExtensionVersionUuid string                 `protobuf:"bytes,8,opt,name=extension_version_uuid,json=extensionVersionUuid,proto3" json:"extension_version_uuid,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *ListExtensionExecutionsRequest) Reset() {
	*x = ListExtensionExecutionsRequest{}
	mi := &file_product_proto_msgTypes[87]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionExecutionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionExecutionsRequest) ProtoMessage() {}

func (x *ListExtensionExecutionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[87]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionExecutionsRequest.ProtoReflect.Descriptor instead.
func (*ListExtensionExecutionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{87}
}

func (x *ListExtensionExecutionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListExtensionExecutionsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetProjectExtensionUuid() string {
	if x != nil {
		return x.ProjectExtensionUuid
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

func (x *ListExtensionExecutionsRequest) GetExtensionVersionUuid() string {
	if x != nil {
		return x.ExtensionVersionUuid
	}
	return ""
}

type ListExtensionExecutionsResponse struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Executions    []*gen.ExtensionExecution `protobuf:"bytes,1,rep,name=executions,proto3" json:"executions,omitempty"`
	NextPageToken string                    `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListExtensionExecutionsResponse) Reset() {
	*x = ListExtensionExecutionsResponse{}
	mi := &file_product_proto_msgTypes[88]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListExtensionExecutionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListExtensionExecutionsResponse) ProtoMessage() {}

func (x *ListExtensionExecutionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[88]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListExtensionExecutionsResponse.ProtoReflect.Descriptor instead.
func (*ListExtensionExecutionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{88}
}

func (x *ListExtensionExecutionsResponse) GetExecutions() []*gen.ExtensionExecution {
	if x != nil {
		return x.Executions
	}
	return nil
}

func (x *ListExtensionExecutionsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetExtensionExecutionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ExecutionUuid string                 `protobuf:"bytes,1,opt,name=execution_uuid,json=executionUuid,proto3" json:"execution_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExtensionExecutionRequest) Reset() {
	*x = GetExtensionExecutionRequest{}
	mi := &file_product_proto_msgTypes[89]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExtensionExecutionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExtensionExecutionRequest) ProtoMessage() {}

func (x *GetExtensionExecutionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[89]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExtensionExecutionRequest.ProtoReflect.Descriptor instead.
func (*GetExtensionExecutionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{89}
}

func (x *GetExtensionExecutionRequest) GetExecutionUuid() string {
	if x != nil {
		return x.ExecutionUuid
	}
	return ""
}

type CreateExtensionExecutionRequest struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Execution     *gen.ExtensionExecution `protobuf:"bytes,1,opt,name=execution,proto3" json:"execution,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateExtensionExecutionRequest) Reset() {
	*x = CreateExtensionExecutionRequest{}
	mi := &file_product_proto_msgTypes[90]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateExtensionExecutionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateExtensionExecutionRequest) ProtoMessage() {}

func (x *CreateExtensionExecutionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[90]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateExtensionExecutionRequest.ProtoReflect.Descriptor instead.
func (*CreateExtensionExecutionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{90}
}

func (x *CreateExtensionExecutionRequest) GetExecution() *gen.ExtensionExecution {
	if x != nil {
		return x.Execution
	}
	return nil
}

type UpdateExtensionExecutionRequest struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Execution     *gen.ExtensionExecution `protobuf:"bytes,1,opt,name=execution,proto3" json:"execution,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask  `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateExtensionExecutionRequest) Reset() {
	*x = UpdateExtensionExecutionRequest{}
	mi := &file_product_proto_msgTypes[91]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateExtensionExecutionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateExtensionExecutionRequest) ProtoMessage() {}

func (x *UpdateExtensionExecutionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[91]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateExtensionExecutionRequest.ProtoReflect.Descriptor instead.
func (*UpdateExtensionExecutionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{91}
}

func (x *UpdateExtensionExecutionRequest) GetExecution() *gen.ExtensionExecution {
	if x != nil {
		return x.Execution
	}
	return nil
}

func (x *UpdateExtensionExecutionRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

// change request start
type ListUserChangeRequestsRequest struct {
	state              protoimpl.MessageState          `protogen:"open.v1"`
	PageSize           int32                           `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken          string                          `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy            string                          `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	ProjectUuid        string                          `protobuf:"bytes,4,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid string                          `protobuf:"bytes,5,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ReviewStatus       []gen.ChangeRequestReviewStatus `protobuf:"varint,6,rep,packed,name=review_status,json=reviewStatus,proto3,enum=nem.ChangeRequestReviewStatus" json:"review_status,omitempty"`
	ChangeRequestType  gen.ChangeRequestChangeType     `protobuf:"varint,7,opt,name=change_request_type,json=changeRequestType,proto3,enum=nem.ChangeRequestChangeType" json:"change_request_type,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *ListUserChangeRequestsRequest) Reset() {
	*x = ListUserChangeRequestsRequest{}
	mi := &file_product_proto_msgTypes[92]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUserChangeRequestsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUserChangeRequestsRequest) ProtoMessage() {}

func (x *ListUserChangeRequestsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[92]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUserChangeRequestsRequest.ProtoReflect.Descriptor instead.
func (*ListUserChangeRequestsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{92}
}

func (x *ListUserChangeRequestsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListUserChangeRequestsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListUserChangeRequestsRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListUserChangeRequestsRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ListUserChangeRequestsRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *ListUserChangeRequestsRequest) GetReviewStatus() []gen.ChangeRequestReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return nil
}

func (x *ListUserChangeRequestsRequest) GetChangeRequestType() gen.ChangeRequestChangeType {
	if x != nil {
		return x.ChangeRequestType
	}
	return gen.ChangeRequestChangeType(0)
}

type ListUserChangeRequestsResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequests []*gen.ChangeRequest   `protobuf:"bytes,1,rep,name=change_requests,json=changeRequests,proto3" json:"change_requests,omitempty"`
	NextPageToken  string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ListUserChangeRequestsResponse) Reset() {
	*x = ListUserChangeRequestsResponse{}
	mi := &file_product_proto_msgTypes[93]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListUserChangeRequestsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListUserChangeRequestsResponse) ProtoMessage() {}

func (x *ListUserChangeRequestsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[93]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListUserChangeRequestsResponse.ProtoReflect.Descriptor instead.
func (*ListUserChangeRequestsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{93}
}

func (x *ListUserChangeRequestsResponse) GetChangeRequests() []*gen.ChangeRequest {
	if x != nil {
		return x.ChangeRequests
	}
	return nil
}

func (x *ListUserChangeRequestsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetChangeRequestRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequestUuid string                 `protobuf:"bytes,1,opt,name=change_request_uuid,json=changeRequestUuid,proto3" json:"change_request_uuid,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *GetChangeRequestRequest) Reset() {
	*x = GetChangeRequestRequest{}
	mi := &file_product_proto_msgTypes[94]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetChangeRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetChangeRequestRequest) ProtoMessage() {}

func (x *GetChangeRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[94]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetChangeRequestRequest.ProtoReflect.Descriptor instead.
func (*GetChangeRequestRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{94}
}

func (x *GetChangeRequestRequest) GetChangeRequestUuid() string {
	if x != nil {
		return x.ChangeRequestUuid
	}
	return ""
}

type CreateChangeRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequest *gen.ChangeRequest     `protobuf:"bytes,1,opt,name=change_request,json=changeRequest,proto3" json:"change_request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateChangeRequestRequest) Reset() {
	*x = CreateChangeRequestRequest{}
	mi := &file_product_proto_msgTypes[95]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateChangeRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateChangeRequestRequest) ProtoMessage() {}

func (x *CreateChangeRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[95]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateChangeRequestRequest.ProtoReflect.Descriptor instead.
func (*CreateChangeRequestRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{95}
}

func (x *CreateChangeRequestRequest) GetChangeRequest() *gen.ChangeRequest {
	if x != nil {
		return x.ChangeRequest
	}
	return nil
}

type UpdateChangeRequestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequest *gen.ChangeRequest     `protobuf:"bytes,1,opt,name=change_request,json=changeRequest,proto3" json:"change_request,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateChangeRequestRequest) Reset() {
	*x = UpdateChangeRequestRequest{}
	mi := &file_product_proto_msgTypes[96]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateChangeRequestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateChangeRequestRequest) ProtoMessage() {}

func (x *UpdateChangeRequestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[96]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateChangeRequestRequest.ProtoReflect.Descriptor instead.
func (*UpdateChangeRequestRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{96}
}

func (x *UpdateChangeRequestRequest) GetChangeRequest() *gen.ChangeRequest {
	if x != nil {
		return x.ChangeRequest
	}
	return nil
}

func (x *UpdateChangeRequestRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type ListChangeRequestsForReviewRequest struct {
	state             protoimpl.MessageState      `protogen:"open.v1"`
	PageSize          int32                       `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken         string                      `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy           string                      `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	ProjectUuid       string                      `protobuf:"bytes,4,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ChangeRequestType gen.ChangeRequestChangeType `protobuf:"varint,5,opt,name=change_request_type,json=changeRequestType,proto3,enum=nem.ChangeRequestChangeType" json:"change_request_type,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListChangeRequestsForReviewRequest) Reset() {
	*x = ListChangeRequestsForReviewRequest{}
	mi := &file_product_proto_msgTypes[97]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListChangeRequestsForReviewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListChangeRequestsForReviewRequest) ProtoMessage() {}

func (x *ListChangeRequestsForReviewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[97]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListChangeRequestsForReviewRequest.ProtoReflect.Descriptor instead.
func (*ListChangeRequestsForReviewRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{97}
}

func (x *ListChangeRequestsForReviewRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListChangeRequestsForReviewRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListChangeRequestsForReviewRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListChangeRequestsForReviewRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ListChangeRequestsForReviewRequest) GetChangeRequestType() gen.ChangeRequestChangeType {
	if x != nil {
		return x.ChangeRequestType
	}
	return gen.ChangeRequestChangeType(0)
}

type ListChangeRequestsForReviewResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequests []*gen.ChangeRequest   `protobuf:"bytes,1,rep,name=change_requests,json=changeRequests,proto3" json:"change_requests,omitempty"`
	NextPageToken  string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ListChangeRequestsForReviewResponse) Reset() {
	*x = ListChangeRequestsForReviewResponse{}
	mi := &file_product_proto_msgTypes[98]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListChangeRequestsForReviewResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListChangeRequestsForReviewResponse) ProtoMessage() {}

func (x *ListChangeRequestsForReviewResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[98]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListChangeRequestsForReviewResponse.ProtoReflect.Descriptor instead.
func (*ListChangeRequestsForReviewResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{98}
}

func (x *ListChangeRequestsForReviewResponse) GetChangeRequests() []*gen.ChangeRequest {
	if x != nil {
		return x.ChangeRequests
	}
	return nil
}

func (x *ListChangeRequestsForReviewResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

// user project version data start
type GetUserProjectVersionDataRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetUserProjectVersionDataRequest) Reset() {
	*x = GetUserProjectVersionDataRequest{}
	mi := &file_product_proto_msgTypes[99]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserProjectVersionDataRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserProjectVersionDataRequest) ProtoMessage() {}

func (x *GetUserProjectVersionDataRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[99]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserProjectVersionDataRequest.ProtoReflect.Descriptor instead.
func (*GetUserProjectVersionDataRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{99}
}

func (x *GetUserProjectVersionDataRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type GetUserProjectVersionDataResponse struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	Data               string                 `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *GetUserProjectVersionDataResponse) Reset() {
	*x = GetUserProjectVersionDataResponse{}
	mi := &file_product_proto_msgTypes[100]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserProjectVersionDataResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserProjectVersionDataResponse) ProtoMessage() {}

func (x *GetUserProjectVersionDataResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[100]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserProjectVersionDataResponse.ProtoReflect.Descriptor instead.
func (*GetUserProjectVersionDataResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{100}
}

func (x *GetUserProjectVersionDataResponse) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *GetUserProjectVersionDataResponse) GetData() string {
	if x != nil {
		return x.Data
	}
	return ""
}

type SaveUserProjectVersionDataRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	Data               string                 `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *SaveUserProjectVersionDataRequest) Reset() {
	*x = SaveUserProjectVersionDataRequest{}
	mi := &file_product_proto_msgTypes[101]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SaveUserProjectVersionDataRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SaveUserProjectVersionDataRequest) ProtoMessage() {}

func (x *SaveUserProjectVersionDataRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[101]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SaveUserProjectVersionDataRequest.ProtoReflect.Descriptor instead.
func (*SaveUserProjectVersionDataRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{101}
}

func (x *SaveUserProjectVersionDataRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *SaveUserProjectVersionDataRequest) GetData() string {
	if x != nil {
		return x.Data
	}
	return ""
}

type SaveUserProjectVersionDataResponse struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ProjectVersionUuid string                 `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	Data               string                 `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *SaveUserProjectVersionDataResponse) Reset() {
	*x = SaveUserProjectVersionDataResponse{}
	mi := &file_product_proto_msgTypes[102]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SaveUserProjectVersionDataResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SaveUserProjectVersionDataResponse) ProtoMessage() {}

func (x *SaveUserProjectVersionDataResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[102]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SaveUserProjectVersionDataResponse.ProtoReflect.Descriptor instead.
func (*SaveUserProjectVersionDataResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{102}
}

func (x *SaveUserProjectVersionDataResponse) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *SaveUserProjectVersionDataResponse) GetData() string {
	if x != nil {
		return x.Data
	}
	return ""
}

// object store start
type GetObjectStoreWithSecretRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	ObjectStoreUuid string                 `protobuf:"bytes,1,opt,name=object_store_uuid,json=objectStoreUuid,proto3" json:"object_store_uuid,omitempty"`
	TeamUuid        string                 `protobuf:"bytes,2,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GetObjectStoreWithSecretRequest) Reset() {
	*x = GetObjectStoreWithSecretRequest{}
	mi := &file_product_proto_msgTypes[103]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetObjectStoreWithSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetObjectStoreWithSecretRequest) ProtoMessage() {}

func (x *GetObjectStoreWithSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[103]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetObjectStoreWithSecretRequest.ProtoReflect.Descriptor instead.
func (*GetObjectStoreWithSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{103}
}

func (x *GetObjectStoreWithSecretRequest) GetObjectStoreUuid() string {
	if x != nil {
		return x.ObjectStoreUuid
	}
	return ""
}

func (x *GetObjectStoreWithSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type CreateObjectStoreSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	ObjectStore   *gen.ObjectStore       `protobuf:"bytes,2,opt,name=object_store,json=objectStore,proto3" json:"object_store,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateObjectStoreSecretRequest) Reset() {
	*x = CreateObjectStoreSecretRequest{}
	mi := &file_product_proto_msgTypes[104]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateObjectStoreSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateObjectStoreSecretRequest) ProtoMessage() {}

func (x *CreateObjectStoreSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[104]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateObjectStoreSecretRequest.ProtoReflect.Descriptor instead.
func (*CreateObjectStoreSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{104}
}

func (x *CreateObjectStoreSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *CreateObjectStoreSecretRequest) GetObjectStore() *gen.ObjectStore {
	if x != nil {
		return x.ObjectStore
	}
	return nil
}

type UpdateObjectStoreSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	ObjectStore   *gen.ObjectStore       `protobuf:"bytes,2,opt,name=object_store,json=objectStore,proto3" json:"object_store,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateObjectStoreSecretRequest) Reset() {
	*x = UpdateObjectStoreSecretRequest{}
	mi := &file_product_proto_msgTypes[105]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateObjectStoreSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateObjectStoreSecretRequest) ProtoMessage() {}

func (x *UpdateObjectStoreSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[105]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateObjectStoreSecretRequest.ProtoReflect.Descriptor instead.
func (*UpdateObjectStoreSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{105}
}

func (x *UpdateObjectStoreSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *UpdateObjectStoreSecretRequest) GetObjectStore() *gen.ObjectStore {
	if x != nil {
		return x.ObjectStore
	}
	return nil
}

type DeleteObjectStoreSecretRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	ObjectStore   *gen.ObjectStore       `protobuf:"bytes,2,opt,name=object_store,json=objectStore,proto3" json:"object_store,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteObjectStoreSecretRequest) Reset() {
	*x = DeleteObjectStoreSecretRequest{}
	mi := &file_product_proto_msgTypes[106]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteObjectStoreSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteObjectStoreSecretRequest) ProtoMessage() {}

func (x *DeleteObjectStoreSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[106]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteObjectStoreSecretRequest.ProtoReflect.Descriptor instead.
func (*DeleteObjectStoreSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{106}
}

func (x *DeleteObjectStoreSecretRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *DeleteObjectStoreSecretRequest) GetObjectStore() *gen.ObjectStore {
	if x != nil {
		return x.ObjectStore
	}
	return nil
}

// data manager start
type GenerateSQLForCRRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequestUuid string                 `protobuf:"bytes,1,opt,name=change_request_uuid,json=changeRequestUuid,proto3" json:"change_request_uuid,omitempty"`
	ConnectionUuid    string                 `protobuf:"bytes,2,opt,name=connection_uuid,json=connectionUuid,proto3" json:"connection_uuid,omitempty"`
	// db_type_override lets the caller specify the dialect directly without
	// resolving via team.connections. Used by ApplyDataCR for LOCAL connections
	// (which don't have a central Connection entity). When set to anything
	// non-INVALID, takes precedence over connection_uuid.
	DbTypeOverride gen.ConnectionDbType `protobuf:"varint,3,opt,name=db_type_override,json=dbTypeOverride,proto3,enum=nem.ConnectionDbType" json:"db_type_override,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GenerateSQLForCRRequest) Reset() {
	*x = GenerateSQLForCRRequest{}
	mi := &file_product_proto_msgTypes[107]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GenerateSQLForCRRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GenerateSQLForCRRequest) ProtoMessage() {}

func (x *GenerateSQLForCRRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[107]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GenerateSQLForCRRequest.ProtoReflect.Descriptor instead.
func (*GenerateSQLForCRRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{107}
}

func (x *GenerateSQLForCRRequest) GetChangeRequestUuid() string {
	if x != nil {
		return x.ChangeRequestUuid
	}
	return ""
}

func (x *GenerateSQLForCRRequest) GetConnectionUuid() string {
	if x != nil {
		return x.ConnectionUuid
	}
	return ""
}

func (x *GenerateSQLForCRRequest) GetDbTypeOverride() gen.ConnectionDbType {
	if x != nil {
		return x.DbTypeOverride
	}
	return gen.ConnectionDbType(0)
}

type GenerateSQLForCRResponse struct {
	state         protoimpl.MessageState       `protogen:"open.v1"`
	Statements    []*GenerateSQLForCRStatement `protobuf:"bytes,1,rep,name=statements,proto3" json:"statements,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GenerateSQLForCRResponse) Reset() {
	*x = GenerateSQLForCRResponse{}
	mi := &file_product_proto_msgTypes[108]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GenerateSQLForCRResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GenerateSQLForCRResponse) ProtoMessage() {}

func (x *GenerateSQLForCRResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[108]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GenerateSQLForCRResponse.ProtoReflect.Descriptor instead.
func (*GenerateSQLForCRResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{108}
}

func (x *GenerateSQLForCRResponse) GetStatements() []*GenerateSQLForCRStatement {
	if x != nil {
		return x.Statements
	}
	return nil
}

type GenerateSQLForCRStatement struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Sql             string                 `protobuf:"bytes,1,opt,name=sql,proto3" json:"sql,omitempty"`                                                // display
	ParametrizedSql string                 `protobuf:"bytes,2,opt,name=parametrized_sql,json=parametrizedSql,proto3" json:"parametrized_sql,omitempty"` // for execution
	Parameters      []string               `protobuf:"bytes,3,rep,name=parameters,proto3" json:"parameters,omitempty"`                                  // for execution
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GenerateSQLForCRStatement) Reset() {
	*x = GenerateSQLForCRStatement{}
	mi := &file_product_proto_msgTypes[109]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GenerateSQLForCRStatement) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GenerateSQLForCRStatement) ProtoMessage() {}

func (x *GenerateSQLForCRStatement) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[109]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GenerateSQLForCRStatement.ProtoReflect.Descriptor instead.
func (*GenerateSQLForCRStatement) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{109}
}

func (x *GenerateSQLForCRStatement) GetSql() string {
	if x != nil {
		return x.Sql
	}
	return ""
}

func (x *GenerateSQLForCRStatement) GetParametrizedSql() string {
	if x != nil {
		return x.ParametrizedSql
	}
	return ""
}

func (x *GenerateSQLForCRStatement) GetParameters() []string {
	if x != nil {
		return x.Parameters
	}
	return nil
}

type ReviewDataChangeRequest struct {
	state              protoimpl.MessageState        `protogen:"open.v1"`
	ProjectVersionUuid string                        `protobuf:"bytes,1,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ChangeRequestUuid  string                        `protobuf:"bytes,2,opt,name=change_request_uuid,json=changeRequestUuid,proto3" json:"change_request_uuid,omitempty"`
	ReviewStatus       gen.ChangeRequestReviewStatus `protobuf:"varint,3,opt,name=review_status,json=reviewStatus,proto3,enum=nem.ChangeRequestReviewStatus" json:"review_status,omitempty"` // APPROVED, REJECTED
	ReviewComment      string                        `protobuf:"bytes,4,opt,name=review_comment,json=reviewComment,proto3" json:"review_comment,omitempty"`                                  // optional
	PublishChanges     bool                          `protobuf:"varint,5,opt,name=publish_changes,json=publishChanges,proto3" json:"publish_changes,omitempty"`                              // the changes will be published if approved
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *ReviewDataChangeRequest) Reset() {
	*x = ReviewDataChangeRequest{}
	mi := &file_product_proto_msgTypes[110]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReviewDataChangeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReviewDataChangeRequest) ProtoMessage() {}

func (x *ReviewDataChangeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[110]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReviewDataChangeRequest.ProtoReflect.Descriptor instead.
func (*ReviewDataChangeRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{110}
}

func (x *ReviewDataChangeRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *ReviewDataChangeRequest) GetChangeRequestUuid() string {
	if x != nil {
		return x.ChangeRequestUuid
	}
	return ""
}

func (x *ReviewDataChangeRequest) GetReviewStatus() gen.ChangeRequestReviewStatus {
	if x != nil {
		return x.ReviewStatus
	}
	return gen.ChangeRequestReviewStatus(0)
}

func (x *ReviewDataChangeRequest) GetReviewComment() string {
	if x != nil {
		return x.ReviewComment
	}
	return ""
}

func (x *ReviewDataChangeRequest) GetPublishChanges() bool {
	if x != nil {
		return x.PublishChanges
	}
	return false
}

type ReviewDataChangeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReviewDataChangeResponse) Reset() {
	*x = ReviewDataChangeResponse{}
	mi := &file_product_proto_msgTypes[111]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReviewDataChangeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReviewDataChangeResponse) ProtoMessage() {}

func (x *ReviewDataChangeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[111]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReviewDataChangeResponse.ProtoReflect.Descriptor instead.
func (*ReviewDataChangeResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{111}
}

// membership start
type GetMembershipRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMembershipRequest) Reset() {
	*x = GetMembershipRequest{}
	mi := &file_product_proto_msgTypes[112]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMembershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMembershipRequest) ProtoMessage() {}

func (x *GetMembershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[112]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMembershipRequest.ProtoReflect.Descriptor instead.
func (*GetMembershipRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{112}
}

type CreateMembershipRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Membership    *gen.Membership        `protobuf:"bytes,1,opt,name=membership,proto3" json:"membership,omitempty"` // uuid should be empty
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateMembershipRequest) Reset() {
	*x = CreateMembershipRequest{}
	mi := &file_product_proto_msgTypes[113]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateMembershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateMembershipRequest) ProtoMessage() {}

func (x *CreateMembershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[113]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateMembershipRequest.ProtoReflect.Descriptor instead.
func (*CreateMembershipRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{113}
}

func (x *CreateMembershipRequest) GetMembership() *gen.Membership {
	if x != nil {
		return x.Membership
	}
	return nil
}

type UpdateMembershipRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Membership    *gen.Membership        `protobuf:"bytes,1,opt,name=membership,proto3" json:"membership,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateMembershipRequest) Reset() {
	*x = UpdateMembershipRequest{}
	mi := &file_product_proto_msgTypes[114]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateMembershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateMembershipRequest) ProtoMessage() {}

func (x *UpdateMembershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[114]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateMembershipRequest.ProtoReflect.Descriptor instead.
func (*UpdateMembershipRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{114}
}

func (x *UpdateMembershipRequest) GetMembership() *gen.Membership {
	if x != nil {
		return x.Membership
	}
	return nil
}

func (x *UpdateMembershipRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type MembershipForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MembershipForProjectRequest) Reset() {
	*x = MembershipForProjectRequest{}
	mi := &file_product_proto_msgTypes[115]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MembershipForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MembershipForProjectRequest) ProtoMessage() {}

func (x *MembershipForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[115]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MembershipForProjectRequest.ProtoReflect.Descriptor instead.
func (*MembershipForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{115}
}

func (x *MembershipForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type MembershipForTeamRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TeamUuid      string                 `protobuf:"bytes,1,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MembershipForTeamRequest) Reset() {
	*x = MembershipForTeamRequest{}
	mi := &file_product_proto_msgTypes[116]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MembershipForTeamRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MembershipForTeamRequest) ProtoMessage() {}

func (x *MembershipForTeamRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[116]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MembershipForTeamRequest.ProtoReflect.Descriptor instead.
func (*MembershipForTeamRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{116}
}

func (x *MembershipForTeamRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

type CreateCheckoutSessionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionType   CheckoutSessionType    `protobuf:"varint,1,opt,name=session_type,json=sessionType,proto3,enum=CheckoutSessionType" json:"session_type,omitempty"`
	PlanType      gen.MembershipType     `protobuf:"varint,3,opt,name=plan_type,json=planType,proto3,enum=nem.MembershipType" json:"plan_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateCheckoutSessionRequest) Reset() {
	*x = CreateCheckoutSessionRequest{}
	mi := &file_product_proto_msgTypes[117]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateCheckoutSessionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateCheckoutSessionRequest) ProtoMessage() {}

func (x *CreateCheckoutSessionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[117]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateCheckoutSessionRequest.ProtoReflect.Descriptor instead.
func (*CreateCheckoutSessionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{117}
}

func (x *CreateCheckoutSessionRequest) GetSessionType() CheckoutSessionType {
	if x != nil {
		return x.SessionType
	}
	return CheckoutSessionType_CHECKOUT_SESSION_TYPE_INVALID
}

func (x *CreateCheckoutSessionRequest) GetPlanType() gen.MembershipType {
	if x != nil {
		return x.PlanType
	}
	return gen.MembershipType(0)
}

type CreateCheckoutSessionResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	SessionSecret  string                 `protobuf:"bytes,1,opt,name=session_secret,json=sessionSecret,proto3" json:"session_secret,omitempty"`    // stripe session secret
	MembershipUuid string                 `protobuf:"bytes,2,opt,name=membership_uuid,json=membershipUuid,proto3" json:"membership_uuid,omitempty"` // the membership uuid
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *CreateCheckoutSessionResponse) Reset() {
	*x = CreateCheckoutSessionResponse{}
	mi := &file_product_proto_msgTypes[118]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateCheckoutSessionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateCheckoutSessionResponse) ProtoMessage() {}

func (x *CreateCheckoutSessionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[118]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateCheckoutSessionResponse.ProtoReflect.Descriptor instead.
func (*CreateCheckoutSessionResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{118}
}

func (x *CreateCheckoutSessionResponse) GetSessionSecret() string {
	if x != nil {
		return x.SessionSecret
	}
	return ""
}

func (x *CreateCheckoutSessionResponse) GetMembershipUuid() string {
	if x != nil {
		return x.MembershipUuid
	}
	return ""
}

type ChangeMembershipPlanRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	NewPlanType   gen.MembershipType     `protobuf:"varint,1,opt,name=new_plan_type,json=newPlanType,proto3,enum=nem.MembershipType" json:"new_plan_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeMembershipPlanRequest) Reset() {
	*x = ChangeMembershipPlanRequest{}
	mi := &file_product_proto_msgTypes[119]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeMembershipPlanRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeMembershipPlanRequest) ProtoMessage() {}

func (x *ChangeMembershipPlanRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[119]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeMembershipPlanRequest.ProtoReflect.Descriptor instead.
func (*ChangeMembershipPlanRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{119}
}

func (x *ChangeMembershipPlanRequest) GetNewPlanType() gen.MembershipType {
	if x != nil {
		return x.NewPlanType
	}
	return gen.MembershipType(0)
}

type ChangeMembershipPlanResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeMembershipPlanResponse) Reset() {
	*x = ChangeMembershipPlanResponse{}
	mi := &file_product_proto_msgTypes[120]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeMembershipPlanResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeMembershipPlanResponse) ProtoMessage() {}

func (x *ChangeMembershipPlanResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[120]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeMembershipPlanResponse.ProtoReflect.Descriptor instead.
func (*ChangeMembershipPlanResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{120}
}

type ListMembershipUsersRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken     string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy       string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMembershipUsersRequest) Reset() {
	*x = ListMembershipUsersRequest{}
	mi := &file_product_proto_msgTypes[121]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipUsersRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipUsersRequest) ProtoMessage() {}

func (x *ListMembershipUsersRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[121]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipUsersRequest.ProtoReflect.Descriptor instead.
func (*ListMembershipUsersRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{121}
}

func (x *ListMembershipUsersRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListMembershipUsersRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListMembershipUsersRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

type ListMembershipUsersResponse struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	Users              []*gen.User            `protobuf:"bytes,1,rep,name=users,proto3" json:"users,omitempty"`
	UnregisteredEmails []string               `protobuf:"bytes,2,rep,name=unregistered_emails,json=unregisteredEmails,proto3" json:"unregistered_emails,omitempty"` // emails of users that are not registered yet
	NextPageToken      string                 `protobuf:"bytes,3,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *ListMembershipUsersResponse) Reset() {
	*x = ListMembershipUsersResponse{}
	mi := &file_product_proto_msgTypes[122]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipUsersResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipUsersResponse) ProtoMessage() {}

func (x *ListMembershipUsersResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[122]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipUsersResponse.ProtoReflect.Descriptor instead.
func (*ListMembershipUsersResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{122}
}

func (x *ListMembershipUsersResponse) GetUsers() []*gen.User {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *ListMembershipUsersResponse) GetUnregisteredEmails() []string {
	if x != nil {
		return x.UnregisteredEmails
	}
	return nil
}

func (x *ListMembershipUsersResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type CancelMembershipRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Reasons       []string               `protobuf:"bytes,1,rep,name=reasons,proto3" json:"reasons,omitempty"`   // optional
	Comments      string                 `protobuf:"bytes,2,opt,name=comments,proto3" json:"comments,omitempty"` // optional
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CancelMembershipRequest) Reset() {
	*x = CancelMembershipRequest{}
	mi := &file_product_proto_msgTypes[123]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelMembershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelMembershipRequest) ProtoMessage() {}

func (x *CancelMembershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[123]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CancelMembershipRequest.ProtoReflect.Descriptor instead.
func (*CancelMembershipRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{123}
}

func (x *CancelMembershipRequest) GetReasons() []string {
	if x != nil {
		return x.Reasons
	}
	return nil
}

func (x *CancelMembershipRequest) GetComments() string {
	if x != nil {
		return x.Comments
	}
	return ""
}

type CancelMembershipResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CancelMembershipResponse) Reset() {
	*x = CancelMembershipResponse{}
	mi := &file_product_proto_msgTypes[124]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelMembershipResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelMembershipResponse) ProtoMessage() {}

func (x *CancelMembershipResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[124]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CancelMembershipResponse.ProtoReflect.Descriptor instead.
func (*CancelMembershipResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{124}
}

type ReactivateMembershipRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReactivateMembershipRequest) Reset() {
	*x = ReactivateMembershipRequest{}
	mi := &file_product_proto_msgTypes[125]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReactivateMembershipRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReactivateMembershipRequest) ProtoMessage() {}

func (x *ReactivateMembershipRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[125]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReactivateMembershipRequest.ProtoReflect.Descriptor instead.
func (*ReactivateMembershipRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{125}
}

type ReactivateMembershipResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReactivateMembershipResponse) Reset() {
	*x = ReactivateMembershipResponse{}
	mi := &file_product_proto_msgTypes[126]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReactivateMembershipResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReactivateMembershipResponse) ProtoMessage() {}

func (x *ReactivateMembershipResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[126]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReactivateMembershipResponse.ProtoReflect.Descriptor instead.
func (*ReactivateMembershipResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{126}
}

type UserHasActiveStripeSubscriptionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserHasActiveStripeSubscriptionRequest) Reset() {
	*x = UserHasActiveStripeSubscriptionRequest{}
	mi := &file_product_proto_msgTypes[127]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserHasActiveStripeSubscriptionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserHasActiveStripeSubscriptionRequest) ProtoMessage() {}

func (x *UserHasActiveStripeSubscriptionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[127]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserHasActiveStripeSubscriptionRequest.ProtoReflect.Descriptor instead.
func (*UserHasActiveStripeSubscriptionRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{127}
}

type UserHasActiveStripeSubscriptionResponse struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	ActiveSubscription bool                   `protobuf:"varint,1,opt,name=active_subscription,json=activeSubscription,proto3" json:"active_subscription,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *UserHasActiveStripeSubscriptionResponse) Reset() {
	*x = UserHasActiveStripeSubscriptionResponse{}
	mi := &file_product_proto_msgTypes[128]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserHasActiveStripeSubscriptionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserHasActiveStripeSubscriptionResponse) ProtoMessage() {}

func (x *UserHasActiveStripeSubscriptionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[128]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserHasActiveStripeSubscriptionResponse.ProtoReflect.Descriptor instead.
func (*UserHasActiveStripeSubscriptionResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{128}
}

func (x *UserHasActiveStripeSubscriptionResponse) GetActiveSubscription() bool {
	if x != nil {
		return x.ActiveSubscription
	}
	return false
}

type ListMembershipUserProjectsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserUuid      string                 `protobuf:"bytes,1,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"` // the user to list projects for
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMembershipUserProjectsRequest) Reset() {
	*x = ListMembershipUserProjectsRequest{}
	mi := &file_product_proto_msgTypes[129]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipUserProjectsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipUserProjectsRequest) ProtoMessage() {}

func (x *ListMembershipUserProjectsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[129]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipUserProjectsRequest.ProtoReflect.Descriptor instead.
func (*ListMembershipUserProjectsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{129}
}

func (x *ListMembershipUserProjectsRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

type ListMembershipUserProjectsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Projects      []*gen.Project         `protobuf:"bytes,1,rep,name=projects,proto3" json:"projects,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMembershipUserProjectsResponse) Reset() {
	*x = ListMembershipUserProjectsResponse{}
	mi := &file_product_proto_msgTypes[130]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipUserProjectsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipUserProjectsResponse) ProtoMessage() {}

func (x *ListMembershipUserProjectsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[130]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipUserProjectsResponse.ProtoReflect.Descriptor instead.
func (*ListMembershipUserProjectsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{130}
}

func (x *ListMembershipUserProjectsResponse) GetProjects() []*gen.Project {
	if x != nil {
		return x.Projects
	}
	return nil
}

type RemoveMembershipUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserUuid      string                 `protobuf:"bytes,1,opt,name=user_uuid,json=userUuid,proto3" json:"user_uuid,omitempty"` // the user to remove from membership
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveMembershipUserRequest) Reset() {
	*x = RemoveMembershipUserRequest{}
	mi := &file_product_proto_msgTypes[131]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveMembershipUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveMembershipUserRequest) ProtoMessage() {}

func (x *RemoveMembershipUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[131]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveMembershipUserRequest.ProtoReflect.Descriptor instead.
func (*RemoveMembershipUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{131}
}

func (x *RemoveMembershipUserRequest) GetUserUuid() string {
	if x != nil {
		return x.UserUuid
	}
	return ""
}

type RemoveMembershipUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveMembershipUserResponse) Reset() {
	*x = RemoveMembershipUserResponse{}
	mi := &file_product_proto_msgTypes[132]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveMembershipUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveMembershipUserResponse) ProtoMessage() {}

func (x *RemoveMembershipUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[132]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveMembershipUserResponse.ProtoReflect.Descriptor instead.
func (*RemoveMembershipUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{132}
}

type ListMembershipsForUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Locale        string                 `protobuf:"bytes,1,opt,name=locale,proto3" json:"locale,omitempty"` // e.g., "en"
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMembershipsForUserRequest) Reset() {
	*x = ListMembershipsForUserRequest{}
	mi := &file_product_proto_msgTypes[133]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipsForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipsForUserRequest) ProtoMessage() {}

func (x *ListMembershipsForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[133]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipsForUserRequest.ProtoReflect.Descriptor instead.
func (*ListMembershipsForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{133}
}

func (x *ListMembershipsForUserRequest) GetLocale() string {
	if x != nil {
		return x.Locale
	}
	return ""
}

type MembershipWithName struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Membership     *gen.Membership        `protobuf:"bytes,1,opt,name=membership,proto3" json:"membership,omitempty"`
	MembershipName string                 `protobuf:"bytes,2,opt,name=membership_name,json=membershipName,proto3" json:"membership_name,omitempty"` // e.g., "Adam's Membership"
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MembershipWithName) Reset() {
	*x = MembershipWithName{}
	mi := &file_product_proto_msgTypes[134]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MembershipWithName) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MembershipWithName) ProtoMessage() {}

func (x *MembershipWithName) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[134]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MembershipWithName.ProtoReflect.Descriptor instead.
func (*MembershipWithName) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{134}
}

func (x *MembershipWithName) GetMembership() *gen.Membership {
	if x != nil {
		return x.Membership
	}
	return nil
}

func (x *MembershipWithName) GetMembershipName() string {
	if x != nil {
		return x.MembershipName
	}
	return ""
}

type ListMembershipsForUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Memberships   []*MembershipWithName  `protobuf:"bytes,1,rep,name=memberships,proto3" json:"memberships,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMembershipsForUserResponse) Reset() {
	*x = ListMembershipsForUserResponse{}
	mi := &file_product_proto_msgTypes[135]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMembershipsForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMembershipsForUserResponse) ProtoMessage() {}

func (x *ListMembershipsForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[135]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMembershipsForUserResponse.ProtoReflect.Descriptor instead.
func (*ListMembershipsForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{135}
}

func (x *ListMembershipsForUserResponse) GetMemberships() []*MembershipWithName {
	if x != nil {
		return x.Memberships
	}
	return nil
}

// support start
type PrioritySupportEmailRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Category        string                 `protobuf:"bytes,1,opt,name=category,proto3" json:"category,omitempty"`
	ProjectUuid     string                 `protobuf:"bytes,2,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"` // optional
	TeamUuid        string                 `protobuf:"bytes,3,opt,name=team_uuid,json=teamUuid,proto3" json:"team_uuid,omitempty"`          // optional
	Message         string                 `protobuf:"bytes,4,opt,name=message,proto3" json:"message,omitempty"`
	OperatingSystem string                 `protobuf:"bytes,5,opt,name=operatingSystem,proto3" json:"operatingSystem,omitempty"`
	AppVersion      string                 `protobuf:"bytes,6,opt,name=appVersion,proto3" json:"appVersion,omitempty"`
	BrowserVersion  string                 `protobuf:"bytes,7,opt,name=browserVersion,proto3" json:"browserVersion,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *PrioritySupportEmailRequest) Reset() {
	*x = PrioritySupportEmailRequest{}
	mi := &file_product_proto_msgTypes[136]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PrioritySupportEmailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PrioritySupportEmailRequest) ProtoMessage() {}

func (x *PrioritySupportEmailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[136]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PrioritySupportEmailRequest.ProtoReflect.Descriptor instead.
func (*PrioritySupportEmailRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{136}
}

func (x *PrioritySupportEmailRequest) GetCategory() string {
	if x != nil {
		return x.Category
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetTeamUuid() string {
	if x != nil {
		return x.TeamUuid
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetOperatingSystem() string {
	if x != nil {
		return x.OperatingSystem
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetAppVersion() string {
	if x != nil {
		return x.AppVersion
	}
	return ""
}

func (x *PrioritySupportEmailRequest) GetBrowserVersion() string {
	if x != nil {
		return x.BrowserVersion
	}
	return ""
}

type PrioritySupportEmailResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PrioritySupportEmailResponse) Reset() {
	*x = PrioritySupportEmailResponse{}
	mi := &file_product_proto_msgTypes[137]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PrioritySupportEmailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PrioritySupportEmailResponse) ProtoMessage() {}

func (x *PrioritySupportEmailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[137]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PrioritySupportEmailResponse.ProtoReflect.Descriptor instead.
func (*PrioritySupportEmailResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{137}
}

// webhook start
type HandleWebhookRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Source        string                 `protobuf:"bytes,1,opt,name=source,proto3" json:"source,omitempty"`                        // e.g., "stripe"
	Payload       string                 `protobuf:"bytes,2,opt,name=payload,proto3" json:"payload,omitempty"`                      // raw payload
	SigHeader     string                 `protobuf:"bytes,3,opt,name=sig_header,json=sigHeader,proto3" json:"sig_header,omitempty"` // signature header for verification, e.g., Stripe-Signature
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HandleWebhookRequest) Reset() {
	*x = HandleWebhookRequest{}
	mi := &file_product_proto_msgTypes[138]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleWebhookRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleWebhookRequest) ProtoMessage() {}

func (x *HandleWebhookRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[138]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleWebhookRequest.ProtoReflect.Descriptor instead.
func (*HandleWebhookRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{138}
}

func (x *HandleWebhookRequest) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
}

func (x *HandleWebhookRequest) GetPayload() string {
	if x != nil {
		return x.Payload
	}
	return ""
}

func (x *HandleWebhookRequest) GetSigHeader() string {
	if x != nil {
		return x.SigHeader
	}
	return ""
}

type HandleWebhookResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HandleWebhookResponse) Reset() {
	*x = HandleWebhookResponse{}
	mi := &file_product_proto_msgTypes[139]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleWebhookResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleWebhookResponse) ProtoMessage() {}

func (x *HandleWebhookResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[139]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleWebhookResponse.ProtoReflect.Descriptor instead.
func (*HandleWebhookResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{139}
}

// pro features start
type IsProActiveForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IsProActiveForProjectRequest) Reset() {
	*x = IsProActiveForProjectRequest{}
	mi := &file_product_proto_msgTypes[140]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsProActiveForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsProActiveForProjectRequest) ProtoMessage() {}

func (x *IsProActiveForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[140]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsProActiveForProjectRequest.ProtoReflect.Descriptor instead.
func (*IsProActiveForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{140}
}

func (x *IsProActiveForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type IsProActiveForProjectResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	IsProActive    bool                   `protobuf:"varint,1,opt,name=is_pro_active,json=isProActive,proto3" json:"is_pro_active,omitempty"`
	MembershipUuid string                 `protobuf:"bytes,2,opt,name=membership_uuid,json=membershipUuid,proto3" json:"membership_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *IsProActiveForProjectResponse) Reset() {
	*x = IsProActiveForProjectResponse{}
	mi := &file_product_proto_msgTypes[141]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IsProActiveForProjectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IsProActiveForProjectResponse) ProtoMessage() {}

func (x *IsProActiveForProjectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[141]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IsProActiveForProjectResponse.ProtoReflect.Descriptor instead.
func (*IsProActiveForProjectResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{141}
}

func (x *IsProActiveForProjectResponse) GetIsProActive() bool {
	if x != nil {
		return x.IsProActive
	}
	return false
}

func (x *IsProActiveForProjectResponse) GetMembershipUuid() string {
	if x != nil {
		return x.MembershipUuid
	}
	return ""
}

type GetMembershipTierForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMembershipTierForProjectRequest) Reset() {
	*x = GetMembershipTierForProjectRequest{}
	mi := &file_product_proto_msgTypes[142]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMembershipTierForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMembershipTierForProjectRequest) ProtoMessage() {}

func (x *GetMembershipTierForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[142]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMembershipTierForProjectRequest.ProtoReflect.Descriptor instead.
func (*GetMembershipTierForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{142}
}

func (x *GetMembershipTierForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type GetMembershipTierForProjectResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	MembershipType gen.MembershipType     `protobuf:"varint,1,opt,name=membership_type,json=membershipType,proto3,enum=nem.MembershipType" json:"membership_type,omitempty"`
	MembershipUuid string                 `protobuf:"bytes,2,opt,name=membership_uuid,json=membershipUuid,proto3" json:"membership_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetMembershipTierForProjectResponse) Reset() {
	*x = GetMembershipTierForProjectResponse{}
	mi := &file_product_proto_msgTypes[143]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMembershipTierForProjectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMembershipTierForProjectResponse) ProtoMessage() {}

func (x *GetMembershipTierForProjectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[143]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMembershipTierForProjectResponse.ProtoReflect.Descriptor instead.
func (*GetMembershipTierForProjectResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{143}
}

func (x *GetMembershipTierForProjectResponse) GetMembershipType() gen.MembershipType {
	if x != nil {
		return x.MembershipType
	}
	return gen.MembershipType(0)
}

func (x *GetMembershipTierForProjectResponse) GetMembershipUuid() string {
	if x != nil {
		return x.MembershipUuid
	}
	return ""
}

type EvaluateUserPromptRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	Context            ProContext             `protobuf:"varint,1,opt,name=context,proto3,enum=ProContext" json:"context,omitempty"`
	UserPrompt         string                 `protobuf:"bytes,2,opt,name=user_prompt,json=userPrompt,proto3" json:"user_prompt,omitempty"`
	ProjectVersionUuid string                 `protobuf:"bytes,3,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *EvaluateUserPromptRequest) Reset() {
	*x = EvaluateUserPromptRequest{}
	mi := &file_product_proto_msgTypes[144]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvaluateUserPromptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvaluateUserPromptRequest) ProtoMessage() {}

func (x *EvaluateUserPromptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[144]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvaluateUserPromptRequest.ProtoReflect.Descriptor instead.
func (*EvaluateUserPromptRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{144}
}

func (x *EvaluateUserPromptRequest) GetContext() ProContext {
	if x != nil {
		return x.Context
	}
	return ProContext_PRO_CONTEXT_INVALID
}

func (x *EvaluateUserPromptRequest) GetUserPrompt() string {
	if x != nil {
		return x.UserPrompt
	}
	return ""
}

func (x *EvaluateUserPromptRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

type EvaluateUserPromptResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsActionable  bool                   `protobuf:"varint,1,opt,name=is_actionable,json=isActionable,proto3" json:"is_actionable,omitempty"`
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	Steps         []string               `protobuf:"bytes,3,rep,name=steps,proto3" json:"steps,omitempty"` // if actionable, list of steps to be performed
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EvaluateUserPromptResponse) Reset() {
	*x = EvaluateUserPromptResponse{}
	mi := &file_product_proto_msgTypes[145]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvaluateUserPromptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvaluateUserPromptResponse) ProtoMessage() {}

func (x *EvaluateUserPromptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[145]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvaluateUserPromptResponse.ProtoReflect.Descriptor instead.
func (*EvaluateUserPromptResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{145}
}

func (x *EvaluateUserPromptResponse) GetIsActionable() bool {
	if x != nil {
		return x.IsActionable
	}
	return false
}

func (x *EvaluateUserPromptResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *EvaluateUserPromptResponse) GetSteps() []string {
	if x != nil {
		return x.Steps
	}
	return nil
}

type HandleUserPromptRequest struct {
	state                 protoimpl.MessageState `protogen:"open.v1"`
	Context               ProContext             `protobuf:"varint,1,opt,name=context,proto3,enum=ProContext" json:"context,omitempty"`
	UserPrompt            string                 `protobuf:"bytes,2,opt,name=user_prompt,json=userPrompt,proto3" json:"user_prompt,omitempty"`
	ProjectVersionUuid    string                 `protobuf:"bytes,3,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ProjectVersionVersion string                 `protobuf:"bytes,4,opt,name=project_version_version,json=projectVersionVersion,proto3" json:"project_version_version,omitempty"`
	Steps                 []string               `protobuf:"bytes,5,rep,name=steps,proto3" json:"steps,omitempty"`                                         // steps to be performed
	AdditionalData        string                 `protobuf:"bytes,6,opt,name=additional_data,json=additionalData,proto3" json:"additional_data,omitempty"` // optional additional data to assist with the prompt handling
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}

func (x *HandleUserPromptRequest) Reset() {
	*x = HandleUserPromptRequest{}
	mi := &file_product_proto_msgTypes[146]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleUserPromptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleUserPromptRequest) ProtoMessage() {}

func (x *HandleUserPromptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[146]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleUserPromptRequest.ProtoReflect.Descriptor instead.
func (*HandleUserPromptRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{146}
}

func (x *HandleUserPromptRequest) GetContext() ProContext {
	if x != nil {
		return x.Context
	}
	return ProContext_PRO_CONTEXT_INVALID
}

func (x *HandleUserPromptRequest) GetUserPrompt() string {
	if x != nil {
		return x.UserPrompt
	}
	return ""
}

func (x *HandleUserPromptRequest) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *HandleUserPromptRequest) GetProjectVersionVersion() string {
	if x != nil {
		return x.ProjectVersionVersion
	}
	return ""
}

func (x *HandleUserPromptRequest) GetSteps() []string {
	if x != nil {
		return x.Steps
	}
	return nil
}

func (x *HandleUserPromptRequest) GetAdditionalData() string {
	if x != nil {
		return x.AdditionalData
	}
	return ""
}

type HandleUserPromptResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	JobUuid       string                 `protobuf:"bytes,1,opt,name=job_uuid,json=jobUuid,proto3" json:"job_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HandleUserPromptResponse) Reset() {
	*x = HandleUserPromptResponse{}
	mi := &file_product_proto_msgTypes[147]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleUserPromptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleUserPromptResponse) ProtoMessage() {}

func (x *HandleUserPromptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[147]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleUserPromptResponse.ProtoReflect.Descriptor instead.
func (*HandleUserPromptResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{147}
}

func (x *HandleUserPromptResponse) GetJobUuid() string {
	if x != nil {
		return x.JobUuid
	}
	return ""
}

type GetUserPromptJobRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Context       ProContext             `protobuf:"varint,1,opt,name=context,proto3,enum=ProContext" json:"context,omitempty"`
	JobUuid       string                 `protobuf:"bytes,2,opt,name=job_uuid,json=jobUuid,proto3" json:"job_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetUserPromptJobRequest) Reset() {
	*x = GetUserPromptJobRequest{}
	mi := &file_product_proto_msgTypes[148]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserPromptJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserPromptJobRequest) ProtoMessage() {}

func (x *GetUserPromptJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[148]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserPromptJobRequest.ProtoReflect.Descriptor instead.
func (*GetUserPromptJobRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{148}
}

func (x *GetUserPromptJobRequest) GetContext() ProContext {
	if x != nil {
		return x.Context
	}
	return ProContext_PRO_CONTEXT_INVALID
}

func (x *GetUserPromptJobRequest) GetJobUuid() string {
	if x != nil {
		return x.JobUuid
	}
	return ""
}

type GetUserPromptJobResponse struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	CurrentStep      string                 `protobuf:"bytes,1,opt,name=current_step,json=currentStep,proto3" json:"current_step,omitempty"`
	ErrorMessage     string                 `protobuf:"bytes,2,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"` // only if step is failed
	GeneratedContent string                 `protobuf:"bytes,3,opt,name=generated_content,json=generatedContent,proto3" json:"generated_content,omitempty"`
	ProjectVersion   *gen.ProjectVersion    `protobuf:"bytes,4,opt,name=project_version,json=projectVersion,proto3" json:"project_version,omitempty"` // optional only returned for editor context
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *GetUserPromptJobResponse) Reset() {
	*x = GetUserPromptJobResponse{}
	mi := &file_product_proto_msgTypes[149]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetUserPromptJobResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetUserPromptJobResponse) ProtoMessage() {}

func (x *GetUserPromptJobResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[149]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetUserPromptJobResponse.ProtoReflect.Descriptor instead.
func (*GetUserPromptJobResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{149}
}

func (x *GetUserPromptJobResponse) GetCurrentStep() string {
	if x != nil {
		return x.CurrentStep
	}
	return ""
}

func (x *GetUserPromptJobResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *GetUserPromptJobResponse) GetGeneratedContent() string {
	if x != nil {
		return x.GeneratedContent
	}
	return ""
}

func (x *GetUserPromptJobResponse) GetProjectVersion() *gen.ProjectVersion {
	if x != nil {
		return x.ProjectVersion
	}
	return nil
}

type HandleFollowupUserPromptRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Context        ProContext             `protobuf:"varint,1,opt,name=context,proto3,enum=ProContext" json:"context,omitempty"`
	JobUuid        string                 `protobuf:"bytes,2,opt,name=job_uuid,json=jobUuid,proto3" json:"job_uuid,omitempty"`
	FollowupPrompt string                 `protobuf:"bytes,3,opt,name=followup_prompt,json=followupPrompt,proto3" json:"followup_prompt,omitempty"`
	AdditionalData string                 `protobuf:"bytes,4,opt,name=additional_data,json=additionalData,proto3" json:"additional_data,omitempty"` // optional additional data to assist with the prompt handling
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *HandleFollowupUserPromptRequest) Reset() {
	*x = HandleFollowupUserPromptRequest{}
	mi := &file_product_proto_msgTypes[150]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleFollowupUserPromptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleFollowupUserPromptRequest) ProtoMessage() {}

func (x *HandleFollowupUserPromptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[150]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleFollowupUserPromptRequest.ProtoReflect.Descriptor instead.
func (*HandleFollowupUserPromptRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{150}
}

func (x *HandleFollowupUserPromptRequest) GetContext() ProContext {
	if x != nil {
		return x.Context
	}
	return ProContext_PRO_CONTEXT_INVALID
}

func (x *HandleFollowupUserPromptRequest) GetJobUuid() string {
	if x != nil {
		return x.JobUuid
	}
	return ""
}

func (x *HandleFollowupUserPromptRequest) GetFollowupPrompt() string {
	if x != nil {
		return x.FollowupPrompt
	}
	return ""
}

func (x *HandleFollowupUserPromptRequest) GetAdditionalData() string {
	if x != nil {
		return x.AdditionalData
	}
	return ""
}

type HandleFollowupUserPromptResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	JobUuid       string                 `protobuf:"bytes,1,opt,name=job_uuid,json=jobUuid,proto3" json:"job_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HandleFollowupUserPromptResponse) Reset() {
	*x = HandleFollowupUserPromptResponse{}
	mi := &file_product_proto_msgTypes[151]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HandleFollowupUserPromptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HandleFollowupUserPromptResponse) ProtoMessage() {}

func (x *HandleFollowupUserPromptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[151]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HandleFollowupUserPromptResponse.ProtoReflect.Descriptor instead.
func (*HandleFollowupUserPromptResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{151}
}

func (x *HandleFollowupUserPromptResponse) GetJobUuid() string {
	if x != nil {
		return x.JobUuid
	}
	return ""
}

type FinalizeUserPromptRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Context       ProContext             `protobuf:"varint,1,opt,name=context,proto3,enum=ProContext" json:"context,omitempty"`
	JobUuid       string                 `protobuf:"bytes,2,opt,name=job_uuid,json=jobUuid,proto3" json:"job_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FinalizeUserPromptRequest) Reset() {
	*x = FinalizeUserPromptRequest{}
	mi := &file_product_proto_msgTypes[152]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FinalizeUserPromptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FinalizeUserPromptRequest) ProtoMessage() {}

func (x *FinalizeUserPromptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[152]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FinalizeUserPromptRequest.ProtoReflect.Descriptor instead.
func (*FinalizeUserPromptRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{152}
}

func (x *FinalizeUserPromptRequest) GetContext() ProContext {
	if x != nil {
		return x.Context
	}
	return ProContext_PRO_CONTEXT_INVALID
}

func (x *FinalizeUserPromptRequest) GetJobUuid() string {
	if x != nil {
		return x.JobUuid
	}
	return ""
}

type FinalizeUserPromptResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FinalizeUserPromptResponse) Reset() {
	*x = FinalizeUserPromptResponse{}
	mi := &file_product_proto_msgTypes[153]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FinalizeUserPromptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FinalizeUserPromptResponse) ProtoMessage() {}

func (x *FinalizeUserPromptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[153]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FinalizeUserPromptResponse.ProtoReflect.Descriptor instead.
func (*FinalizeUserPromptResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{153}
}

type ListAIUsageForUserRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	PageSize       int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken      string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	OrderBy        string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3" json:"order_by,omitempty"`
	MembershipUuid string                 `protobuf:"bytes,4,opt,name=membership_uuid,json=membershipUuid,proto3" json:"membership_uuid,omitempty"` // this is required
	StartDate      string                 `protobuf:"bytes,5,opt,name=start_date,json=startDate,proto3" json:"start_date,omitempty"`                // YYYY-MM-DD
	EndDate        string                 `protobuf:"bytes,6,opt,name=end_date,json=endDate,proto3" json:"end_date,omitempty"`                      // YYYY-MM-DD
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ListAIUsageForUserRequest) Reset() {
	*x = ListAIUsageForUserRequest{}
	mi := &file_product_proto_msgTypes[154]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAIUsageForUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAIUsageForUserRequest) ProtoMessage() {}

func (x *ListAIUsageForUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[154]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAIUsageForUserRequest.ProtoReflect.Descriptor instead.
func (*ListAIUsageForUserRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{154}
}

func (x *ListAIUsageForUserRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListAIUsageForUserRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListAIUsageForUserRequest) GetOrderBy() string {
	if x != nil {
		return x.OrderBy
	}
	return ""
}

func (x *ListAIUsageForUserRequest) GetMembershipUuid() string {
	if x != nil {
		return x.MembershipUuid
	}
	return ""
}

func (x *ListAIUsageForUserRequest) GetStartDate() string {
	if x != nil {
		return x.StartDate
	}
	return ""
}

func (x *ListAIUsageForUserRequest) GetEndDate() string {
	if x != nil {
		return x.EndDate
	}
	return ""
}

type ListAIUsageForUserResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Usages        []*gen.AiUsage         `protobuf:"bytes,1,rep,name=usages,proto3" json:"usages,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAIUsageForUserResponse) Reset() {
	*x = ListAIUsageForUserResponse{}
	mi := &file_product_proto_msgTypes[155]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAIUsageForUserResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAIUsageForUserResponse) ProtoMessage() {}

func (x *ListAIUsageForUserResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[155]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAIUsageForUserResponse.ProtoReflect.Descriptor instead.
func (*ListAIUsageForUserResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{155}
}

func (x *ListAIUsageForUserResponse) GetUsages() []*gen.AiUsage {
	if x != nil {
		return x.Usages
	}
	return nil
}

func (x *ListAIUsageForUserResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetAIUsageForUserInPeriodRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	MembershipUuid string                 `protobuf:"bytes,1,opt,name=membership_uuid,json=membershipUuid,proto3" json:"membership_uuid,omitempty"` // this is required
	StartDate      string                 `protobuf:"bytes,2,opt,name=start_date,json=startDate,proto3" json:"start_date,omitempty"`                // YYYY-MM-DD optional, otherwise will use the current membership period start date
	EndDate        string                 `protobuf:"bytes,3,opt,name=end_date,json=endDate,proto3" json:"end_date,omitempty"`                      // YYYY-MM-DD optional, otherwise will use the current membership period end date
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetAIUsageForUserInPeriodRequest) Reset() {
	*x = GetAIUsageForUserInPeriodRequest{}
	mi := &file_product_proto_msgTypes[156]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAIUsageForUserInPeriodRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAIUsageForUserInPeriodRequest) ProtoMessage() {}

func (x *GetAIUsageForUserInPeriodRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[156]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAIUsageForUserInPeriodRequest.ProtoReflect.Descriptor instead.
func (*GetAIUsageForUserInPeriodRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{156}
}

func (x *GetAIUsageForUserInPeriodRequest) GetMembershipUuid() string {
	if x != nil {
		return x.MembershipUuid
	}
	return ""
}

func (x *GetAIUsageForUserInPeriodRequest) GetStartDate() string {
	if x != nil {
		return x.StartDate
	}
	return ""
}

func (x *GetAIUsageForUserInPeriodRequest) GetEndDate() string {
	if x != nil {
		return x.EndDate
	}
	return ""
}

type GetAIUsageForUserInPeriodResponse struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	InputTokensUsed     int64                  `protobuf:"varint,1,opt,name=input_tokens_used,json=inputTokensUsed,proto3" json:"input_tokens_used,omitempty"`
	OutputTokensUsed    int64                  `protobuf:"varint,2,opt,name=output_tokens_used,json=outputTokensUsed,proto3" json:"output_tokens_used,omitempty"`
	RemainingPercentage float32                `protobuf:"fixed32,3,opt,name=remaining_percentage,json=remainingPercentage,proto3" json:"remaining_percentage,omitempty"`
	StartDate           string                 `protobuf:"bytes,4,opt,name=start_date,json=startDate,proto3" json:"start_date,omitempty"`              // YYYY-MM-DD
	EndDate             string                 `protobuf:"bytes,5,opt,name=end_date,json=endDate,proto3" json:"end_date,omitempty"`                    // YYYY-MM-DD
	DaysRemaining       int32                  `protobuf:"varint,6,opt,name=days_remaining,json=daysRemaining,proto3" json:"days_remaining,omitempty"` // days remaining in the membership period
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *GetAIUsageForUserInPeriodResponse) Reset() {
	*x = GetAIUsageForUserInPeriodResponse{}
	mi := &file_product_proto_msgTypes[157]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAIUsageForUserInPeriodResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAIUsageForUserInPeriodResponse) ProtoMessage() {}

func (x *GetAIUsageForUserInPeriodResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[157]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAIUsageForUserInPeriodResponse.ProtoReflect.Descriptor instead.
func (*GetAIUsageForUserInPeriodResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{157}
}

func (x *GetAIUsageForUserInPeriodResponse) GetInputTokensUsed() int64 {
	if x != nil {
		return x.InputTokensUsed
	}
	return 0
}

func (x *GetAIUsageForUserInPeriodResponse) GetOutputTokensUsed() int64 {
	if x != nil {
		return x.OutputTokensUsed
	}
	return 0
}

func (x *GetAIUsageForUserInPeriodResponse) GetRemainingPercentage() float32 {
	if x != nil {
		return x.RemainingPercentage
	}
	return 0
}

func (x *GetAIUsageForUserInPeriodResponse) GetStartDate() string {
	if x != nil {
		return x.StartDate
	}
	return ""
}

func (x *GetAIUsageForUserInPeriodResponse) GetEndDate() string {
	if x != nil {
		return x.EndDate
	}
	return ""
}

func (x *GetAIUsageForUserInPeriodResponse) GetDaysRemaining() int32 {
	if x != nil {
		return x.DaysRemaining
	}
	return 0
}

// local agent start
type RegisterLocalAgentRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	MachineName   string                 `protobuf:"bytes,1,opt,name=machine_name,json=machineName,proto3" json:"machine_name,omitempty"`
	Os            string                 `protobuf:"bytes,2,opt,name=os,proto3" json:"os,omitempty"`
	CliVersion    string                 `protobuf:"bytes,3,opt,name=cli_version,json=cliVersion,proto3" json:"cli_version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterLocalAgentRequest) Reset() {
	*x = RegisterLocalAgentRequest{}
	mi := &file_product_proto_msgTypes[158]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterLocalAgentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterLocalAgentRequest) ProtoMessage() {}

func (x *RegisterLocalAgentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[158]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterLocalAgentRequest.ProtoReflect.Descriptor instead.
func (*RegisterLocalAgentRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{158}
}

func (x *RegisterLocalAgentRequest) GetMachineName() string {
	if x != nil {
		return x.MachineName
	}
	return ""
}

func (x *RegisterLocalAgentRequest) GetOs() string {
	if x != nil {
		return x.Os
	}
	return ""
}

func (x *RegisterLocalAgentRequest) GetCliVersion() string {
	if x != nil {
		return x.CliVersion
	}
	return ""
}

type RegisterLocalAgentResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	LocalAgentUuid string                 `protobuf:"bytes,1,opt,name=local_agent_uuid,json=localAgentUuid,proto3" json:"local_agent_uuid,omitempty"`
	// Plaintext token. Returned ONCE at registration. Cloud only stores its SHA-256.
	// CLI must persist this; there is no API to retrieve it later.
	LocalAgentToken string `protobuf:"bytes,2,opt,name=local_agent_token,json=localAgentToken,proto3" json:"local_agent_token,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *RegisterLocalAgentResponse) Reset() {
	*x = RegisterLocalAgentResponse{}
	mi := &file_product_proto_msgTypes[159]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterLocalAgentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterLocalAgentResponse) ProtoMessage() {}

func (x *RegisterLocalAgentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[159]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterLocalAgentResponse.ProtoReflect.Descriptor instead.
func (*RegisterLocalAgentResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{159}
}

func (x *RegisterLocalAgentResponse) GetLocalAgentUuid() string {
	if x != nil {
		return x.LocalAgentUuid
	}
	return ""
}

func (x *RegisterLocalAgentResponse) GetLocalAgentToken() string {
	if x != nil {
		return x.LocalAgentToken
	}
	return ""
}

type RevokeLocalAgentRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	LocalAgentUuid string                 `protobuf:"bytes,1,opt,name=local_agent_uuid,json=localAgentUuid,proto3" json:"local_agent_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RevokeLocalAgentRequest) Reset() {
	*x = RevokeLocalAgentRequest{}
	mi := &file_product_proto_msgTypes[160]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevokeLocalAgentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevokeLocalAgentRequest) ProtoMessage() {}

func (x *RevokeLocalAgentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[160]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevokeLocalAgentRequest.ProtoReflect.Descriptor instead.
func (*RevokeLocalAgentRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{160}
}

func (x *RevokeLocalAgentRequest) GetLocalAgentUuid() string {
	if x != nil {
		return x.LocalAgentUuid
	}
	return ""
}

type RevokeLocalAgentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RevokeLocalAgentResponse) Reset() {
	*x = RevokeLocalAgentResponse{}
	mi := &file_product_proto_msgTypes[161]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevokeLocalAgentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevokeLocalAgentResponse) ProtoMessage() {}

func (x *RevokeLocalAgentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[161]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevokeLocalAgentResponse.ProtoReflect.Descriptor instead.
func (*RevokeLocalAgentResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{161}
}

type ListLocalAgentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListLocalAgentsRequest) Reset() {
	*x = ListLocalAgentsRequest{}
	mi := &file_product_proto_msgTypes[162]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListLocalAgentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListLocalAgentsRequest) ProtoMessage() {}

func (x *ListLocalAgentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[162]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListLocalAgentsRequest.ProtoReflect.Descriptor instead.
func (*ListLocalAgentsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{162}
}

type ListLocalAgentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LocalAgents   []*gen.LocalAgent      `protobuf:"bytes,1,rep,name=local_agents,json=localAgents,proto3" json:"local_agents,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListLocalAgentsResponse) Reset() {
	*x = ListLocalAgentsResponse{}
	mi := &file_product_proto_msgTypes[163]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListLocalAgentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListLocalAgentsResponse) ProtoMessage() {}

func (x *ListLocalAgentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[163]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListLocalAgentsResponse.ProtoReflect.Descriptor instead.
func (*ListLocalAgentsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{163}
}

func (x *ListLocalAgentsResponse) GetLocalAgents() []*gen.LocalAgent {
	if x != nil {
		return x.LocalAgents
	}
	return nil
}

type UpdateLocalAgentConnectionsRequest struct {
	state          protoimpl.MessageState      `protogen:"open.v1"`
	LocalAgentUuid string                      `protobuf:"bytes,1,opt,name=local_agent_uuid,json=localAgentUuid,proto3" json:"local_agent_uuid,omitempty"`
	Connections    []*gen.LocalAgentConnection `protobuf:"bytes,2,rep,name=connections,proto3" json:"connections,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UpdateLocalAgentConnectionsRequest) Reset() {
	*x = UpdateLocalAgentConnectionsRequest{}
	mi := &file_product_proto_msgTypes[164]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateLocalAgentConnectionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateLocalAgentConnectionsRequest) ProtoMessage() {}

func (x *UpdateLocalAgentConnectionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[164]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateLocalAgentConnectionsRequest.ProtoReflect.Descriptor instead.
func (*UpdateLocalAgentConnectionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{164}
}

func (x *UpdateLocalAgentConnectionsRequest) GetLocalAgentUuid() string {
	if x != nil {
		return x.LocalAgentUuid
	}
	return ""
}

func (x *UpdateLocalAgentConnectionsRequest) GetConnections() []*gen.LocalAgentConnection {
	if x != nil {
		return x.Connections
	}
	return nil
}

type UpdateLocalAgentConnectionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateLocalAgentConnectionsResponse) Reset() {
	*x = UpdateLocalAgentConnectionsResponse{}
	mi := &file_product_proto_msgTypes[165]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateLocalAgentConnectionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateLocalAgentConnectionsResponse) ProtoMessage() {}

func (x *UpdateLocalAgentConnectionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[165]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateLocalAgentConnectionsResponse.ProtoReflect.Descriptor instead.
func (*UpdateLocalAgentConnectionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{165}
}

type GetLocalAgentConnectionsRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	LocalAgentUuid string                 `protobuf:"bytes,1,opt,name=local_agent_uuid,json=localAgentUuid,proto3" json:"local_agent_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetLocalAgentConnectionsRequest) Reset() {
	*x = GetLocalAgentConnectionsRequest{}
	mi := &file_product_proto_msgTypes[166]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLocalAgentConnectionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLocalAgentConnectionsRequest) ProtoMessage() {}

func (x *GetLocalAgentConnectionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[166]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLocalAgentConnectionsRequest.ProtoReflect.Descriptor instead.
func (*GetLocalAgentConnectionsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{166}
}

func (x *GetLocalAgentConnectionsRequest) GetLocalAgentUuid() string {
	if x != nil {
		return x.LocalAgentUuid
	}
	return ""
}

type GetLocalAgentConnectionsResponse struct {
	state         protoimpl.MessageState      `protogen:"open.v1"`
	Connections   []*gen.LocalAgentConnection `protobuf:"bytes,1,rep,name=connections,proto3" json:"connections,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetLocalAgentConnectionsResponse) Reset() {
	*x = GetLocalAgentConnectionsResponse{}
	mi := &file_product_proto_msgTypes[167]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetLocalAgentConnectionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetLocalAgentConnectionsResponse) ProtoMessage() {}

func (x *GetLocalAgentConnectionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[167]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetLocalAgentConnectionsResponse.ProtoReflect.Descriptor instead.
func (*GetLocalAgentConnectionsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{167}
}

func (x *GetLocalAgentConnectionsResponse) GetConnections() []*gen.LocalAgentConnection {
	if x != nil {
		return x.Connections
	}
	return nil
}

// deployments
type UpsertDeploymentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The deployment to record. The server stamps user_uuid + created_by/
	// updated_by + status from the authed token; uuid is minted on create and
	// preserved on update. Idempotent by (user_uuid, host, identifier): a second
	// deploy of the same app on the same box updates the existing row (bumping
	// project_version_uuid, ports, urls, last_deployed_at) instead of inserting.
	Deployment    *gen.Deployment `protobuf:"bytes,1,opt,name=deployment,proto3" json:"deployment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpsertDeploymentRequest) Reset() {
	*x = UpsertDeploymentRequest{}
	mi := &file_product_proto_msgTypes[168]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpsertDeploymentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpsertDeploymentRequest) ProtoMessage() {}

func (x *UpsertDeploymentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[168]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpsertDeploymentRequest.ProtoReflect.Descriptor instead.
func (*UpsertDeploymentRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{168}
}

func (x *UpsertDeploymentRequest) GetDeployment() *gen.Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

type UpsertDeploymentResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The persisted row, including the server-minted uuid.
	Deployment    *gen.Deployment `protobuf:"bytes,1,opt,name=deployment,proto3" json:"deployment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpsertDeploymentResponse) Reset() {
	*x = UpsertDeploymentResponse{}
	mi := &file_product_proto_msgTypes[169]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpsertDeploymentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpsertDeploymentResponse) ProtoMessage() {}

func (x *UpsertDeploymentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[169]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpsertDeploymentResponse.ProtoReflect.Descriptor instead.
func (*UpsertDeploymentResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{169}
}

func (x *UpsertDeploymentResponse) GetDeployment() *gen.Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

type ListDeploymentsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeploymentsRequest) Reset() {
	*x = ListDeploymentsRequest{}
	mi := &file_product_proto_msgTypes[170]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeploymentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeploymentsRequest) ProtoMessage() {}

func (x *ListDeploymentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[170]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeploymentsRequest.ProtoReflect.Descriptor instead.
func (*ListDeploymentsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{170}
}

type ListDeploymentsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Newest-first, scoped to the authed user.
	Deployments   []*gen.Deployment `protobuf:"bytes,1,rep,name=deployments,proto3" json:"deployments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeploymentsResponse) Reset() {
	*x = ListDeploymentsResponse{}
	mi := &file_product_proto_msgTypes[171]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeploymentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeploymentsResponse) ProtoMessage() {}

func (x *ListDeploymentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[171]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeploymentsResponse.ProtoReflect.Descriptor instead.
func (*ListDeploymentsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{171}
}

func (x *ListDeploymentsResponse) GetDeployments() []*gen.Deployment {
	if x != nil {
		return x.Deployments
	}
	return nil
}

// MarkDeploymentDestroyed flips the row to status=DESTROYED (kept as history).
// Keyed by (host, identifier) under the authed user — the same upsert key — so
// the CLI destroy path doesn't need to persist the cloud-side uuid.
type MarkDeploymentDestroyedRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Host          string                 `protobuf:"bytes,1,opt,name=host,proto3" json:"host,omitempty"`
	Identifier    string                 `protobuf:"bytes,2,opt,name=identifier,proto3" json:"identifier,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkDeploymentDestroyedRequest) Reset() {
	*x = MarkDeploymentDestroyedRequest{}
	mi := &file_product_proto_msgTypes[172]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkDeploymentDestroyedRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkDeploymentDestroyedRequest) ProtoMessage() {}

func (x *MarkDeploymentDestroyedRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[172]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkDeploymentDestroyedRequest.ProtoReflect.Descriptor instead.
func (*MarkDeploymentDestroyedRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{172}
}

func (x *MarkDeploymentDestroyedRequest) GetHost() string {
	if x != nil {
		return x.Host
	}
	return ""
}

func (x *MarkDeploymentDestroyedRequest) GetIdentifier() string {
	if x != nil {
		return x.Identifier
	}
	return ""
}

type MarkDeploymentDestroyedResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkDeploymentDestroyedResponse) Reset() {
	*x = MarkDeploymentDestroyedResponse{}
	mi := &file_product_proto_msgTypes[173]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkDeploymentDestroyedResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkDeploymentDestroyedResponse) ProtoMessage() {}

func (x *MarkDeploymentDestroyedResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[173]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkDeploymentDestroyedResponse.ProtoReflect.Descriptor instead.
func (*MarkDeploymentDestroyedResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{173}
}

type IssueProvisioningTokenRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Optional project the deployed agent will manage; scopes the token.
	ProjectUuid   string `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IssueProvisioningTokenRequest) Reset() {
	*x = IssueProvisioningTokenRequest{}
	mi := &file_product_proto_msgTypes[174]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IssueProvisioningTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IssueProvisioningTokenRequest) ProtoMessage() {}

func (x *IssueProvisioningTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[174]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IssueProvisioningTokenRequest.ProtoReflect.Descriptor instead.
func (*IssueProvisioningTokenRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{174}
}

func (x *IssueProvisioningTokenRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type IssueProvisioningTokenResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Plaintext provisioning token. Single-use, ~15-min TTL. Returned ONCE.
	ProvisioningToken string `protobuf:"bytes,1,opt,name=provisioning_token,json=provisioningToken,proto3" json:"provisioning_token,omitempty"`
	// Unix seconds at which the token expires (informational, for the caller).
	ExpiresAt     int64 `protobuf:"varint,2,opt,name=expires_at,json=expiresAt,proto3" json:"expires_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IssueProvisioningTokenResponse) Reset() {
	*x = IssueProvisioningTokenResponse{}
	mi := &file_product_proto_msgTypes[175]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IssueProvisioningTokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IssueProvisioningTokenResponse) ProtoMessage() {}

func (x *IssueProvisioningTokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[175]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IssueProvisioningTokenResponse.ProtoReflect.Descriptor instead.
func (*IssueProvisioningTokenResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{175}
}

func (x *IssueProvisioningTokenResponse) GetProvisioningToken() string {
	if x != nil {
		return x.ProvisioningToken
	}
	return ""
}

func (x *IssueProvisioningTokenResponse) GetExpiresAt() int64 {
	if x != nil {
		return x.ExpiresAt
	}
	return 0
}

type ExchangeProvisioningTokenRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The provisioning token authenticates this call in place of a user JWT.
	ProvisioningToken string `protobuf:"bytes,1,opt,name=provisioning_token,json=provisioningToken,proto3" json:"provisioning_token,omitempty"`
	MachineName       string `protobuf:"bytes,2,opt,name=machine_name,json=machineName,proto3" json:"machine_name,omitempty"`
	Os                string `protobuf:"bytes,3,opt,name=os,proto3" json:"os,omitempty"`
	CliVersion        string `protobuf:"bytes,4,opt,name=cli_version,json=cliVersion,proto3" json:"cli_version,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ExchangeProvisioningTokenRequest) Reset() {
	*x = ExchangeProvisioningTokenRequest{}
	mi := &file_product_proto_msgTypes[176]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExchangeProvisioningTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExchangeProvisioningTokenRequest) ProtoMessage() {}

func (x *ExchangeProvisioningTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[176]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExchangeProvisioningTokenRequest.ProtoReflect.Descriptor instead.
func (*ExchangeProvisioningTokenRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{176}
}

func (x *ExchangeProvisioningTokenRequest) GetProvisioningToken() string {
	if x != nil {
		return x.ProvisioningToken
	}
	return ""
}

func (x *ExchangeProvisioningTokenRequest) GetMachineName() string {
	if x != nil {
		return x.MachineName
	}
	return ""
}

func (x *ExchangeProvisioningTokenRequest) GetOs() string {
	if x != nil {
		return x.Os
	}
	return ""
}

func (x *ExchangeProvisioningTokenRequest) GetCliVersion() string {
	if x != nil {
		return x.CliVersion
	}
	return ""
}

type ExchangeProvisioningTokenResponse struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	LocalAgentUuid string                 `protobuf:"bytes,1,opt,name=local_agent_uuid,json=localAgentUuid,proto3" json:"local_agent_uuid,omitempty"`
	// Plaintext agent token. Returned ONCE. Cloud only stores its SHA-256.
	LocalAgentToken string `protobuf:"bytes,2,opt,name=local_agent_token,json=localAgentToken,proto3" json:"local_agent_token,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ExchangeProvisioningTokenResponse) Reset() {
	*x = ExchangeProvisioningTokenResponse{}
	mi := &file_product_proto_msgTypes[177]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExchangeProvisioningTokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExchangeProvisioningTokenResponse) ProtoMessage() {}

func (x *ExchangeProvisioningTokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[177]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExchangeProvisioningTokenResponse.ProtoReflect.Descriptor instead.
func (*ExchangeProvisioningTokenResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{177}
}

func (x *ExchangeProvisioningTokenResponse) GetLocalAgentUuid() string {
	if x != nil {
		return x.LocalAgentUuid
	}
	return ""
}

func (x *ExchangeProvisioningTokenResponse) GetLocalAgentToken() string {
	if x != nil {
		return x.LocalAgentToken
	}
	return ""
}

type CheckExtensionExecutionLimitRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ExtensionUuid string                 `protobuf:"bytes,2,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CheckExtensionExecutionLimitRequest) Reset() {
	*x = CheckExtensionExecutionLimitRequest{}
	mi := &file_product_proto_msgTypes[178]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CheckExtensionExecutionLimitRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CheckExtensionExecutionLimitRequest) ProtoMessage() {}

func (x *CheckExtensionExecutionLimitRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[178]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CheckExtensionExecutionLimitRequest.ProtoReflect.Descriptor instead.
func (*CheckExtensionExecutionLimitRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{178}
}

func (x *CheckExtensionExecutionLimitRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *CheckExtensionExecutionLimitRequest) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

type CheckExtensionExecutionLimitResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsLimited     bool                   `protobuf:"varint,1,opt,name=is_limited,json=isLimited,proto3" json:"is_limited,omitempty"`
	Limit         int64                  `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	Current       int64                  `protobuf:"varint,3,opt,name=current,proto3" json:"current,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CheckExtensionExecutionLimitResponse) Reset() {
	*x = CheckExtensionExecutionLimitResponse{}
	mi := &file_product_proto_msgTypes[179]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CheckExtensionExecutionLimitResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CheckExtensionExecutionLimitResponse) ProtoMessage() {}

func (x *CheckExtensionExecutionLimitResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[179]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CheckExtensionExecutionLimitResponse.ProtoReflect.Descriptor instead.
func (*CheckExtensionExecutionLimitResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{179}
}

func (x *CheckExtensionExecutionLimitResponse) GetIsLimited() bool {
	if x != nil {
		return x.IsLimited
	}
	return false
}

func (x *CheckExtensionExecutionLimitResponse) GetLimit() int64 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *CheckExtensionExecutionLimitResponse) GetCurrent() int64 {
	if x != nil {
		return x.Current
	}
	return 0
}

// automations start
type CreateAutomationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Automation    *gen.Automation        `protobuf:"bytes,1,opt,name=automation,proto3" json:"automation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateAutomationRequest) Reset() {
	*x = CreateAutomationRequest{}
	mi := &file_product_proto_msgTypes[180]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateAutomationRequest) ProtoMessage() {}

func (x *CreateAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[180]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateAutomationRequest.ProtoReflect.Descriptor instead.
func (*CreateAutomationRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{180}
}

func (x *CreateAutomationRequest) GetAutomation() *gen.Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

type CreateAutomationResponse struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Automation *gen.Automation        `protobuf:"bytes,1,opt,name=automation,proto3" json:"automation,omitempty"`
	// hmac signing secret for the webhook, returned only once at creation;
	// stored in the team secret manager, never in the database
	SigningSecret string `protobuf:"bytes,2,opt,name=signing_secret,json=signingSecret,proto3" json:"signing_secret,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateAutomationResponse) Reset() {
	*x = CreateAutomationResponse{}
	mi := &file_product_proto_msgTypes[181]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateAutomationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateAutomationResponse) ProtoMessage() {}

func (x *CreateAutomationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[181]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateAutomationResponse.ProtoReflect.Descriptor instead.
func (*CreateAutomationResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{181}
}

func (x *CreateAutomationResponse) GetAutomation() *gen.Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

func (x *CreateAutomationResponse) GetSigningSecret() string {
	if x != nil {
		return x.SigningSecret
	}
	return ""
}

type RotateAutomationSecretRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	AutomationUuid string                 `protobuf:"bytes,1,opt,name=automation_uuid,json=automationUuid,proto3" json:"automation_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RotateAutomationSecretRequest) Reset() {
	*x = RotateAutomationSecretRequest{}
	mi := &file_product_proto_msgTypes[182]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RotateAutomationSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RotateAutomationSecretRequest) ProtoMessage() {}

func (x *RotateAutomationSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[182]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RotateAutomationSecretRequest.ProtoReflect.Descriptor instead.
func (*RotateAutomationSecretRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{182}
}

func (x *RotateAutomationSecretRequest) GetAutomationUuid() string {
	if x != nil {
		return x.AutomationUuid
	}
	return ""
}

type RotateAutomationSecretResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// the new hmac signing secret, returned only once — the previous secret
	// stops being used for new deliveries immediately (in-flight deliveries
	// may sign with the old secret for up to the dispatcher cache TTL)
	SigningSecret string `protobuf:"bytes,1,opt,name=signing_secret,json=signingSecret,proto3" json:"signing_secret,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RotateAutomationSecretResponse) Reset() {
	*x = RotateAutomationSecretResponse{}
	mi := &file_product_proto_msgTypes[183]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RotateAutomationSecretResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RotateAutomationSecretResponse) ProtoMessage() {}

func (x *RotateAutomationSecretResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[183]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RotateAutomationSecretResponse.ProtoReflect.Descriptor instead.
func (*RotateAutomationSecretResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{183}
}

func (x *RotateAutomationSecretResponse) GetSigningSecret() string {
	if x != nil {
		return x.SigningSecret
	}
	return ""
}

type UpdateAutomationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Automation    *gen.Automation        `protobuf:"bytes,1,opt,name=automation,proto3" json:"automation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateAutomationRequest) Reset() {
	*x = UpdateAutomationRequest{}
	mi := &file_product_proto_msgTypes[184]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateAutomationRequest) ProtoMessage() {}

func (x *UpdateAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[184]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateAutomationRequest.ProtoReflect.Descriptor instead.
func (*UpdateAutomationRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{184}
}

func (x *UpdateAutomationRequest) GetAutomation() *gen.Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

type DeleteAutomationRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	AutomationUuid string                 `protobuf:"bytes,1,opt,name=automation_uuid,json=automationUuid,proto3" json:"automation_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *DeleteAutomationRequest) Reset() {
	*x = DeleteAutomationRequest{}
	mi := &file_product_proto_msgTypes[185]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteAutomationRequest) ProtoMessage() {}

func (x *DeleteAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[185]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteAutomationRequest.ProtoReflect.Descriptor instead.
func (*DeleteAutomationRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{185}
}

func (x *DeleteAutomationRequest) GetAutomationUuid() string {
	if x != nil {
		return x.AutomationUuid
	}
	return ""
}

type DeleteAutomationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteAutomationResponse) Reset() {
	*x = DeleteAutomationResponse{}
	mi := &file_product_proto_msgTypes[186]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteAutomationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteAutomationResponse) ProtoMessage() {}

func (x *DeleteAutomationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[186]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteAutomationResponse.ProtoReflect.Descriptor instead.
func (*DeleteAutomationResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{186}
}

type GetAutomationRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	AutomationUuid string                 `protobuf:"bytes,1,opt,name=automation_uuid,json=automationUuid,proto3" json:"automation_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GetAutomationRequest) Reset() {
	*x = GetAutomationRequest{}
	mi := &file_product_proto_msgTypes[187]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAutomationRequest) ProtoMessage() {}

func (x *GetAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[187]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAutomationRequest.ProtoReflect.Descriptor instead.
func (*GetAutomationRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{187}
}

func (x *GetAutomationRequest) GetAutomationUuid() string {
	if x != nil {
		return x.AutomationUuid
	}
	return ""
}

type ListAutomationsForProjectRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProjectUuid   string                 `protobuf:"bytes,1,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAutomationsForProjectRequest) Reset() {
	*x = ListAutomationsForProjectRequest{}
	mi := &file_product_proto_msgTypes[188]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationsForProjectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationsForProjectRequest) ProtoMessage() {}

func (x *ListAutomationsForProjectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[188]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationsForProjectRequest.ProtoReflect.Descriptor instead.
func (*ListAutomationsForProjectRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{188}
}

func (x *ListAutomationsForProjectRequest) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

type ListAutomationsForProjectResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Automations   []*gen.Automation      `protobuf:"bytes,1,rep,name=automations,proto3" json:"automations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAutomationsForProjectResponse) Reset() {
	*x = ListAutomationsForProjectResponse{}
	mi := &file_product_proto_msgTypes[189]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationsForProjectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationsForProjectResponse) ProtoMessage() {}

func (x *ListAutomationsForProjectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[189]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationsForProjectResponse.ProtoReflect.Descriptor instead.
func (*ListAutomationsForProjectResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{189}
}

func (x *ListAutomationsForProjectResponse) GetAutomations() []*gen.Automation {
	if x != nil {
		return x.Automations
	}
	return nil
}

type TestAutomationRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	AutomationUuid string                 `protobuf:"bytes,1,opt,name=automation_uuid,json=automationUuid,proto3" json:"automation_uuid,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *TestAutomationRequest) Reset() {
	*x = TestAutomationRequest{}
	mi := &file_product_proto_msgTypes[190]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TestAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TestAutomationRequest) ProtoMessage() {}

func (x *TestAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[190]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TestAutomationRequest.ProtoReflect.Descriptor instead.
func (*TestAutomationRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{190}
}

func (x *TestAutomationRequest) GetAutomationUuid() string {
	if x != nil {
		return x.AutomationUuid
	}
	return ""
}

type TestAutomationResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Success         bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	StatusCode      int64                  `protobuf:"varint,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	ResponseSnippet string                 `protobuf:"bytes,3,opt,name=response_snippet,json=responseSnippet,proto3" json:"response_snippet,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *TestAutomationResponse) Reset() {
	*x = TestAutomationResponse{}
	mi := &file_product_proto_msgTypes[191]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TestAutomationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TestAutomationResponse) ProtoMessage() {}

func (x *TestAutomationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[191]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TestAutomationResponse.ProtoReflect.Descriptor instead.
func (*TestAutomationResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{191}
}

func (x *TestAutomationResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *TestAutomationResponse) GetStatusCode() int64 {
	if x != nil {
		return x.StatusCode
	}
	return 0
}

func (x *TestAutomationResponse) GetResponseSnippet() string {
	if x != nil {
		return x.ResponseSnippet
	}
	return ""
}

type ListAutomationEventsRequest struct {
	state          protoimpl.MessageState    `protogen:"open.v1"`
	AutomationUuid string                    `protobuf:"bytes,1,opt,name=automation_uuid,json=automationUuid,proto3" json:"automation_uuid,omitempty"`
	Status         gen.AutomationEventStatus `protobuf:"varint,2,opt,name=status,proto3,enum=nem.AutomationEventStatus" json:"status,omitempty"` // optional filter, invalid/zero = all
	PageSize       int64                     `protobuf:"varint,3,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageToken      string                    `protobuf:"bytes,4,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ListAutomationEventsRequest) Reset() {
	*x = ListAutomationEventsRequest{}
	mi := &file_product_proto_msgTypes[192]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationEventsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationEventsRequest) ProtoMessage() {}

func (x *ListAutomationEventsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[192]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationEventsRequest.ProtoReflect.Descriptor instead.
func (*ListAutomationEventsRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{192}
}

func (x *ListAutomationEventsRequest) GetAutomationUuid() string {
	if x != nil {
		return x.AutomationUuid
	}
	return ""
}

func (x *ListAutomationEventsRequest) GetStatus() gen.AutomationEventStatus {
	if x != nil {
		return x.Status
	}
	return gen.AutomationEventStatus(0)
}

func (x *ListAutomationEventsRequest) GetPageSize() int64 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListAutomationEventsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

type ListAutomationEventsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// payload is excluded from list responses; fetch a single event to get it
	AutomationEvents []*gen.AutomationEvent `protobuf:"bytes,1,rep,name=automation_events,json=automationEvents,proto3" json:"automation_events,omitempty"`
	NextPageToken    string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *ListAutomationEventsResponse) Reset() {
	*x = ListAutomationEventsResponse{}
	mi := &file_product_proto_msgTypes[193]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationEventsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationEventsResponse) ProtoMessage() {}

func (x *ListAutomationEventsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[193]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationEventsResponse.ProtoReflect.Descriptor instead.
func (*ListAutomationEventsResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{193}
}

func (x *ListAutomationEventsResponse) GetAutomationEvents() []*gen.AutomationEvent {
	if x != nil {
		return x.AutomationEvents
	}
	return nil
}

func (x *ListAutomationEventsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetAutomationEventRequest struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	AutomationEventUuid string                 `protobuf:"bytes,1,opt,name=automation_event_uuid,json=automationEventUuid,proto3" json:"automation_event_uuid,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *GetAutomationEventRequest) Reset() {
	*x = GetAutomationEventRequest{}
	mi := &file_product_proto_msgTypes[194]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAutomationEventRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAutomationEventRequest) ProtoMessage() {}

func (x *GetAutomationEventRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[194]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAutomationEventRequest.ProtoReflect.Descriptor instead.
func (*GetAutomationEventRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{194}
}

func (x *GetAutomationEventRequest) GetAutomationEventUuid() string {
	if x != nil {
		return x.AutomationEventUuid
	}
	return ""
}

type RetryAutomationEventRequest struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	AutomationEventUuid string                 `protobuf:"bytes,1,opt,name=automation_event_uuid,json=automationEventUuid,proto3" json:"automation_event_uuid,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *RetryAutomationEventRequest) Reset() {
	*x = RetryAutomationEventRequest{}
	mi := &file_product_proto_msgTypes[195]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RetryAutomationEventRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RetryAutomationEventRequest) ProtoMessage() {}

func (x *RetryAutomationEventRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[195]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RetryAutomationEventRequest.ProtoReflect.Descriptor instead.
func (*RetryAutomationEventRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{195}
}

func (x *RetryAutomationEventRequest) GetAutomationEventUuid() string {
	if x != nil {
		return x.AutomationEventUuid
	}
	return ""
}

type MarkChangeRequestAppliedRequest struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequestUuid string                 `protobuf:"bytes,1,opt,name=change_request_uuid,json=changeRequestUuid,proto3" json:"change_request_uuid,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *MarkChangeRequestAppliedRequest) Reset() {
	*x = MarkChangeRequestAppliedRequest{}
	mi := &file_product_proto_msgTypes[196]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkChangeRequestAppliedRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkChangeRequestAppliedRequest) ProtoMessage() {}

func (x *MarkChangeRequestAppliedRequest) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[196]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkChangeRequestAppliedRequest.ProtoReflect.Descriptor instead.
func (*MarkChangeRequestAppliedRequest) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{196}
}

func (x *MarkChangeRequestAppliedRequest) GetChangeRequestUuid() string {
	if x != nil {
		return x.ChangeRequestUuid
	}
	return ""
}

type MarkChangeRequestAppliedResponse struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	ChangeRequest           *gen.ChangeRequest     `protobuf:"bytes,1,opt,name=change_request,json=changeRequest,proto3" json:"change_request,omitempty"`
	AutomationEventsCreated int64                  `protobuf:"varint,2,opt,name=automation_events_created,json=automationEventsCreated,proto3" json:"automation_events_created,omitempty"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *MarkChangeRequestAppliedResponse) Reset() {
	*x = MarkChangeRequestAppliedResponse{}
	mi := &file_product_proto_msgTypes[197]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkChangeRequestAppliedResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkChangeRequestAppliedResponse) ProtoMessage() {}

func (x *MarkChangeRequestAppliedResponse) ProtoReflect() protoreflect.Message {
	mi := &file_product_proto_msgTypes[197]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkChangeRequestAppliedResponse.ProtoReflect.Descriptor instead.
func (*MarkChangeRequestAppliedResponse) Descriptor() ([]byte, []int) {
	return file_product_proto_rawDescGZIP(), []int{197}
}

func (x *MarkChangeRequestAppliedResponse) GetChangeRequest() *gen.ChangeRequest {
	if x != nil {
		return x.ChangeRequest
	}
	return nil
}

func (x *MarkChangeRequestAppliedResponse) GetAutomationEventsCreated() int64 {
	if x != nil {
		return x.AutomationEventsCreated
	}
	return 0
}

var File_product_proto protoreflect.FileDescriptor

const file_product_proto_rawDesc = "" +
	"\n" +
	"\rproduct.proto\x1a\venums.proto\x1a\n" +
	"team.proto\x1a\x0fuser_team.proto\x1a\x12user_project.proto\x1a\n" +
	"user.proto\x1a\rproject.proto\x1a\x15project_version.proto\x1a\x10connection.proto\x1a\x12object_store.proto\x1a\x0fextension.proto\x1a\x17extension_version.proto\x1a\x19extension_execution.proto\x1a\x14change_request.proto\x1a\x10membership.proto\x1a\x0eai_usage.proto\x1a\x11local_agent.proto\x1a\x1clocal_agent_connection.proto\x1a\x10automation.proto\x1a\x16automation_event.proto\x1a\x10deployment.proto\x1a google/protobuf/field_mask.proto\"-\n" +
	"\x0eGetUserRequest\x12\x1b\n" +
	"\tuser_uuid\x18\x01 \x01(\tR\buserUuid\"\x15\n" +
	"\x13GetTokenUserRequest\"-\n" +
	"\x15GetUserByEmailRequest\x12\x14\n" +
	"\x05email\x18\x01 \x01(\tR\x05email\"A\n" +
	"\x1cGetUserRoleForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"\xb7\x01\n" +
	"\x1dGetUserRoleForProjectResponse\x12(\n" +
	"\x04role\x18\x01 \x01(\x0e2\x14.nem.UserProjectRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x02 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x03 \x01(\bR\x12reviewRequiredData\"8\n" +
	"\x19GetUserRoleForTeamRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\"\xb1\x01\n" +
	"\x1aGetUserRoleForTeamResponse\x12%\n" +
	"\x04role\x18\x01 \x01(\x0e2\x11.nem.UserTeamRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x02 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x03 \x01(\bR\x12reviewRequiredData\"2\n" +
	"\x11UpdateUserRequest\x12\x1d\n" +
	"\x04user\x18\x01 \x01(\v2\t.nem.UserR\x04user\"\x8d\x01\n" +
	"\x17ListUsersForTeamRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x12\x1b\n" +
	"\tpage_size\x18\x02 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x03 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x04 \x01(\tR\aorderBy\"\x91\x01\n" +
	"\x18ListUsersForTeamResponse\x12\x1f\n" +
	"\x05users\x18\x01 \x03(\v2\t.nem.UserR\x05users\x12,\n" +
	"\n" +
	"user_teams\x18\x02 \x03(\v2\r.nem.UserTeamR\tuserTeams\x12&\n" +
	"\x0fnext_page_token\x18\x03 \x01(\tR\rnextPageToken\"\x96\x01\n" +
	"\x1aListUsersForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12\x1b\n" +
	"\tpage_size\x18\x02 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x03 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x04 \x01(\tR\aorderBy\"\x9d\x01\n" +
	"\x1bListUsersForProjectResponse\x12\x1f\n" +
	"\x05users\x18\x01 \x03(\v2\t.nem.UserR\x05users\x125\n" +
	"\ruser_projects\x18\x02 \x03(\v2\x10.nem.UserProjectR\fuserProjects\x12&\n" +
	"\x0fnext_page_token\x18\x03 \x01(\tR\rnextPageToken\"\x90\x02\n" +
	"\x17AddUserToProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12\x1b\n" +
	"\tuser_uuid\x18\x02 \x01(\tR\buserUuid\x12\x1d\n" +
	"\n" +
	"user_email\x18\x03 \x01(\tR\tuserEmail\x12(\n" +
	"\x04role\x18\x04 \x01(\x0e2\x14.nem.UserProjectRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x05 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x06 \x01(\bR\x12reviewRequiredData\"\x82\x02\n" +
	"\x18UpdateUserProjectRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x1b\n" +
	"\tuser_uuid\x18\x02 \x01(\tR\buserUuid\x12\x1d\n" +
	"\n" +
	"user_email\x18\x03 \x01(\tR\tuserEmail\x12(\n" +
	"\x04role\x18\x04 \x01(\x0e2\x14.nem.UserProjectRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x05 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x06 \x01(\bR\x12reviewRequiredData\"U\n" +
	"\x1cRemoveUserFromProjectRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12!\n" +
	"\fproject_uuid\x18\x02 \x01(\tR\vprojectUuid\"\x84\x02\n" +
	"\x14AddUserToTeamRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x12\x1b\n" +
	"\tuser_uuid\x18\x02 \x01(\tR\buserUuid\x12\x1d\n" +
	"\n" +
	"user_email\x18\x03 \x01(\tR\tuserEmail\x12%\n" +
	"\x04role\x18\x04 \x01(\x0e2\x11.nem.UserTeamRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x05 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x06 \x01(\bR\x12reviewRequiredData\"\xfc\x01\n" +
	"\x15UpdateUserTeamRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x1b\n" +
	"\tuser_uuid\x18\x02 \x01(\tR\buserUuid\x12\x1d\n" +
	"\n" +
	"user_email\x18\x03 \x01(\tR\tuserEmail\x12%\n" +
	"\x04role\x18\x04 \x01(\x0e2\x11.nem.UserTeamRoleR\x04role\x12:\n" +
	"\x19review_required_structure\x18\x05 \x01(\bR\x17reviewRequiredStructure\x120\n" +
	"\x14review_required_data\x18\x06 \x01(\bR\x12reviewRequiredData\"L\n" +
	"\x19RemoveUserFromTeamRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x1b\n" +
	"\tteam_uuid\x18\x02 \x01(\tR\bteamUuid\"\x93\x01\n" +
	"\x17ListTeamsForUserRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12!\n" +
	"\fminimal_info\x18\x04 \x01(\bR\vminimalInfo\"c\n" +
	"\x18ListTeamsForUserResponse\x12\x1f\n" +
	"\x05teams\x18\x01 \x03(\v2\t.nem.TeamR\x05teams\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\x98\x01\n" +
	"\x1cListTeamsForAdminUserRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12!\n" +
	"\fminimal_info\x18\x04 \x01(\bR\vminimalInfo\"h\n" +
	"\x1dListTeamsForAdminUserResponse\x12\x1f\n" +
	"\x05teams\x18\x01 \x03(\v2\t.nem.TeamR\x05teams\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"4\n" +
	"\x15GetTeamForUserRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\"2\n" +
	"\x11CreateTeamRequest\x12\x1d\n" +
	"\x04team\x18\x01 \x01(\v2\t.nem.TeamR\x04team\"o\n" +
	"\x11UpdateTeamRequest\x12\x1d\n" +
	"\x04team\x18\x01 \x01(\v2\t.nem.TeamR\x04team\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"f\n" +
	"\x1eGetConnectionWithSecretRequest\x12'\n" +
	"\x0fconnection_uuid\x18\x01 \x01(\tR\x0econnectionUuid\x12\x1b\n" +
	"\tteam_uuid\x18\x02 \x01(\tR\bteamUuid\"m\n" +
	"\x1dCreateConnectionSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x12/\n" +
	"\n" +
	"connection\x18\x02 \x01(\v2\x0f.nem.ConnectionR\n" +
	"connection\"m\n" +
	"\x1dUpdateConnectionSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x12/\n" +
	"\n" +
	"connection\x18\x02 \x01(\v2\x0f.nem.ConnectionR\n" +
	"connection\"m\n" +
	"\x1dDeleteConnectionSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x12/\n" +
	"\n" +
	"connection\x18\x02 \x01(\v2\x0f.nem.ConnectionR\n" +
	"connection\"\x90\x01\n" +
	"\x1aListProjectsForUserRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12\x1b\n" +
	"\tteam_uuid\x18\x04 \x01(\tR\bteamUuid\"o\n" +
	"\x1bListProjectsForUserResponse\x12(\n" +
	"\bprojects\x18\x01 \x03(\v2\f.nem.ProjectR\bprojects\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"=\n" +
	"\x18GetProjectForUserRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\">\n" +
	"\x14CreateProjectRequest\x12&\n" +
	"\aproject\x18\x01 \x01(\v2\f.nem.ProjectR\aproject\"{\n" +
	"\x14UpdateProjectRequest\x12&\n" +
	"\aproject\x18\x01 \x01(\v2\f.nem.ProjectR\aproject\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\":\n" +
	"\x15GetProjectNameRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\";\n" +
	"\x16GetProjectNameResponse\x12!\n" +
	"\fproject_name\x18\x01 \x01(\tR\vprojectName\"{\n" +
	"\x1aListProjectVersionsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12!\n" +
	"\fproject_uuid\x18\x03 \x01(\tR\vprojectUuid\"\x85\x01\n" +
	"\x1bListProjectVersionsResponse\x12>\n" +
	"\x10project_versions\x18\x01 \x03(\v2\x13.nem.ProjectVersionR\x0fprojectVersions\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\xf8\x01\n" +
	"!ListProjectVersionsForUserRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12!\n" +
	"\fproject_uuid\x18\x03 \x01(\tR\vprojectUuid\x12D\n" +
	"\rreview_status\x18\x04 \x01(\x0e2\x1f.nem.ProjectVersionReviewStatusR\freviewStatus\x12.\n" +
	"\x13exclude_json_fields\x18\x05 \x01(\bR\x11excludeJsonFields\"\x8c\x01\n" +
	"\"ListProjectVersionsForUserResponse\x12>\n" +
	"\x10project_versions\x18\x01 \x03(\v2\x13.nem.ProjectVersionR\x0fprojectVersions\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"S\n" +
	"\x1fGetProjectVersionForUserRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"\x89\x01\n" +
	"\x1eGetLatestProjectVersionRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12D\n" +
	"\rreview_status\x18\x02 \x01(\x0e2\x1f.nem.ProjectVersionReviewStatusR\freviewStatus\"\x90\x01\n" +
	"%GetLatestProjectVersionForUserRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12D\n" +
	"\rreview_status\x18\x02 \x01(\x0e2\x1f.nem.ProjectVersionReviewStatusR\freviewStatus\"\x94\x01\n" +
	")GetLatestProjectVersionUUIDForUserRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12D\n" +
	"\rreview_status\x18\x02 \x01(\x0e2\x1f.nem.ProjectVersionReviewStatusR\freviewStatus\"^\n" +
	"*GetLatestProjectVersionUUIDForUserResponse\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"S\n" +
	"\x1fGetProjectVersionVersionRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"<\n" +
	" GetProjectVersionVersionResponse\x12\x18\n" +
	"\aversion\x18\x01 \x01(\x03R\aversion\"V\n" +
	"\"GetProjectVersionIdentifierRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"c\n" +
	"#GetProjectVersionIdentifierResponse\x12<\n" +
	"\x1aproject_version_identifier\x18\x01 \x01(\tR\x18projectVersionIdentifier\"[\n" +
	"\x1bCreateProjectVersionRequest\x12<\n" +
	"\x0fproject_version\x18\x01 \x01(\v2\x13.nem.ProjectVersionR\x0eprojectVersion\"W\n" +
	"#CreateProjectVersionFromJSONRequest\x120\n" +
	"\x14project_version_json\x18\x01 \x01(\tR\x12projectVersionJson\"\x98\x01\n" +
	"\x1bUpdateProjectVersionRequest\x12<\n" +
	"\x0fproject_version\x18\x01 \x01(\v2\x13.nem.ProjectVersionR\x0eprojectVersion\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\xb9\x01\n" +
	"2CreateDraftProjectVersionForExistingProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12\x1e\n" +
	"\n" +
	"identifier\x18\x02 \x01(\tR\n" +
	"identifier\x12@\n" +
	"\x1dbased_on_project_version_uuid\x18\x03 \x01(\tR\x19basedOnProjectVersionUuid\"U\n" +
	"!DiscardDraftProjectVersionRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"\xa8\x01\n" +
	"\"SendProjectVersionForReviewRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12-\n" +
	"\x12review_description\x18\x02 \x01(\tR\x11reviewDescription\x12!\n" +
	"\fai_generated\x18\x03 \x01(\bR\vaiGenerated\"[\n" +
	"'WithdrawFromReviewProjectVersionRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"\x95\x02\n" +
	"\x1bReviewProjectVersionRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12.\n" +
	"\x13change_request_uuid\x18\x02 \x01(\tR\x11changeRequestUuid\x12D\n" +
	"\rreview_status\x18\x03 \x01(\x0e2\x1f.nem.ProjectVersionReviewStatusR\freviewStatus\x12%\n" +
	"\x0ereview_comment\x18\x04 \x01(\tR\rreviewComment\x12'\n" +
	"\x0fpublish_changes\x18\x05 \x01(\bR\x0epublishChanges\"\x99\x01\n" +
	"#UploadProjectVersionSnapshotRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12\x1d\n" +
	"\n" +
	"image_data\x18\x03 \x01(\fR\timageData\"8\n" +
	"$UploadProjectVersionSnapshotResponse\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\"+\n" +
	"\x17GetSignedFileURLRequest\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\",\n" +
	"\x18GetSignedFileURLResponse\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\"\xb9\x02\n" +
	"#UploadExtensionExecutionFileRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12%\n" +
	"\x0eextension_uuid\x18\x03 \x01(\tR\rextensionUuid\x124\n" +
	"\x16extension_version_uuid\x18\x04 \x01(\tR\x14extensionVersionUuid\x12%\n" +
	"\x0eexecution_uuid\x18\x05 \x01(\tR\rexecutionUuid\x12\x12\n" +
	"\x04data\x18\x06 \x01(\fR\x04data\x12%\n" +
	"\x0efile_extension\x18\a \x01(\tR\rfileExtension\"B\n" +
	"$UploadExtensionExecutionFileResponse\x12\x1a\n" +
	"\bfilepath\x18\x01 \x01(\tR\bfilepath\"\xa2\x02\n" +
	" GetExtensionExecutionFileRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12%\n" +
	"\x0eextension_uuid\x18\x03 \x01(\tR\rextensionUuid\x124\n" +
	"\x16extension_version_uuid\x18\x04 \x01(\tR\x14extensionVersionUuid\x12%\n" +
	"\x0eexecution_uuid\x18\x05 \x01(\tR\rexecutionUuid\x12%\n" +
	"\x0efile_extension\x18\x06 \x01(\tR\rfileExtension\"^\n" +
	"!GetExtensionExecutionFileResponse\x12\x1a\n" +
	"\bfilepath\x18\x01 \x01(\tR\bfilepath\x12\x1d\n" +
	"\n" +
	"signed_url\x18\x02 \x01(\tR\tsignedUrl\"b\n" +
	"\x1aUploadExtensionIconRequest\x12%\n" +
	"\x0eextension_uuid\x18\x01 \x01(\tR\rextensionUuid\x12\x1d\n" +
	"\n" +
	"image_data\x18\x02 \x01(\fR\timageData\"9\n" +
	"\x1bUploadExtensionIconResponse\x12\x1a\n" +
	"\bfilepath\x18\x01 \x01(\tR\bfilepath\"\x93\x01\n" +
	" UploadDataManagerMetadataRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12\x1a\n" +
	"\bmetadata\x18\x03 \x01(\tR\bmetadata\"?\n" +
	"!UploadDataManagerMetadataResponse\x12\x1a\n" +
	"\bfilepath\x18\x01 \x01(\tR\bfilepath\"\x94\x02\n" +
	"\x1cUploadRecordFieldFileRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12\x1f\n" +
	"\ventity_uuid\x18\x03 \x01(\tR\n" +
	"entityUuid\x12\x1d\n" +
	"\n" +
	"field_uuid\x18\x04 \x01(\tR\tfieldUuid\x12\x1b\n" +
	"\tfile_name\x18\x05 \x01(\tR\bfileName\x12\x1b\n" +
	"\tfile_data\x18\x06 \x01(\fR\bfileData\x12%\n" +
	"\x0eforce_override\x18\a \x01(\bR\rforceOverride\"1\n" +
	"\x1dUploadRecordFieldFileResponse\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\"\xd4\x01\n" +
	"\"GetRecordFieldSignedFileURLRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12\x1f\n" +
	"\ventity_uuid\x18\x03 \x01(\tR\n" +
	"entityUuid\x12\x1d\n" +
	"\n" +
	"field_uuid\x18\x04 \x01(\tR\tfieldUuid\x12\x19\n" +
	"\bfile_url\x18\x05 \x01(\tR\afileUrl\"D\n" +
	"#GetRecordFieldSignedFileURLResponse\x12\x1d\n" +
	"\n" +
	"signed_url\x18\x01 \x01(\tR\tsignedUrl\"\xcd\x01\n" +
	"\x1bGetRecordFileContentRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x02 \x01(\tR\x12projectVersionUuid\x12\x1f\n" +
	"\ventity_uuid\x18\x03 \x01(\tR\n" +
	"entityUuid\x12\x1d\n" +
	"\n" +
	"field_uuid\x18\x04 \x01(\tR\tfieldUuid\x12\x19\n" +
	"\bfile_url\x18\x05 \x01(\tR\afileUrl\"8\n" +
	"\x1cGetRecordFileContentResponse\x12\x18\n" +
	"\acontent\x18\x01 \x01(\fR\acontent\"8\n" +
	"\x17UploadUserAvatarRequest\x12\x1d\n" +
	"\n" +
	"image_data\x18\x01 \x01(\fR\timageData\",\n" +
	"\x18UploadUserAvatarResponse\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\"\xb4\x01\n" +
	"\x15ListExtensionsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12\x1d\n" +
	"\n" +
	"owner_uuid\x18\x04 \x01(\tR\townerUuid\x12%\n" +
	"\x0epublished_only\x18\x05 \x01(\bR\rpublishedOnly\"p\n" +
	"\x16ListExtensionsResponse\x12.\n" +
	"\n" +
	"extensions\x18\x01 \x03(\v2\x0e.nem.ExtensionR\n" +
	"extensions\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"<\n" +
	"\x13GetExtensionRequest\x12%\n" +
	"\x0eextension_uuid\x18\x01 \x01(\tR\rextensionUuid\"F\n" +
	"\x16CreateExtensionRequest\x12,\n" +
	"\textension\x18\x01 \x01(\v2\x0e.nem.ExtensionR\textension\"\x83\x01\n" +
	"\x16UpdateExtensionRequest\x12,\n" +
	"\textension\x18\x01 \x01(\v2\x0e.nem.ExtensionR\textension\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\x9c\x01\n" +
	"\x1cListExtensionVersionsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12%\n" +
	"\x0eextension_uuid\x18\x04 \x01(\tR\rextensionUuid\"z\n" +
	"\x1dListExtensionVersionsResponse\x121\n" +
	"\bversions\x18\x01 \x03(\v2\x15.nem.ExtensionVersionR\bversions\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"?\n" +
	"\x1aGetExtensionVersionRequest\x12!\n" +
	"\fversion_uuid\x18\x01 \x01(\tR\vversionUuid\"P\n" +
	"\x1dCreateExtensionVersionRequest\x12/\n" +
	"\aversion\x18\x01 \x01(\v2\x15.nem.ExtensionVersionR\aversion\"\x8d\x01\n" +
	"\x1dUpdateExtensionVersionRequest\x12/\n" +
	"\aversion\x18\x01 \x01(\v2\x15.nem.ExtensionVersionR\aversion\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\xdf\x02\n" +
	"\x1eListExtensionExecutionsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12!\n" +
	"\fproject_uuid\x18\x04 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x05 \x01(\tR\x12projectVersionUuid\x124\n" +
	"\x16project_extension_uuid\x18\x06 \x01(\tR\x14projectExtensionUuid\x12%\n" +
	"\x0eextension_uuid\x18\a \x01(\tR\rextensionUuid\x124\n" +
	"\x16extension_version_uuid\x18\b \x01(\tR\x14extensionVersionUuid\"\x82\x01\n" +
	"\x1fListExtensionExecutionsResponse\x127\n" +
	"\n" +
	"executions\x18\x01 \x03(\v2\x17.nem.ExtensionExecutionR\n" +
	"executions\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"E\n" +
	"\x1cGetExtensionExecutionRequest\x12%\n" +
	"\x0eexecution_uuid\x18\x01 \x01(\tR\rexecutionUuid\"X\n" +
	"\x1fCreateExtensionExecutionRequest\x125\n" +
	"\texecution\x18\x01 \x01(\v2\x17.nem.ExtensionExecutionR\texecution\"\x95\x01\n" +
	"\x1fUpdateExtensionExecutionRequest\x125\n" +
	"\texecution\x18\x01 \x01(\v2\x17.nem.ExtensionExecutionR\texecution\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\xde\x02\n" +
	"\x1dListUserChangeRequestsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12!\n" +
	"\fproject_uuid\x18\x04 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x05 \x01(\tR\x12projectVersionUuid\x12C\n" +
	"\rreview_status\x18\x06 \x03(\x0e2\x1e.nem.ChangeRequestReviewStatusR\freviewStatus\x12L\n" +
	"\x13change_request_type\x18\a \x01(\x0e2\x1c.nem.ChangeRequestChangeTypeR\x11changeRequestType\"\x85\x01\n" +
	"\x1eListUserChangeRequestsResponse\x12;\n" +
	"\x0fchange_requests\x18\x01 \x03(\v2\x12.nem.ChangeRequestR\x0echangeRequests\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"I\n" +
	"\x17GetChangeRequestRequest\x12.\n" +
	"\x13change_request_uuid\x18\x01 \x01(\tR\x11changeRequestUuid\"W\n" +
	"\x1aCreateChangeRequestRequest\x129\n" +
	"\x0echange_request\x18\x01 \x01(\v2\x12.nem.ChangeRequestR\rchangeRequest\"\x94\x01\n" +
	"\x1aUpdateChangeRequestRequest\x129\n" +
	"\x0echange_request\x18\x01 \x01(\v2\x12.nem.ChangeRequestR\rchangeRequest\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\xec\x01\n" +
	"\"ListChangeRequestsForReviewRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12!\n" +
	"\fproject_uuid\x18\x04 \x01(\tR\vprojectUuid\x12L\n" +
	"\x13change_request_type\x18\x05 \x01(\x0e2\x1c.nem.ChangeRequestChangeTypeR\x11changeRequestType\"\x8a\x01\n" +
	"#ListChangeRequestsForReviewResponse\x12;\n" +
	"\x0fchange_requests\x18\x01 \x03(\v2\x12.nem.ChangeRequestR\x0echangeRequests\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"T\n" +
	" GetUserProjectVersionDataRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\"i\n" +
	"!GetUserProjectVersionDataResponse\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12\x12\n" +
	"\x04data\x18\x02 \x01(\tR\x04data\"i\n" +
	"!SaveUserProjectVersionDataRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12\x12\n" +
	"\x04data\x18\x02 \x01(\tR\x04data\"j\n" +
	"\"SaveUserProjectVersionDataResponse\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12\x12\n" +
	"\x04data\x18\x02 \x01(\tR\x04data\"j\n" +
	"\x1fGetObjectStoreWithSecretRequest\x12*\n" +
	"\x11object_store_uuid\x18\x01 \x01(\tR\x0fobjectStoreUuid\x12\x1b\n" +
	"\tteam_uuid\x18\x02 \x01(\tR\bteamUuid\"r\n" +
	"\x1eCreateObjectStoreSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x123\n" +
	"\fobject_store\x18\x02 \x01(\v2\x10.nem.ObjectStoreR\vobjectStore\"r\n" +
	"\x1eUpdateObjectStoreSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x123\n" +
	"\fobject_store\x18\x02 \x01(\v2\x10.nem.ObjectStoreR\vobjectStore\"r\n" +
	"\x1eDeleteObjectStoreSecretRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\x123\n" +
	"\fobject_store\x18\x02 \x01(\v2\x10.nem.ObjectStoreR\vobjectStore\"\xb3\x01\n" +
	"\x17GenerateSQLForCRRequest\x12.\n" +
	"\x13change_request_uuid\x18\x01 \x01(\tR\x11changeRequestUuid\x12'\n" +
	"\x0fconnection_uuid\x18\x02 \x01(\tR\x0econnectionUuid\x12?\n" +
	"\x10db_type_override\x18\x03 \x01(\x0e2\x15.nem.ConnectionDbTypeR\x0edbTypeOverride\"V\n" +
	"\x18GenerateSQLForCRResponse\x12:\n" +
	"\n" +
	"statements\x18\x01 \x03(\v2\x1a.GenerateSQLForCRStatementR\n" +
	"statements\"x\n" +
	"\x19GenerateSQLForCRStatement\x12\x10\n" +
	"\x03sql\x18\x01 \x01(\tR\x03sql\x12)\n" +
	"\x10parametrized_sql\x18\x02 \x01(\tR\x0fparametrizedSql\x12\x1e\n" +
	"\n" +
	"parameters\x18\x03 \x03(\tR\n" +
	"parameters\"\x90\x02\n" +
	"\x17ReviewDataChangeRequest\x120\n" +
	"\x14project_version_uuid\x18\x01 \x01(\tR\x12projectVersionUuid\x12.\n" +
	"\x13change_request_uuid\x18\x02 \x01(\tR\x11changeRequestUuid\x12C\n" +
	"\rreview_status\x18\x03 \x01(\x0e2\x1e.nem.ChangeRequestReviewStatusR\freviewStatus\x12%\n" +
	"\x0ereview_comment\x18\x04 \x01(\tR\rreviewComment\x12'\n" +
	"\x0fpublish_changes\x18\x05 \x01(\bR\x0epublishChanges\"\x1a\n" +
	"\x18ReviewDataChangeResponse\"\x16\n" +
	"\x14GetMembershipRequest\"J\n" +
	"\x17CreateMembershipRequest\x12/\n" +
	"\n" +
	"membership\x18\x01 \x01(\v2\x0f.nem.MembershipR\n" +
	"membership\"\x87\x01\n" +
	"\x17UpdateMembershipRequest\x12/\n" +
	"\n" +
	"membership\x18\x01 \x01(\v2\x0f.nem.MembershipR\n" +
	"membership\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"@\n" +
	"\x1bMembershipForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"7\n" +
	"\x18MembershipForTeamRequest\x12\x1b\n" +
	"\tteam_uuid\x18\x01 \x01(\tR\bteamUuid\"\x89\x01\n" +
	"\x1cCreateCheckoutSessionRequest\x127\n" +
	"\fsession_type\x18\x01 \x01(\x0e2\x14.CheckoutSessionTypeR\vsessionType\x120\n" +
	"\tplan_type\x18\x03 \x01(\x0e2\x13.nem.MembershipTypeR\bplanType\"o\n" +
	"\x1dCreateCheckoutSessionResponse\x12%\n" +
	"\x0esession_secret\x18\x01 \x01(\tR\rsessionSecret\x12'\n" +
	"\x0fmembership_uuid\x18\x02 \x01(\tR\x0emembershipUuid\"V\n" +
	"\x1bChangeMembershipPlanRequest\x127\n" +
	"\rnew_plan_type\x18\x01 \x01(\x0e2\x13.nem.MembershipTypeR\vnewPlanType\"\x1e\n" +
	"\x1cChangeMembershipPlanResponse\"s\n" +
	"\x1aListMembershipUsersRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\"\x97\x01\n" +
	"\x1bListMembershipUsersResponse\x12\x1f\n" +
	"\x05users\x18\x01 \x03(\v2\t.nem.UserR\x05users\x12/\n" +
	"\x13unregistered_emails\x18\x02 \x03(\tR\x12unregisteredEmails\x12&\n" +
	"\x0fnext_page_token\x18\x03 \x01(\tR\rnextPageToken\"O\n" +
	"\x17CancelMembershipRequest\x12\x18\n" +
	"\areasons\x18\x01 \x03(\tR\areasons\x12\x1a\n" +
	"\bcomments\x18\x02 \x01(\tR\bcomments\"\x1a\n" +
	"\x18CancelMembershipResponse\"\x1d\n" +
	"\x1bReactivateMembershipRequest\"\x1e\n" +
	"\x1cReactivateMembershipResponse\"(\n" +
	"&UserHasActiveStripeSubscriptionRequest\"Z\n" +
	"'UserHasActiveStripeSubscriptionResponse\x12/\n" +
	"\x13active_subscription\x18\x01 \x01(\bR\x12activeSubscription\"@\n" +
	"!ListMembershipUserProjectsRequest\x12\x1b\n" +
	"\tuser_uuid\x18\x01 \x01(\tR\buserUuid\"N\n" +
	"\"ListMembershipUserProjectsResponse\x12(\n" +
	"\bprojects\x18\x01 \x03(\v2\f.nem.ProjectR\bprojects\":\n" +
	"\x1bRemoveMembershipUserRequest\x12\x1b\n" +
	"\tuser_uuid\x18\x01 \x01(\tR\buserUuid\"\x1e\n" +
	"\x1cRemoveMembershipUserResponse\"7\n" +
	"\x1dListMembershipsForUserRequest\x12\x16\n" +
	"\x06locale\x18\x01 \x01(\tR\x06locale\"n\n" +
	"\x12MembershipWithName\x12/\n" +
	"\n" +
	"membership\x18\x01 \x01(\v2\x0f.nem.MembershipR\n" +
	"membership\x12'\n" +
	"\x0fmembership_name\x18\x02 \x01(\tR\x0emembershipName\"W\n" +
	"\x1eListMembershipsForUserResponse\x125\n" +
	"\vmemberships\x18\x01 \x03(\v2\x13.MembershipWithNameR\vmemberships\"\x85\x02\n" +
	"\x1bPrioritySupportEmailRequest\x12\x1a\n" +
	"\bcategory\x18\x01 \x01(\tR\bcategory\x12!\n" +
	"\fproject_uuid\x18\x02 \x01(\tR\vprojectUuid\x12\x1b\n" +
	"\tteam_uuid\x18\x03 \x01(\tR\bteamUuid\x12\x18\n" +
	"\amessage\x18\x04 \x01(\tR\amessage\x12(\n" +
	"\x0foperatingSystem\x18\x05 \x01(\tR\x0foperatingSystem\x12\x1e\n" +
	"\n" +
	"appVersion\x18\x06 \x01(\tR\n" +
	"appVersion\x12&\n" +
	"\x0ebrowserVersion\x18\a \x01(\tR\x0ebrowserVersion\"\x1e\n" +
	"\x1cPrioritySupportEmailResponse\"g\n" +
	"\x14HandleWebhookRequest\x12\x16\n" +
	"\x06source\x18\x01 \x01(\tR\x06source\x12\x18\n" +
	"\apayload\x18\x02 \x01(\tR\apayload\x12\x1d\n" +
	"\n" +
	"sig_header\x18\x03 \x01(\tR\tsigHeader\"\x17\n" +
	"\x15HandleWebhookResponse\"A\n" +
	"\x1cIsProActiveForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"l\n" +
	"\x1dIsProActiveForProjectResponse\x12\"\n" +
	"\ris_pro_active\x18\x01 \x01(\bR\visProActive\x12'\n" +
	"\x0fmembership_uuid\x18\x02 \x01(\tR\x0emembershipUuid\"G\n" +
	"\"GetMembershipTierForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"\x8c\x01\n" +
	"#GetMembershipTierForProjectResponse\x12<\n" +
	"\x0fmembership_type\x18\x01 \x01(\x0e2\x13.nem.MembershipTypeR\x0emembershipType\x12'\n" +
	"\x0fmembership_uuid\x18\x02 \x01(\tR\x0emembershipUuid\"\x95\x01\n" +
	"\x19EvaluateUserPromptRequest\x12%\n" +
	"\acontext\x18\x01 \x01(\x0e2\v.ProContextR\acontext\x12\x1f\n" +
	"\vuser_prompt\x18\x02 \x01(\tR\n" +
	"userPrompt\x120\n" +
	"\x14project_version_uuid\x18\x03 \x01(\tR\x12projectVersionUuid\"q\n" +
	"\x1aEvaluateUserPromptResponse\x12#\n" +
	"\ris_actionable\x18\x01 \x01(\bR\fisActionable\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x12\x14\n" +
	"\x05steps\x18\x03 \x03(\tR\x05steps\"\x8a\x02\n" +
	"\x17HandleUserPromptRequest\x12%\n" +
	"\acontext\x18\x01 \x01(\x0e2\v.ProContextR\acontext\x12\x1f\n" +
	"\vuser_prompt\x18\x02 \x01(\tR\n" +
	"userPrompt\x120\n" +
	"\x14project_version_uuid\x18\x03 \x01(\tR\x12projectVersionUuid\x126\n" +
	"\x17project_version_version\x18\x04 \x01(\tR\x15projectVersionVersion\x12\x14\n" +
	"\x05steps\x18\x05 \x03(\tR\x05steps\x12'\n" +
	"\x0fadditional_data\x18\x06 \x01(\tR\x0eadditionalData\"5\n" +
	"\x18HandleUserPromptResponse\x12\x19\n" +
	"\bjob_uuid\x18\x01 \x01(\tR\ajobUuid\"[\n" +
	"\x17GetUserPromptJobRequest\x12%\n" +
	"\acontext\x18\x01 \x01(\x0e2\v.ProContextR\acontext\x12\x19\n" +
	"\bjob_uuid\x18\x02 \x01(\tR\ajobUuid\"\xcd\x01\n" +
	"\x18GetUserPromptJobResponse\x12!\n" +
	"\fcurrent_step\x18\x01 \x01(\tR\vcurrentStep\x12#\n" +
	"\rerror_message\x18\x02 \x01(\tR\ferrorMessage\x12+\n" +
	"\x11generated_content\x18\x03 \x01(\tR\x10generatedContent\x12<\n" +
	"\x0fproject_version\x18\x04 \x01(\v2\x13.nem.ProjectVersionR\x0eprojectVersion\"\xb5\x01\n" +
	"\x1fHandleFollowupUserPromptRequest\x12%\n" +
	"\acontext\x18\x01 \x01(\x0e2\v.ProContextR\acontext\x12\x19\n" +
	"\bjob_uuid\x18\x02 \x01(\tR\ajobUuid\x12'\n" +
	"\x0ffollowup_prompt\x18\x03 \x01(\tR\x0efollowupPrompt\x12'\n" +
	"\x0fadditional_data\x18\x04 \x01(\tR\x0eadditionalData\"=\n" +
	" HandleFollowupUserPromptResponse\x12\x19\n" +
	"\bjob_uuid\x18\x01 \x01(\tR\ajobUuid\"]\n" +
	"\x19FinalizeUserPromptRequest\x12%\n" +
	"\acontext\x18\x01 \x01(\x0e2\v.ProContextR\acontext\x12\x19\n" +
	"\bjob_uuid\x18\x02 \x01(\tR\ajobUuid\"\x1c\n" +
	"\x1aFinalizeUserPromptResponse\"\xd5\x01\n" +
	"\x19ListAIUsageForUserRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12'\n" +
	"\x0fmembership_uuid\x18\x04 \x01(\tR\x0emembershipUuid\x12\x1d\n" +
	"\n" +
	"start_date\x18\x05 \x01(\tR\tstartDate\x12\x19\n" +
	"\bend_date\x18\x06 \x01(\tR\aendDate\"j\n" +
	"\x1aListAIUsageForUserResponse\x12$\n" +
	"\x06usages\x18\x01 \x03(\v2\f.nem.AiUsageR\x06usages\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\x85\x01\n" +
	" GetAIUsageForUserInPeriodRequest\x12'\n" +
	"\x0fmembership_uuid\x18\x01 \x01(\tR\x0emembershipUuid\x12\x1d\n" +
	"\n" +
	"start_date\x18\x02 \x01(\tR\tstartDate\x12\x19\n" +
	"\bend_date\x18\x03 \x01(\tR\aendDate\"\x91\x02\n" +
	"!GetAIUsageForUserInPeriodResponse\x12*\n" +
	"\x11input_tokens_used\x18\x01 \x01(\x03R\x0finputTokensUsed\x12,\n" +
	"\x12output_tokens_used\x18\x02 \x01(\x03R\x10outputTokensUsed\x121\n" +
	"\x14remaining_percentage\x18\x03 \x01(\x02R\x13remainingPercentage\x12\x1d\n" +
	"\n" +
	"start_date\x18\x04 \x01(\tR\tstartDate\x12\x19\n" +
	"\bend_date\x18\x05 \x01(\tR\aendDate\x12%\n" +
	"\x0edays_remaining\x18\x06 \x01(\x05R\rdaysRemaining\"o\n" +
	"\x19RegisterLocalAgentRequest\x12!\n" +
	"\fmachine_name\x18\x01 \x01(\tR\vmachineName\x12\x0e\n" +
	"\x02os\x18\x02 \x01(\tR\x02os\x12\x1f\n" +
	"\vcli_version\x18\x03 \x01(\tR\n" +
	"cliVersion\"r\n" +
	"\x1aRegisterLocalAgentResponse\x12(\n" +
	"\x10local_agent_uuid\x18\x01 \x01(\tR\x0elocalAgentUuid\x12*\n" +
	"\x11local_agent_token\x18\x02 \x01(\tR\x0flocalAgentToken\"C\n" +
	"\x17RevokeLocalAgentRequest\x12(\n" +
	"\x10local_agent_uuid\x18\x01 \x01(\tR\x0elocalAgentUuid\"\x1a\n" +
	"\x18RevokeLocalAgentResponse\"\x18\n" +
	"\x16ListLocalAgentsRequest\"M\n" +
	"\x17ListLocalAgentsResponse\x122\n" +
	"\flocal_agents\x18\x01 \x03(\v2\x0f.nem.LocalAgentR\vlocalAgents\"\x8b\x01\n" +
	"\"UpdateLocalAgentConnectionsRequest\x12(\n" +
	"\x10local_agent_uuid\x18\x01 \x01(\tR\x0elocalAgentUuid\x12;\n" +
	"\vconnections\x18\x02 \x03(\v2\x19.nem.LocalAgentConnectionR\vconnections\"%\n" +
	"#UpdateLocalAgentConnectionsResponse\"K\n" +
	"\x1fGetLocalAgentConnectionsRequest\x12(\n" +
	"\x10local_agent_uuid\x18\x01 \x01(\tR\x0elocalAgentUuid\"_\n" +
	" GetLocalAgentConnectionsResponse\x12;\n" +
	"\vconnections\x18\x01 \x03(\v2\x19.nem.LocalAgentConnectionR\vconnections\"J\n" +
	"\x17UpsertDeploymentRequest\x12/\n" +
	"\n" +
	"deployment\x18\x01 \x01(\v2\x0f.nem.DeploymentR\n" +
	"deployment\"K\n" +
	"\x18UpsertDeploymentResponse\x12/\n" +
	"\n" +
	"deployment\x18\x01 \x01(\v2\x0f.nem.DeploymentR\n" +
	"deployment\"\x18\n" +
	"\x16ListDeploymentsRequest\"L\n" +
	"\x17ListDeploymentsResponse\x121\n" +
	"\vdeployments\x18\x01 \x03(\v2\x0f.nem.DeploymentR\vdeployments\"T\n" +
	"\x1eMarkDeploymentDestroyedRequest\x12\x12\n" +
	"\x04host\x18\x01 \x01(\tR\x04host\x12\x1e\n" +
	"\n" +
	"identifier\x18\x02 \x01(\tR\n" +
	"identifier\"!\n" +
	"\x1fMarkDeploymentDestroyedResponse\"B\n" +
	"\x1dIssueProvisioningTokenRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"n\n" +
	"\x1eIssueProvisioningTokenResponse\x12-\n" +
	"\x12provisioning_token\x18\x01 \x01(\tR\x11provisioningToken\x12\x1d\n" +
	"\n" +
	"expires_at\x18\x02 \x01(\x03R\texpiresAt\"\xa5\x01\n" +
	" ExchangeProvisioningTokenRequest\x12-\n" +
	"\x12provisioning_token\x18\x01 \x01(\tR\x11provisioningToken\x12!\n" +
	"\fmachine_name\x18\x02 \x01(\tR\vmachineName\x12\x0e\n" +
	"\x02os\x18\x03 \x01(\tR\x02os\x12\x1f\n" +
	"\vcli_version\x18\x04 \x01(\tR\n" +
	"cliVersion\"y\n" +
	"!ExchangeProvisioningTokenResponse\x12(\n" +
	"\x10local_agent_uuid\x18\x01 \x01(\tR\x0elocalAgentUuid\x12*\n" +
	"\x11local_agent_token\x18\x02 \x01(\tR\x0flocalAgentToken\"o\n" +
	"#CheckExtensionExecutionLimitRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\x12%\n" +
	"\x0eextension_uuid\x18\x02 \x01(\tR\rextensionUuid\"u\n" +
	"$CheckExtensionExecutionLimitResponse\x12\x1d\n" +
	"\n" +
	"is_limited\x18\x01 \x01(\bR\tisLimited\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x03R\x05limit\x12\x18\n" +
	"\acurrent\x18\x03 \x01(\x03R\acurrent\"J\n" +
	"\x17CreateAutomationRequest\x12/\n" +
	"\n" +
	"automation\x18\x01 \x01(\v2\x0f.nem.AutomationR\n" +
	"automation\"r\n" +
	"\x18CreateAutomationResponse\x12/\n" +
	"\n" +
	"automation\x18\x01 \x01(\v2\x0f.nem.AutomationR\n" +
	"automation\x12%\n" +
	"\x0esigning_secret\x18\x02 \x01(\tR\rsigningSecret\"H\n" +
	"\x1dRotateAutomationSecretRequest\x12'\n" +
	"\x0fautomation_uuid\x18\x01 \x01(\tR\x0eautomationUuid\"G\n" +
	"\x1eRotateAutomationSecretResponse\x12%\n" +
	"\x0esigning_secret\x18\x01 \x01(\tR\rsigningSecret\"J\n" +
	"\x17UpdateAutomationRequest\x12/\n" +
	"\n" +
	"automation\x18\x01 \x01(\v2\x0f.nem.AutomationR\n" +
	"automation\"B\n" +
	"\x17DeleteAutomationRequest\x12'\n" +
	"\x0fautomation_uuid\x18\x01 \x01(\tR\x0eautomationUuid\"\x1a\n" +
	"\x18DeleteAutomationResponse\"?\n" +
	"\x14GetAutomationRequest\x12'\n" +
	"\x0fautomation_uuid\x18\x01 \x01(\tR\x0eautomationUuid\"E\n" +
	" ListAutomationsForProjectRequest\x12!\n" +
	"\fproject_uuid\x18\x01 \x01(\tR\vprojectUuid\"V\n" +
	"!ListAutomationsForProjectResponse\x121\n" +
	"\vautomations\x18\x01 \x03(\v2\x0f.nem.AutomationR\vautomations\"@\n" +
	"\x15TestAutomationRequest\x12'\n" +
	"\x0fautomation_uuid\x18\x01 \x01(\tR\x0eautomationUuid\"~\n" +
	"\x16TestAutomationResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\x12\x1f\n" +
	"\vstatus_code\x18\x02 \x01(\x03R\n" +
	"statusCode\x12)\n" +
	"\x10response_snippet\x18\x03 \x01(\tR\x0fresponseSnippet\"\xb6\x01\n" +
	"\x1bListAutomationEventsRequest\x12'\n" +
	"\x0fautomation_uuid\x18\x01 \x01(\tR\x0eautomationUuid\x122\n" +
	"\x06status\x18\x02 \x01(\x0e2\x1a.nem.AutomationEventStatusR\x06status\x12\x1b\n" +
	"\tpage_size\x18\x03 \x01(\x03R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x04 \x01(\tR\tpageToken\"\x89\x01\n" +
	"\x1cListAutomationEventsResponse\x12A\n" +
	"\x11automation_events\x18\x01 \x03(\v2\x14.nem.AutomationEventR\x10automationEvents\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"O\n" +
	"\x19GetAutomationEventRequest\x122\n" +
	"\x15automation_event_uuid\x18\x01 \x01(\tR\x13automationEventUuid\"Q\n" +
	"\x1bRetryAutomationEventRequest\x122\n" +
	"\x15automation_event_uuid\x18\x01 \x01(\tR\x13automationEventUuid\"Q\n" +
	"\x1fMarkChangeRequestAppliedRequest\x12.\n" +
	"\x13change_request_uuid\x18\x01 \x01(\tR\x11changeRequestUuid\"\x99\x01\n" +
	" MarkChangeRequestAppliedResponse\x129\n" +
	"\x0echange_request\x18\x01 \x01(\v2\x12.nem.ChangeRequestR\rchangeRequest\x12:\n" +
	"\x19automation_events_created\x18\x02 \x01(\x03R\x17automationEventsCreated*}\n" +
	"\x13CheckoutSessionType\x12!\n" +
	"\x1dCHECKOUT_SESSION_TYPE_INVALID\x10\x00\x12!\n" +
	"\x1dCHECKOUT_SESSION_TYPE_MONTHLY\x10\x01\x12 \n" +
	"\x1cCHECKOUT_SESSION_TYPE_YEARLY\x10\x02*[\n" +
	"\n" +
	"ProContext\x12\x17\n" +
	"\x13PRO_CONTEXT_INVALID\x10\x00\x12\x16\n" +
	"\x12PRO_CONTEXT_EDITOR\x10\x01\x12\x1c\n" +
	"\x18PRO_CONTEXT_DATA_MANAGER\x10\x022\x9dS\n" +
	"\fNuzurProduct\x12'\n" +
	"\aGetUser\x12\x0f.GetUserRequest\x1a\t.nem.User\"\x00\x121\n" +
	"\fGetTokenUser\x12\x14.GetTokenUserRequest\x1a\t.nem.User\"\x00\x125\n" +
	"\x0eGetUserByEmail\x12\x16.GetUserByEmailRequest\x1a\t.nem.User\"\x00\x12]\n" +
	"\x1aGetTokenUserRoleForProject\x12\x1d.GetUserRoleForProjectRequest\x1a\x1e.GetUserRoleForProjectResponse\"\x00\x12T\n" +
	"\x17GetTokenUserRoleForTeam\x12\x1a.GetUserRoleForTeamRequest\x1a\x1b.GetUserRoleForTeamResponse\"\x00\x122\n" +
	"\x0fUpdateTokenUser\x12\x12.UpdateUserRequest\x1a\t.nem.User\"\x00\x12I\n" +
	"\x10ListUsersForTeam\x12\x18.ListUsersForTeamRequest\x1a\x19.ListUsersForTeamResponse\"\x00\x12R\n" +
	"\x13ListUsersForProject\x12\x1b.ListUsersForProjectRequest\x1a\x1c.ListUsersForProjectResponse\"\x00\x12@\n" +
	"\x10AddUserToProject\x12\x18.AddUserToProjectRequest\x1a\x10.nem.UserProject\"\x00\x12B\n" +
	"\x11UpdateUserProject\x12\x19.UpdateUserProjectRequest\x1a\x10.nem.UserProject\"\x00\x12J\n" +
	"\x15RemoveUserFromProject\x12\x1d.RemoveUserFromProjectRequest\x1a\x10.nem.UserProject\"\x00\x127\n" +
	"\rAddUserToTeam\x12\x15.AddUserToTeamRequest\x1a\r.nem.UserTeam\"\x00\x129\n" +
	"\x0eUpdateUserTeam\x12\x16.UpdateUserTeamRequest\x1a\r.nem.UserTeam\"\x00\x12A\n" +
	"\x12RemoveUserFromTeam\x12\x1a.RemoveUserFromTeamRequest\x1a\r.nem.UserTeam\"\x00\x12I\n" +
	"\x10ListTeamsForUser\x12\x18.ListTeamsForUserRequest\x1a\x19.ListTeamsForUserResponse\"\x00\x12X\n" +
	"\x15ListTeamsForAdminUser\x12\x1d.ListTeamsForAdminUserRequest\x1a\x1e.ListTeamsForAdminUserResponse\"\x00\x125\n" +
	"\x0eGetTeamForUser\x12\x16.GetTeamForUserRequest\x1a\t.nem.Team\"\x00\x12-\n" +
	"\n" +
	"CreateTeam\x12\x12.CreateTeamRequest\x1a\t.nem.Team\"\x00\x12-\n" +
	"\n" +
	"UpdateTeam\x12\x12.UpdateTeamRequest\x1a\t.nem.Team\"\x00\x12M\n" +
	"\x17GetConnectionWithSecret\x12\x1f.GetConnectionWithSecretRequest\x1a\x0f.nem.Connection\"\x00\x12K\n" +
	"\x16CreateConnectionSecret\x12\x1e.CreateConnectionSecretRequest\x1a\x0f.nem.Connection\"\x00\x12K\n" +
	"\x16UpdateConnectionSecret\x12\x1e.UpdateConnectionSecretRequest\x1a\x0f.nem.Connection\"\x00\x12K\n" +
	"\x16DeleteConnectionSecret\x12\x1e.DeleteConnectionSecretRequest\x1a\x0f.nem.Connection\"\x00\x12R\n" +
	"\x13ListProjectsForUser\x12\x1b.ListProjectsForUserRequest\x1a\x1c.ListProjectsForUserResponse\"\x00\x12>\n" +
	"\x11GetProjectForUser\x12\x19.GetProjectForUserRequest\x1a\f.nem.Project\"\x00\x126\n" +
	"\rCreateProject\x12\x15.CreateProjectRequest\x1a\f.nem.Project\"\x00\x126\n" +
	"\rUpdateProject\x12\x15.UpdateProjectRequest\x1a\f.nem.Project\"\x00\x12C\n" +
	"\x0eGetProjectName\x12\x16.GetProjectNameRequest\x1a\x17.GetProjectNameResponse\"\x00\x12R\n" +
	"\x13ListProjectVersions\x12\x1b.ListProjectVersionsRequest\x1a\x1c.ListProjectVersionsResponse\"\x00\x12g\n" +
	"\x1aListProjectVersionsForUser\x12\".ListProjectVersionsForUserRequest\x1a#.ListProjectVersionsForUserResponse\"\x00\x12m\n" +
	" ListProjectVersionsForUserCached\x12\".ListProjectVersionsForUserRequest\x1a#.ListProjectVersionsForUserResponse\"\x00\x12S\n" +
	"\x18GetProjectVersionForUser\x12 .GetProjectVersionForUserRequest\x1a\x13.nem.ProjectVersion\"\x00\x12Y\n" +
	"\x1eGetProjectVersionForUserCached\x12 .GetProjectVersionForUserRequest\x1a\x13.nem.ProjectVersion\"\x00\x12W\n" +
	"\x1cGetProjectVersionLeanForUser\x12 .GetProjectVersionForUserRequest\x1a\x13.nem.ProjectVersion\"\x00\x12Q\n" +
	"\x17GetLatestProjectVersion\x12\x1f.GetLatestProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12_\n" +
	"\x1eGetLatestProjectVersionForUser\x12&.GetLatestProjectVersionForUserRequest\x1a\x13.nem.ProjectVersion\"\x00\x12c\n" +
	"\"GetLatestProjectVersionLeanForUser\x12&.GetLatestProjectVersionForUserRequest\x1a\x13.nem.ProjectVersion\"\x00\x12\x7f\n" +
	"\"GetLatestProjectVersionUUIDForUser\x12*.GetLatestProjectVersionUUIDForUserRequest\x1a+.GetLatestProjectVersionUUIDForUserResponse\"\x00\x12a\n" +
	"\x18GetProjectVersionVersion\x12 .GetProjectVersionVersionRequest\x1a!.GetProjectVersionVersionResponse\"\x00\x12j\n" +
	"\x1bGetProjectVersionIdentifier\x12#.GetProjectVersionIdentifierRequest\x1a$.GetProjectVersionIdentifierResponse\"\x00\x12K\n" +
	"\x14CreateProjectVersion\x12\x1c.CreateProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12[\n" +
	"\x1cCreateProjectVersionFromJSON\x12$.CreateProjectVersionFromJSONRequest\x1a\x13.nem.ProjectVersion\"\x00\x12y\n" +
	"+CreateDraftProjectVersionForExistingProject\x123.CreateDraftProjectVersionForExistingProjectRequest\x1a\x13.nem.ProjectVersion\"\x00\x12K\n" +
	"\x14UpdateProjectVersion\x12\x1c.UpdateProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12W\n" +
	"\x1aDiscardDraftProjectVersion\x12\".DiscardDraftProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12Y\n" +
	"\x1bSendProjectVersionForReview\x12#.SendProjectVersionForReviewRequest\x1a\x13.nem.ProjectVersion\"\x00\x12c\n" +
	" WithdrawFromReviewProjectVersion\x12(.WithdrawFromReviewProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12K\n" +
	"\x14ReviewProjectVersion\x12\x1c.ReviewProjectVersionRequest\x1a\x13.nem.ProjectVersion\"\x00\x12m\n" +
	"\x1cUploadProjectVersionSnapshot\x12$.UploadProjectVersionSnapshotRequest\x1a%.UploadProjectVersionSnapshotResponse\"\x00\x12I\n" +
	"\x10GetSignedFileURL\x12\x18.GetSignedFileURLRequest\x1a\x19.GetSignedFileURLResponse\"\x00\x12m\n" +
	"\x1cUploadExtensionExecutionFile\x12$.UploadExtensionExecutionFileRequest\x1a%.UploadExtensionExecutionFileResponse\"\x00\x12d\n" +
	"\x19GetExtensionExecutionFile\x12!.GetExtensionExecutionFileRequest\x1a\".GetExtensionExecutionFileResponse\"\x00\x12R\n" +
	"\x13UploadExtensionIcon\x12\x1b.UploadExtensionIconRequest\x1a\x1c.UploadExtensionIconResponse\"\x00\x12I\n" +
	"\x10UploadUserAvatar\x12\x18.UploadUserAvatarRequest\x1a\x19.UploadUserAvatarResponse\"\x00\x12X\n" +
	"\x15UploadRecordFieldFile\x12\x1d.UploadRecordFieldFileRequest\x1a\x1e.UploadRecordFieldFileResponse\"\x00\x12j\n" +
	"\x1bGetRecordFieldSignedFileURL\x12#.GetRecordFieldSignedFileURLRequest\x1a$.GetRecordFieldSignedFileURLResponse\"\x00\x12U\n" +
	"\x14GetRecordFileContent\x12\x1c.GetRecordFileContentRequest\x1a\x1d.GetRecordFileContentResponse\"\x00\x12C\n" +
	"\x0eListExtensions\x12\x16.ListExtensionsRequest\x1a\x17.ListExtensionsResponse\"\x00\x126\n" +
	"\fGetExtension\x12\x14.GetExtensionRequest\x1a\x0e.nem.Extension\"\x00\x12<\n" +
	"\x0fCreateExtension\x12\x17.CreateExtensionRequest\x1a\x0e.nem.Extension\"\x00\x12<\n" +
	"\x0fUpdateExtension\x12\x17.UpdateExtensionRequest\x1a\x0e.nem.Extension\"\x00\x12X\n" +
	"\x15ListExtensionVersions\x12\x1d.ListExtensionVersionsRequest\x1a\x1e.ListExtensionVersionsResponse\"\x00\x12K\n" +
	"\x13GetExtensionVersion\x12\x1b.GetExtensionVersionRequest\x1a\x15.nem.ExtensionVersion\"\x00\x12Q\n" +
	"\x16CreateExtensionVersion\x12\x1e.CreateExtensionVersionRequest\x1a\x15.nem.ExtensionVersion\"\x00\x12Q\n" +
	"\x16UpdateExtensionVersion\x12\x1e.UpdateExtensionVersionRequest\x1a\x15.nem.ExtensionVersion\"\x00\x12^\n" +
	"\x17ListExtensionExecutions\x12\x1f.ListExtensionExecutionsRequest\x1a .ListExtensionExecutionsResponse\"\x00\x12Q\n" +
	"\x15GetExtensionExecution\x12\x1d.GetExtensionExecutionRequest\x1a\x17.nem.ExtensionExecution\"\x00\x12W\n" +
	"\x18CreateExtensionExecution\x12 .CreateExtensionExecutionRequest\x1a\x17.nem.ExtensionExecution\"\x00\x12W\n" +
	"\x18UpdateExtensionExecution\x12 .UpdateExtensionExecutionRequest\x1a\x17.nem.ExtensionExecution\"\x00\x12m\n" +
	"\x1cCheckExtensionExecutionLimit\x12$.CheckExtensionExecutionLimitRequest\x1a%.CheckExtensionExecutionLimitResponse\"\x00\x12[\n" +
	"\x16ListUserChangeRequests\x12\x1e.ListUserChangeRequestsRequest\x1a\x1f.ListUserChangeRequestsResponse\"\x00\x12B\n" +
	"\x10GetChangeRequest\x12\x18.GetChangeRequestRequest\x1a\x12.nem.ChangeRequest\"\x00\x12H\n" +
	"\x13CreateChangeRequest\x12\x1b.CreateChangeRequestRequest\x1a\x12.nem.ChangeRequest\"\x00\x12H\n" +
	"\x13UpdateChangeRequest\x12\x1b.UpdateChangeRequestRequest\x1a\x12.nem.ChangeRequest\"\x00\x12j\n" +
	"\x1bListChangeRequestsForReview\x12#.ListChangeRequestsForReviewRequest\x1a$.ListChangeRequestsForReviewResponse\"\x00\x12d\n" +
	"\x19GetUserProjectVersionData\x12!.GetUserProjectVersionDataRequest\x1a\".GetUserProjectVersionDataResponse\"\x00\x12g\n" +
	"\x1aSaveUserProjectVersionData\x12\".SaveUserProjectVersionDataRequest\x1a#.SaveUserProjectVersionDataResponse\"\x00\x12P\n" +
	"\x18GetObjectStoreWithSecret\x12 .GetObjectStoreWithSecretRequest\x1a\x10.nem.ObjectStore\"\x00\x12N\n" +
	"\x17CreateObjectStoreSecret\x12\x1f.CreateObjectStoreSecretRequest\x1a\x10.nem.ObjectStore\"\x00\x12N\n" +
	"\x17UpdateObjectStoreSecret\x12\x1f.UpdateObjectStoreSecretRequest\x1a\x10.nem.ObjectStore\"\x00\x12N\n" +
	"\x17DeleteObjectStoreSecret\x12\x1f.DeleteObjectStoreSecretRequest\x1a\x10.nem.ObjectStore\"\x00\x12I\n" +
	"\x10GenerateSQLForCR\x12\x18.GenerateSQLForCRRequest\x1a\x19.GenerateSQLForCRResponse\"\x00\x12I\n" +
	"\x10ReviewDataChange\x12\x18.ReviewDataChangeRequest\x1a\x19.ReviewDataChangeResponse\"\x00\x129\n" +
	"\rGetMembership\x12\x15.GetMembershipRequest\x1a\x0f.nem.Membership\"\x00\x12?\n" +
	"\x10CreateMembership\x12\x18.CreateMembershipRequest\x1a\x0f.nem.Membership\"\x00\x12?\n" +
	"\x10UpdateMembership\x12\x18.UpdateMembershipRequest\x1a\x0f.nem.Membership\"\x00\x12G\n" +
	"\x14MembershipForProject\x12\x1c.MembershipForProjectRequest\x1a\x0f.nem.Membership\"\x00\x12A\n" +
	"\x11MembershipForTeam\x12\x19.MembershipForTeamRequest\x1a\x0f.nem.Membership\"\x00\x12X\n" +
	"\x15CreateCheckoutSession\x12\x1d.CreateCheckoutSessionRequest\x1a\x1e.CreateCheckoutSessionResponse\"\x00\x12R\n" +
	"\x13ListMembershipUsers\x12\x1b.ListMembershipUsersRequest\x1a\x1c.ListMembershipUsersResponse\"\x00\x12I\n" +
	"\x10CancelMembership\x12\x18.CancelMembershipRequest\x1a\x19.CancelMembershipResponse\"\x00\x12U\n" +
	"\x14ReactivateMembership\x12\x1c.ReactivateMembershipRequest\x1a\x1d.ReactivateMembershipResponse\"\x00\x12v\n" +
	"\x1fUserHasActiveStripeSubscription\x12'.UserHasActiveStripeSubscriptionRequest\x1a(.UserHasActiveStripeSubscriptionResponse\"\x00\x12g\n" +
	"\x1aListMembershipUserProjects\x12\".ListMembershipUserProjectsRequest\x1a#.ListMembershipUserProjectsResponse\"\x00\x12U\n" +
	"\x14RemoveMembershipUser\x12\x1c.RemoveMembershipUserRequest\x1a\x1d.RemoveMembershipUserResponse\"\x00\x12[\n" +
	"\x16ListMembershipsForUser\x12\x1e.ListMembershipsForUserRequest\x1a\x1f.ListMembershipsForUserResponse\"\x00\x12U\n" +
	"\x14ChangeMembershipPlan\x12\x1c.ChangeMembershipPlanRequest\x1a\x1d.ChangeMembershipPlanResponse\"\x00\x12Y\n" +
	"\x18SendPrioritySupportEmail\x12\x1c.PrioritySupportEmailRequest\x1a\x1d.PrioritySupportEmailResponse\"\x00\x12@\n" +
	"\rHandleWebhook\x12\x15.HandleWebhookRequest\x1a\x16.HandleWebhookResponse\"\x00\x12X\n" +
	"\x15IsProActiveForProject\x12\x1d.IsProActiveForProjectRequest\x1a\x1e.IsProActiveForProjectResponse\"\x00\x12j\n" +
	"\x1bGetMembershipTierForProject\x12#.GetMembershipTierForProjectRequest\x1a$.GetMembershipTierForProjectResponse\"\x00\x12O\n" +
	"\x12EvaluateUserPrompt\x12\x1a.EvaluateUserPromptRequest\x1a\x1b.EvaluateUserPromptResponse\"\x00\x12I\n" +
	"\x10HandleUserPrompt\x12\x18.HandleUserPromptRequest\x1a\x19.HandleUserPromptResponse\"\x00\x12I\n" +
	"\x10GetUserPromptJob\x12\x18.GetUserPromptJobRequest\x1a\x19.GetUserPromptJobResponse\"\x00\x12a\n" +
	"\x18HandleFollowupUserPrompt\x12 .HandleFollowupUserPromptRequest\x1a!.HandleFollowupUserPromptResponse\"\x00\x12O\n" +
	"\x12FinalizeUserPrompt\x12\x1a.FinalizeUserPromptRequest\x1a\x1b.FinalizeUserPromptResponse\"\x00\x12O\n" +
	"\x12ListAIUsageForUser\x12\x1a.ListAIUsageForUserRequest\x1a\x1b.ListAIUsageForUserResponse\"\x00\x12d\n" +
	"\x19GetAIUsageForUserInPeriod\x12!.GetAIUsageForUserInPeriodRequest\x1a\".GetAIUsageForUserInPeriodResponse\"\x00\x12O\n" +
	"\x12RegisterLocalAgent\x12\x1a.RegisterLocalAgentRequest\x1a\x1b.RegisterLocalAgentResponse\"\x00\x12I\n" +
	"\x10RevokeLocalAgent\x12\x18.RevokeLocalAgentRequest\x1a\x19.RevokeLocalAgentResponse\"\x00\x12F\n" +
	"\x0fListLocalAgents\x12\x17.ListLocalAgentsRequest\x1a\x18.ListLocalAgentsResponse\"\x00\x12j\n" +
	"\x1bUpdateLocalAgentConnections\x12#.UpdateLocalAgentConnectionsRequest\x1a$.UpdateLocalAgentConnectionsResponse\"\x00\x12a\n" +
	"\x18GetLocalAgentConnections\x12 .GetLocalAgentConnectionsRequest\x1a!.GetLocalAgentConnectionsResponse\"\x00\x12[\n" +
	"\x16IssueProvisioningToken\x12\x1e.IssueProvisioningTokenRequest\x1a\x1f.IssueProvisioningTokenResponse\"\x00\x12d\n" +
	"\x19ExchangeProvisioningToken\x12!.ExchangeProvisioningTokenRequest\x1a\".ExchangeProvisioningTokenResponse\"\x00\x12I\n" +
	"\x10UpsertDeployment\x12\x18.UpsertDeploymentRequest\x1a\x19.UpsertDeploymentResponse\"\x00\x12F\n" +
	"\x0fListDeployments\x12\x17.ListDeploymentsRequest\x1a\x18.ListDeploymentsResponse\"\x00\x12^\n" +
	"\x17MarkDeploymentDestroyed\x12\x1f.MarkDeploymentDestroyedRequest\x1a .MarkDeploymentDestroyedResponse\"\x00\x12I\n" +
	"\x10CreateAutomation\x12\x18.CreateAutomationRequest\x1a\x19.CreateAutomationResponse\"\x00\x12[\n" +
	"\x16RotateAutomationSecret\x12\x1e.RotateAutomationSecretRequest\x1a\x1f.RotateAutomationSecretResponse\"\x00\x12?\n" +
	"\x10UpdateAutomation\x12\x18.UpdateAutomationRequest\x1a\x0f.nem.Automation\"\x00\x12I\n" +
	"\x10DeleteAutomation\x12\x18.DeleteAutomationRequest\x1a\x19.DeleteAutomationResponse\"\x00\x129\n" +
	"\rGetAutomation\x12\x15.GetAutomationRequest\x1a\x0f.nem.Automation\"\x00\x12d\n" +
	"\x19ListAutomationsForProject\x12!.ListAutomationsForProjectRequest\x1a\".ListAutomationsForProjectResponse\"\x00\x12C\n" +
	"\x0eTestAutomation\x12\x16.TestAutomationRequest\x1a\x17.TestAutomationResponse\"\x00\x12U\n" +
	"\x14ListAutomationEvents\x12\x1c.ListAutomationEventsRequest\x1a\x1d.ListAutomationEventsResponse\"\x00\x12H\n" +
	"\x12GetAutomationEvent\x12\x1a.GetAutomationEventRequest\x1a\x14.nem.AutomationEvent\"\x00\x12L\n" +
	"\x14RetryAutomationEvent\x12\x1c.RetryAutomationEventRequest\x1a\x14.nem.AutomationEvent\"\x00\x12a\n" +
	"\x18MarkChangeRequestApplied\x12 .MarkChangeRequestAppliedRequest\x1a!.MarkChangeRequestAppliedResponse\"\x00B\x1d\n" +
	"\x05nuzurB\fNuzurProductP\x01Z\x04gen/b\x06proto3"

var (
	file_product_proto_rawDescOnce sync.Once
	file_product_proto_rawDescData []byte
)

func file_product_proto_rawDescGZIP() []byte {
	file_product_proto_rawDescOnce.Do(func() {
		file_product_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_product_proto_rawDesc), len(file_product_proto_rawDesc)))
	})
	return file_product_proto_rawDescData
}

var file_product_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_product_proto_msgTypes = make([]protoimpl.MessageInfo, 198)
var file_product_proto_goTypes = []any{
	(CheckoutSessionType)(0),                                   // 0: CheckoutSessionType
	(ProContext)(0),                                            // 1: ProContext
	(*GetUserRequest)(nil),                                     // 2: GetUserRequest
	(*GetTokenUserRequest)(nil),                                // 3: GetTokenUserRequest
	(*GetUserByEmailRequest)(nil),                              // 4: GetUserByEmailRequest
	(*GetUserRoleForProjectRequest)(nil),                       // 5: GetUserRoleForProjectRequest
	(*GetUserRoleForProjectResponse)(nil),                      // 6: GetUserRoleForProjectResponse
	(*GetUserRoleForTeamRequest)(nil),                          // 7: GetUserRoleForTeamRequest
	(*GetUserRoleForTeamResponse)(nil),                         // 8: GetUserRoleForTeamResponse
	(*UpdateUserRequest)(nil),                                  // 9: UpdateUserRequest
	(*ListUsersForTeamRequest)(nil),                            // 10: ListUsersForTeamRequest
	(*ListUsersForTeamResponse)(nil),                           // 11: ListUsersForTeamResponse
	(*ListUsersForProjectRequest)(nil),                         // 12: ListUsersForProjectRequest
	(*ListUsersForProjectResponse)(nil),                        // 13: ListUsersForProjectResponse
	(*AddUserToProjectRequest)(nil),                            // 14: AddUserToProjectRequest
	(*UpdateUserProjectRequest)(nil),                           // 15: UpdateUserProjectRequest
	(*RemoveUserFromProjectRequest)(nil),                       // 16: RemoveUserFromProjectRequest
	(*AddUserToTeamRequest)(nil),                               // 17: AddUserToTeamRequest
	(*UpdateUserTeamRequest)(nil),                              // 18: UpdateUserTeamRequest
	(*RemoveUserFromTeamRequest)(nil),                          // 19: RemoveUserFromTeamRequest
	(*ListTeamsForUserRequest)(nil),                            // 20: ListTeamsForUserRequest
	(*ListTeamsForUserResponse)(nil),                           // 21: ListTeamsForUserResponse
	(*ListTeamsForAdminUserRequest)(nil),                       // 22: ListTeamsForAdminUserRequest
	(*ListTeamsForAdminUserResponse)(nil),                      // 23: ListTeamsForAdminUserResponse
	(*GetTeamForUserRequest)(nil),                              // 24: GetTeamForUserRequest
	(*CreateTeamRequest)(nil),                                  // 25: CreateTeamRequest
	(*UpdateTeamRequest)(nil),                                  // 26: UpdateTeamRequest
	(*GetConnectionWithSecretRequest)(nil),                     // 27: GetConnectionWithSecretRequest
	(*CreateConnectionSecretRequest)(nil),                      // 28: CreateConnectionSecretRequest
	(*UpdateConnectionSecretRequest)(nil),                      // 29: UpdateConnectionSecretRequest
	(*DeleteConnectionSecretRequest)(nil),                      // 30: DeleteConnectionSecretRequest
	(*ListProjectsForUserRequest)(nil),                         // 31: ListProjectsForUserRequest
	(*ListProjectsForUserResponse)(nil),                        // 32: ListProjectsForUserResponse
	(*GetProjectForUserRequest)(nil),                           // 33: GetProjectForUserRequest
	(*CreateProjectRequest)(nil),                               // 34: CreateProjectRequest
	(*UpdateProjectRequest)(nil),                               // 35: UpdateProjectRequest
	(*GetProjectNameRequest)(nil),                              // 36: GetProjectNameRequest
	(*GetProjectNameResponse)(nil),                             // 37: GetProjectNameResponse
	(*ListProjectVersionsRequest)(nil),                         // 38: ListProjectVersionsRequest
	(*ListProjectVersionsResponse)(nil),                        // 39: ListProjectVersionsResponse
	(*ListProjectVersionsForUserRequest)(nil),                  // 40: ListProjectVersionsForUserRequest
	(*ListProjectVersionsForUserResponse)(nil),                 // 41: ListProjectVersionsForUserResponse
	(*GetProjectVersionForUserRequest)(nil),                    // 42: GetProjectVersionForUserRequest
	(*GetLatestProjectVersionRequest)(nil),                     // 43: GetLatestProjectVersionRequest
	(*GetLatestProjectVersionForUserRequest)(nil),              // 44: GetLatestProjectVersionForUserRequest
	(*GetLatestProjectVersionUUIDForUserRequest)(nil),          // 45: GetLatestProjectVersionUUIDForUserRequest
	(*GetLatestProjectVersionUUIDForUserResponse)(nil),         // 46: GetLatestProjectVersionUUIDForUserResponse
	(*GetProjectVersionVersionRequest)(nil),                    // 47: GetProjectVersionVersionRequest
	(*GetProjectVersionVersionResponse)(nil),                   // 48: GetProjectVersionVersionResponse
	(*GetProjectVersionIdentifierRequest)(nil),                 // 49: GetProjectVersionIdentifierRequest
	(*GetProjectVersionIdentifierResponse)(nil),                // 50: GetProjectVersionIdentifierResponse
	(*CreateProjectVersionRequest)(nil),                        // 51: CreateProjectVersionRequest
	(*CreateProjectVersionFromJSONRequest)(nil),                // 52: CreateProjectVersionFromJSONRequest
	(*UpdateProjectVersionRequest)(nil),                        // 53: UpdateProjectVersionRequest
	(*CreateDraftProjectVersionForExistingProjectRequest)(nil), // 54: CreateDraftProjectVersionForExistingProjectRequest
	(*DiscardDraftProjectVersionRequest)(nil),                  // 55: DiscardDraftProjectVersionRequest
	(*SendProjectVersionForReviewRequest)(nil),                 // 56: SendProjectVersionForReviewRequest
	(*WithdrawFromReviewProjectVersionRequest)(nil),            // 57: WithdrawFromReviewProjectVersionRequest
	(*ReviewProjectVersionRequest)(nil),                        // 58: ReviewProjectVersionRequest
	(*UploadProjectVersionSnapshotRequest)(nil),                // 59: UploadProjectVersionSnapshotRequest
	(*UploadProjectVersionSnapshotResponse)(nil),               // 60: UploadProjectVersionSnapshotResponse
	(*GetSignedFileURLRequest)(nil),                            // 61: GetSignedFileURLRequest
	(*GetSignedFileURLResponse)(nil),                           // 62: GetSignedFileURLResponse
	(*UploadExtensionExecutionFileRequest)(nil),                // 63: UploadExtensionExecutionFileRequest
	(*UploadExtensionExecutionFileResponse)(nil),               // 64: UploadExtensionExecutionFileResponse
	(*GetExtensionExecutionFileRequest)(nil),                   // 65: GetExtensionExecutionFileRequest
	(*GetExtensionExecutionFileResponse)(nil),                  // 66: GetExtensionExecutionFileResponse
	(*UploadExtensionIconRequest)(nil),                         // 67: UploadExtensionIconRequest
	(*UploadExtensionIconResponse)(nil),                        // 68: UploadExtensionIconResponse
	(*UploadDataManagerMetadataRequest)(nil),                   // 69: UploadDataManagerMetadataRequest
	(*UploadDataManagerMetadataResponse)(nil),                  // 70: UploadDataManagerMetadataResponse
	(*UploadRecordFieldFileRequest)(nil),                       // 71: UploadRecordFieldFileRequest
	(*UploadRecordFieldFileResponse)(nil),                      // 72: UploadRecordFieldFileResponse
	(*GetRecordFieldSignedFileURLRequest)(nil),                 // 73: GetRecordFieldSignedFileURLRequest
	(*GetRecordFieldSignedFileURLResponse)(nil),                // 74: GetRecordFieldSignedFileURLResponse
	(*GetRecordFileContentRequest)(nil),                        // 75: GetRecordFileContentRequest
	(*GetRecordFileContentResponse)(nil),                       // 76: GetRecordFileContentResponse
	(*UploadUserAvatarRequest)(nil),                            // 77: UploadUserAvatarRequest
	(*UploadUserAvatarResponse)(nil),                           // 78: UploadUserAvatarResponse
	(*ListExtensionsRequest)(nil),                              // 79: ListExtensionsRequest
	(*ListExtensionsResponse)(nil),                             // 80: ListExtensionsResponse
	(*GetExtensionRequest)(nil),                                // 81: GetExtensionRequest
	(*CreateExtensionRequest)(nil),                             // 82: CreateExtensionRequest
	(*UpdateExtensionRequest)(nil),                             // 83: UpdateExtensionRequest
	(*ListExtensionVersionsRequest)(nil),                       // 84: ListExtensionVersionsRequest
	(*ListExtensionVersionsResponse)(nil),                      // 85: ListExtensionVersionsResponse
	(*GetExtensionVersionRequest)(nil),                         // 86: GetExtensionVersionRequest
	(*CreateExtensionVersionRequest)(nil),                      // 87: CreateExtensionVersionRequest
	(*UpdateExtensionVersionRequest)(nil),                      // 88: UpdateExtensionVersionRequest
	(*ListExtensionExecutionsRequest)(nil),                     // 89: ListExtensionExecutionsRequest
	(*ListExtensionExecutionsResponse)(nil),                    // 90: ListExtensionExecutionsResponse
	(*GetExtensionExecutionRequest)(nil),                       // 91: GetExtensionExecutionRequest
	(*CreateExtensionExecutionRequest)(nil),                    // 92: CreateExtensionExecutionRequest
	(*UpdateExtensionExecutionRequest)(nil),                    // 93: UpdateExtensionExecutionRequest
	(*ListUserChangeRequestsRequest)(nil),                      // 94: ListUserChangeRequestsRequest
	(*ListUserChangeRequestsResponse)(nil),                     // 95: ListUserChangeRequestsResponse
	(*GetChangeRequestRequest)(nil),                            // 96: GetChangeRequestRequest
	(*CreateChangeRequestRequest)(nil),                         // 97: CreateChangeRequestRequest
	(*UpdateChangeRequestRequest)(nil),                         // 98: UpdateChangeRequestRequest
	(*ListChangeRequestsForReviewRequest)(nil),                 // 99: ListChangeRequestsForReviewRequest
	(*ListChangeRequestsForReviewResponse)(nil),                // 100: ListChangeRequestsForReviewResponse
	(*GetUserProjectVersionDataRequest)(nil),                   // 101: GetUserProjectVersionDataRequest
	(*GetUserProjectVersionDataResponse)(nil),                  // 102: GetUserProjectVersionDataResponse
	(*SaveUserProjectVersionDataRequest)(nil),                  // 103: SaveUserProjectVersionDataRequest
	(*SaveUserProjectVersionDataResponse)(nil),                 // 104: SaveUserProjectVersionDataResponse
	(*GetObjectStoreWithSecretRequest)(nil),                    // 105: GetObjectStoreWithSecretRequest
	(*CreateObjectStoreSecretRequest)(nil),                     // 106: CreateObjectStoreSecretRequest
	(*UpdateObjectStoreSecretRequest)(nil),                     // 107: UpdateObjectStoreSecretRequest
	(*DeleteObjectStoreSecretRequest)(nil),                     // 108: DeleteObjectStoreSecretRequest
	(*GenerateSQLForCRRequest)(nil),                            // 109: GenerateSQLForCRRequest
	(*GenerateSQLForCRResponse)(nil),                           // 110: GenerateSQLForCRResponse
	(*GenerateSQLForCRStatement)(nil),                          // 111: GenerateSQLForCRStatement
	(*ReviewDataChangeRequest)(nil),                            // 112: ReviewDataChangeRequest
	(*ReviewDataChangeResponse)(nil),                           // 113: ReviewDataChangeResponse
	(*GetMembershipRequest)(nil),                               // 114: GetMembershipRequest
	(*CreateMembershipRequest)(nil),                            // 115: CreateMembershipRequest
	(*UpdateMembershipRequest)(nil),                            // 116: UpdateMembershipRequest
	(*MembershipForProjectRequest)(nil),                        // 117: MembershipForProjectRequest
	(*MembershipForTeamRequest)(nil),                           // 118: MembershipForTeamRequest
	(*CreateCheckoutSessionRequest)(nil),                       // 119: CreateCheckoutSessionRequest
	(*CreateCheckoutSessionResponse)(nil),                      // 120: CreateCheckoutSessionResponse
	(*ChangeMembershipPlanRequest)(nil),                        // 121: ChangeMembershipPlanRequest
	(*ChangeMembershipPlanResponse)(nil),                       // 122: ChangeMembershipPlanResponse
	(*ListMembershipUsersRequest)(nil),                         // 123: ListMembershipUsersRequest
	(*ListMembershipUsersResponse)(nil),                        // 124: ListMembershipUsersResponse
	(*CancelMembershipRequest)(nil),                            // 125: CancelMembershipRequest
	(*CancelMembershipResponse)(nil),                           // 126: CancelMembershipResponse
	(*ReactivateMembershipRequest)(nil),                        // 127: ReactivateMembershipRequest
	(*ReactivateMembershipResponse)(nil),                       // 128: ReactivateMembershipResponse
	(*UserHasActiveStripeSubscriptionRequest)(nil),             // 129: UserHasActiveStripeSubscriptionRequest
	(*UserHasActiveStripeSubscriptionResponse)(nil),            // 130: UserHasActiveStripeSubscriptionResponse
	(*ListMembershipUserProjectsRequest)(nil),                  // 131: ListMembershipUserProjectsRequest
	(*ListMembershipUserProjectsResponse)(nil),                 // 132: ListMembershipUserProjectsResponse
	(*RemoveMembershipUserRequest)(nil),                        // 133: RemoveMembershipUserRequest
	(*RemoveMembershipUserResponse)(nil),                       // 134: RemoveMembershipUserResponse
	(*ListMembershipsForUserRequest)(nil),                      // 135: ListMembershipsForUserRequest
	(*MembershipWithName)(nil),                                 // 136: MembershipWithName
	(*ListMembershipsForUserResponse)(nil),                     // 137: ListMembershipsForUserResponse
	(*PrioritySupportEmailRequest)(nil),                        // 138: PrioritySupportEmailRequest
	(*PrioritySupportEmailResponse)(nil),                       // 139: PrioritySupportEmailResponse
	(*HandleWebhookRequest)(nil),                               // 140: HandleWebhookRequest
	(*HandleWebhookResponse)(nil),                              // 141: HandleWebhookResponse
	(*IsProActiveForProjectRequest)(nil),                       // 142: IsProActiveForProjectRequest
	(*IsProActiveForProjectResponse)(nil),                      // 143: IsProActiveForProjectResponse
	(*GetMembershipTierForProjectRequest)(nil),                 // 144: GetMembershipTierForProjectRequest
	(*GetMembershipTierForProjectResponse)(nil),                // 145: GetMembershipTierForProjectResponse
	(*EvaluateUserPromptRequest)(nil),                          // 146: EvaluateUserPromptRequest
	(*EvaluateUserPromptResponse)(nil),                         // 147: EvaluateUserPromptResponse
	(*HandleUserPromptRequest)(nil),                            // 148: HandleUserPromptRequest
	(*HandleUserPromptResponse)(nil),                           // 149: HandleUserPromptResponse
	(*GetUserPromptJobRequest)(nil),                            // 150: GetUserPromptJobRequest
	(*GetUserPromptJobResponse)(nil),                           // 151: GetUserPromptJobResponse
	(*HandleFollowupUserPromptRequest)(nil),                    // 152: HandleFollowupUserPromptRequest
	(*HandleFollowupUserPromptResponse)(nil),                   // 153: HandleFollowupUserPromptResponse
	(*FinalizeUserPromptRequest)(nil),                          // 154: FinalizeUserPromptRequest
	(*FinalizeUserPromptResponse)(nil),                         // 155: FinalizeUserPromptResponse
	(*ListAIUsageForUserRequest)(nil),                          // 156: ListAIUsageForUserRequest
	(*ListAIUsageForUserResponse)(nil),                         // 157: ListAIUsageForUserResponse
	(*GetAIUsageForUserInPeriodRequest)(nil),                   // 158: GetAIUsageForUserInPeriodRequest
	(*GetAIUsageForUserInPeriodResponse)(nil),                  // 159: GetAIUsageForUserInPeriodResponse
	(*RegisterLocalAgentRequest)(nil),                          // 160: RegisterLocalAgentRequest
	(*RegisterLocalAgentResponse)(nil),                         // 161: RegisterLocalAgentResponse
	(*RevokeLocalAgentRequest)(nil),                            // 162: RevokeLocalAgentRequest
	(*RevokeLocalAgentResponse)(nil),                           // 163: RevokeLocalAgentResponse
	(*ListLocalAgentsRequest)(nil),                             // 164: ListLocalAgentsRequest
	(*ListLocalAgentsResponse)(nil),                            // 165: ListLocalAgentsResponse
	(*UpdateLocalAgentConnectionsRequest)(nil),                 // 166: UpdateLocalAgentConnectionsRequest
	(*UpdateLocalAgentConnectionsResponse)(nil),                // 167: UpdateLocalAgentConnectionsResponse
	(*GetLocalAgentConnectionsRequest)(nil),                    // 168: GetLocalAgentConnectionsRequest
	(*GetLocalAgentConnectionsResponse)(nil),                   // 169: GetLocalAgentConnectionsResponse
	(*UpsertDeploymentRequest)(nil),                            // 170: UpsertDeploymentRequest
	(*UpsertDeploymentResponse)(nil),                           // 171: UpsertDeploymentResponse
	(*ListDeploymentsRequest)(nil),                             // 172: ListDeploymentsRequest
	(*ListDeploymentsResponse)(nil),                            // 173: ListDeploymentsResponse
	(*MarkDeploymentDestroyedRequest)(nil),                     // 174: MarkDeploymentDestroyedRequest
	(*MarkDeploymentDestroyedResponse)(nil),                    // 175: MarkDeploymentDestroyedResponse
	(*IssueProvisioningTokenRequest)(nil),                      // 176: IssueProvisioningTokenRequest
	(*IssueProvisioningTokenResponse)(nil),                     // 177: IssueProvisioningTokenResponse
	(*ExchangeProvisioningTokenRequest)(nil),                   // 178: ExchangeProvisioningTokenRequest
	(*ExchangeProvisioningTokenResponse)(nil),                  // 179: ExchangeProvisioningTokenResponse
	(*CheckExtensionExecutionLimitRequest)(nil),                // 180: CheckExtensionExecutionLimitRequest
	(*CheckExtensionExecutionLimitResponse)(nil),               // 181: CheckExtensionExecutionLimitResponse
	(*CreateAutomationRequest)(nil),                            // 182: CreateAutomationRequest
	(*CreateAutomationResponse)(nil),                           // 183: CreateAutomationResponse
	(*RotateAutomationSecretRequest)(nil),                      // 184: RotateAutomationSecretRequest
	(*RotateAutomationSecretResponse)(nil),                     // 185: RotateAutomationSecretResponse
	(*UpdateAutomationRequest)(nil),                            // 186: UpdateAutomationRequest
	(*DeleteAutomationRequest)(nil),                            // 187: DeleteAutomationRequest
	(*DeleteAutomationResponse)(nil),                           // 188: DeleteAutomationResponse
	(*GetAutomationRequest)(nil),                               // 189: GetAutomationRequest
	(*ListAutomationsForProjectRequest)(nil),                   // 190: ListAutomationsForProjectRequest
	(*ListAutomationsForProjectResponse)(nil),                  // 191: ListAutomationsForProjectResponse
	(*TestAutomationRequest)(nil),                              // 192: TestAutomationRequest
	(*TestAutomationResponse)(nil),                             // 193: TestAutomationResponse
	(*ListAutomationEventsRequest)(nil),                        // 194: ListAutomationEventsRequest
	(*ListAutomationEventsResponse)(nil),                       // 195: ListAutomationEventsResponse
	(*GetAutomationEventRequest)(nil),                          // 196: GetAutomationEventRequest
	(*RetryAutomationEventRequest)(nil),                        // 197: RetryAutomationEventRequest
	(*MarkChangeRequestAppliedRequest)(nil),                    // 198: MarkChangeRequestAppliedRequest
	(*MarkChangeRequestAppliedResponse)(nil),                   // 199: MarkChangeRequestAppliedResponse
	(gen.UserProjectRole)(0),                                   // 200: nem.UserProjectRole
	(gen.UserTeamRole)(0),                                      // 201: nem.UserTeamRole
	(*gen.User)(nil),                                           // 202: nem.User
	(*gen.UserTeam)(nil),                                       // 203: nem.UserTeam
	(*gen.UserProject)(nil),                                    // 204: nem.UserProject
	(*gen.Team)(nil),                                           // 205: nem.Team
	(*fieldmaskpb.FieldMask)(nil),                              // 206: google.protobuf.FieldMask
	(*gen.Connection)(nil),                                     // 207: nem.Connection
	(*gen.Project)(nil),                                        // 208: nem.Project
	(*gen.ProjectVersion)(nil),                                 // 209: nem.ProjectVersion
	(gen.ProjectVersionReviewStatus)(0),                        // 210: nem.ProjectVersionReviewStatus
	(*gen.Extension)(nil),                                      // 211: nem.Extension
	(*gen.ExtensionVersion)(nil),                               // 212: nem.ExtensionVersion
	(*gen.ExtensionExecution)(nil),                             // 213: nem.ExtensionExecution
	(gen.ChangeRequestReviewStatus)(0),                         // 214: nem.ChangeRequestReviewStatus
	(gen.ChangeRequestChangeType)(0),                           // 215: nem.ChangeRequestChangeType
	(*gen.ChangeRequest)(nil),                                  // 216: nem.ChangeRequest
	(*gen.ObjectStore)(nil),                                    // 217: nem.ObjectStore
	(gen.ConnectionDbType)(0),                                  // 218: nem.ConnectionDbType
	(*gen.Membership)(nil),                                     // 219: nem.Membership
	(gen.MembershipType)(0),                                    // 220: nem.MembershipType
	(*gen.AiUsage)(nil),                                        // 221: nem.AiUsage
	(*gen.LocalAgent)(nil),                                     // 222: nem.LocalAgent
	(*gen.LocalAgentConnection)(nil),                           // 223: nem.LocalAgentConnection
	(*gen.Deployment)(nil),                                     // 224: nem.Deployment
	(*gen.Automation)(nil),                                     // 225: nem.Automation
	(gen.AutomationEventStatus)(0),                             // 226: nem.AutomationEventStatus
	(*gen.AutomationEvent)(nil),                                // 227: nem.AutomationEvent
}
var file_product_proto_depIdxs = []int32{
	200, // 0: GetUserRoleForProjectResponse.role:type_name -> nem.UserProjectRole
	201, // 1: GetUserRoleForTeamResponse.role:type_name -> nem.UserTeamRole
	202, // 2: UpdateUserRequest.user:type_name -> nem.User
	202, // 3: ListUsersForTeamResponse.users:type_name -> nem.User
	203, // 4: ListUsersForTeamResponse.user_teams:type_name -> nem.UserTeam
	202, // 5: ListUsersForProjectResponse.users:type_name -> nem.User
	204, // 6: ListUsersForProjectResponse.user_projects:type_name -> nem.UserProject
	200, // 7: AddUserToProjectRequest.role:type_name -> nem.UserProjectRole
	200, // 8: UpdateUserProjectRequest.role:type_name -> nem.UserProjectRole
	201, // 9: AddUserToTeamRequest.role:type_name -> nem.UserTeamRole
	201, // 10: UpdateUserTeamRequest.role:type_name -> nem.UserTeamRole
	205, // 11: ListTeamsForUserResponse.teams:type_name -> nem.Team
	205, // 12: ListTeamsForAdminUserResponse.teams:type_name -> nem.Team
	205, // 13: CreateTeamRequest.team:type_name -> nem.Team
	205, // 14: UpdateTeamRequest.team:type_name -> nem.Team
	206, // 15: UpdateTeamRequest.update_mask:type_name -> google.protobuf.FieldMask
	207, // 16: CreateConnectionSecretRequest.connection:type_name -> nem.Connection
	207, // 17: UpdateConnectionSecretRequest.connection:type_name -> nem.Connection
	207, // 18: DeleteConnectionSecretRequest.connection:type_name -> nem.Connection
	208, // 19: ListProjectsForUserResponse.projects:type_name -> nem.Project
	208, // 20: CreateProjectRequest.project:type_name -> nem.Project
	208, // 21: UpdateProjectRequest.project:type_name -> nem.Project
	206, // 22: UpdateProjectRequest.update_mask:type_name -> google.protobuf.FieldMask
	209, // 23: ListProjectVersionsResponse.project_versions:type_name -> nem.ProjectVersion
	210, // 24: ListProjectVersionsForUserRequest.review_status:type_name -> nem.ProjectVersionReviewStatus
	209, // 25: ListProjectVersionsForUserResponse.project_versions:type_name -> nem.ProjectVersion
	210, // 26: GetLatestProjectVersionRequest.review_status:type_name -> nem.ProjectVersionReviewStatus
	210, // 27: GetLatestProjectVersionForUserRequest.review_status:type_name -> nem.ProjectVersionReviewStatus
	210, // 28: GetLatestProjectVersionUUIDForUserRequest.review_status:type_name -> nem.ProjectVersionReviewStatus
	209, // 29: CreateProjectVersionRequest.project_version:type_name -> nem.ProjectVersion
	209, // 30: UpdateProjectVersionRequest.project_version:type_name -> nem.ProjectVersion
	206, // 31: UpdateProjectVersionRequest.update_mask:type_name -> google.protobuf.FieldMask
	210, // 32: ReviewProjectVersionRequest.review_status:type_name -> nem.ProjectVersionReviewStatus
	211, // 33: ListExtensionsResponse.extensions:type_name -> nem.Extension
	211, // 34: CreateExtensionRequest.extension:type_name -> nem.Extension
	211, // 35: UpdateExtensionRequest.extension:type_name -> nem.Extension
	206, // 36: UpdateExtensionRequest.update_mask:type_name -> google.protobuf.FieldMask
	212, // 37: ListExtensionVersionsResponse.versions:type_name -> nem.ExtensionVersion
	212, // 38: CreateExtensionVersionRequest.version:type_name -> nem.ExtensionVersion
	212, // 39: UpdateExtensionVersionRequest.version:type_name -> nem.ExtensionVersion
	206, // 40: UpdateExtensionVersionRequest.update_mask:type_name -> google.protobuf.FieldMask
	213, // 41: ListExtensionExecutionsResponse.executions:type_name -> nem.ExtensionExecution
	213, // 42: CreateExtensionExecutionRequest.execution:type_name -> nem.ExtensionExecution
	213, // 43: UpdateExtensionExecutionRequest.execution:type_name -> nem.ExtensionExecution
	206, // 44: UpdateExtensionExecutionRequest.update_mask:type_name -> google.protobuf.FieldMask
	214, // 45: ListUserChangeRequestsRequest.review_status:type_name -> nem.ChangeRequestReviewStatus
	215, // 46: ListUserChangeRequestsRequest.change_request_type:type_name -> nem.ChangeRequestChangeType
	216, // 47: ListUserChangeRequestsResponse.change_requests:type_name -> nem.ChangeRequest
	216, // 48: CreateChangeRequestRequest.change_request:type_name -> nem.ChangeRequest
	216, // 49: UpdateChangeRequestRequest.change_request:type_name -> nem.ChangeRequest
	206, // 50: UpdateChangeRequestRequest.update_mask:type_name -> google.protobuf.FieldMask
	215, // 51: ListChangeRequestsForReviewRequest.change_request_type:type_name -> nem.ChangeRequestChangeType
	216, // 52: ListChangeRequestsForReviewResponse.change_requests:type_name -> nem.ChangeRequest
	217, // 53: CreateObjectStoreSecretRequest.object_store:type_name -> nem.ObjectStore
	217, // 54: UpdateObjectStoreSecretRequest.object_store:type_name -> nem.ObjectStore
	217, // 55: DeleteObjectStoreSecretRequest.object_store:type_name -> nem.ObjectStore
	218, // 56: GenerateSQLForCRRequest.db_type_override:type_name -> nem.ConnectionDbType
	111, // 57: GenerateSQLForCRResponse.statements:type_name -> GenerateSQLForCRStatement
	214, // 58: ReviewDataChangeRequest.review_status:type_name -> nem.ChangeRequestReviewStatus
	219, // 59: CreateMembershipRequest.membership:type_name -> nem.Membership
	219, // 60: UpdateMembershipRequest.membership:type_name -> nem.Membership
	206, // 61: UpdateMembershipRequest.update_mask:type_name -> google.protobuf.FieldMask
	0,   // 62: CreateCheckoutSessionRequest.session_type:type_name -> CheckoutSessionType
	220, // 63: CreateCheckoutSessionRequest.plan_type:type_name -> nem.MembershipType
	220, // 64: ChangeMembershipPlanRequest.new_plan_type:type_name -> nem.MembershipType
	202, // 65: ListMembershipUsersResponse.users:type_name -> nem.User
	208, // 66: ListMembershipUserProjectsResponse.projects:type_name -> nem.Project
	219, // 67: MembershipWithName.membership:type_name -> nem.Membership
	136, // 68: ListMembershipsForUserResponse.memberships:type_name -> MembershipWithName
	220, // 69: GetMembershipTierForProjectResponse.membership_type:type_name -> nem.MembershipType
	1,   // 70: EvaluateUserPromptRequest.context:type_name -> ProContext
	1,   // 71: HandleUserPromptRequest.context:type_name -> ProContext
	1,   // 72: GetUserPromptJobRequest.context:type_name -> ProContext
	209, // 73: GetUserPromptJobResponse.project_version:type_name -> nem.ProjectVersion
	1,   // 74: HandleFollowupUserPromptRequest.context:type_name -> ProContext
	1,   // 75: FinalizeUserPromptRequest.context:type_name -> ProContext
	221, // 76: ListAIUsageForUserResponse.usages:type_name -> nem.AiUsage
	222, // 77: ListLocalAgentsResponse.local_agents:type_name -> nem.LocalAgent
	223, // 78: UpdateLocalAgentConnectionsRequest.connections:type_name -> nem.LocalAgentConnection
	223, // 79: GetLocalAgentConnectionsResponse.connections:type_name -> nem.LocalAgentConnection
	224, // 80: UpsertDeploymentRequest.deployment:type_name -> nem.Deployment
	224, // 81: UpsertDeploymentResponse.deployment:type_name -> nem.Deployment
	224, // 82: ListDeploymentsResponse.deployments:type_name -> nem.Deployment
	225, // 83: CreateAutomationRequest.automation:type_name -> nem.Automation
	225, // 84: CreateAutomationResponse.automation:type_name -> nem.Automation
	225, // 85: UpdateAutomationRequest.automation:type_name -> nem.Automation
	225, // 86: ListAutomationsForProjectResponse.automations:type_name -> nem.Automation
	226, // 87: ListAutomationEventsRequest.status:type_name -> nem.AutomationEventStatus
	227, // 88: ListAutomationEventsResponse.automation_events:type_name -> nem.AutomationEvent
	216, // 89: MarkChangeRequestAppliedResponse.change_request:type_name -> nem.ChangeRequest
	2,   // 90: NuzurProduct.GetUser:input_type -> GetUserRequest
	3,   // 91: NuzurProduct.GetTokenUser:input_type -> GetTokenUserRequest
	4,   // 92: NuzurProduct.GetUserByEmail:input_type -> GetUserByEmailRequest
	5,   // 93: NuzurProduct.GetTokenUserRoleForProject:input_type -> GetUserRoleForProjectRequest
	7,   // 94: NuzurProduct.GetTokenUserRoleForTeam:input_type -> GetUserRoleForTeamRequest
	9,   // 95: NuzurProduct.UpdateTokenUser:input_type -> UpdateUserRequest
	10,  // 96: NuzurProduct.ListUsersForTeam:input_type -> ListUsersForTeamRequest
	12,  // 97: NuzurProduct.ListUsersForProject:input_type -> ListUsersForProjectRequest
	14,  // 98: NuzurProduct.AddUserToProject:input_type -> AddUserToProjectRequest
	15,  // 99: NuzurProduct.UpdateUserProject:input_type -> UpdateUserProjectRequest
	16,  // 100: NuzurProduct.RemoveUserFromProject:input_type -> RemoveUserFromProjectRequest
	17,  // 101: NuzurProduct.AddUserToTeam:input_type -> AddUserToTeamRequest
	18,  // 102: NuzurProduct.UpdateUserTeam:input_type -> UpdateUserTeamRequest
	19,  // 103: NuzurProduct.RemoveUserFromTeam:input_type -> RemoveUserFromTeamRequest
	20,  // 104: NuzurProduct.ListTeamsForUser:input_type -> ListTeamsForUserRequest
	22,  // 105: NuzurProduct.ListTeamsForAdminUser:input_type -> ListTeamsForAdminUserRequest
	24,  // 106: NuzurProduct.GetTeamForUser:input_type -> GetTeamForUserRequest
	25,  // 107: NuzurProduct.CreateTeam:input_type -> CreateTeamRequest
	26,  // 108: NuzurProduct.UpdateTeam:input_type -> UpdateTeamRequest
	27,  // 109: NuzurProduct.GetConnectionWithSecret:input_type -> GetConnectionWithSecretRequest
	28,  // 110: NuzurProduct.CreateConnectionSecret:input_type -> CreateConnectionSecretRequest
	29,  // 111: NuzurProduct.UpdateConnectionSecret:input_type -> UpdateConnectionSecretRequest
	30,  // 112: NuzurProduct.DeleteConnectionSecret:input_type -> DeleteConnectionSecretRequest
	31,  // 113: NuzurProduct.ListProjectsForUser:input_type -> ListProjectsForUserRequest
	33,  // 114: NuzurProduct.GetProjectForUser:input_type -> GetProjectForUserRequest
	34,  // 115: NuzurProduct.CreateProject:input_type -> CreateProjectRequest
	35,  // 116: NuzurProduct.UpdateProject:input_type -> UpdateProjectRequest
	36,  // 117: NuzurProduct.GetProjectName:input_type -> GetProjectNameRequest
	38,  // 118: NuzurProduct.ListProjectVersions:input_type -> ListProjectVersionsRequest
	40,  // 119: NuzurProduct.ListProjectVersionsForUser:input_type -> ListProjectVersionsForUserRequest
	40,  // 120: NuzurProduct.ListProjectVersionsForUserCached:input_type -> ListProjectVersionsForUserRequest
	42,  // 121: NuzurProduct.GetProjectVersionForUser:input_type -> GetProjectVersionForUserRequest
	42,  // 122: NuzurProduct.GetProjectVersionForUserCached:input_type -> GetProjectVersionForUserRequest
	42,  // 123: NuzurProduct.GetProjectVersionLeanForUser:input_type -> GetProjectVersionForUserRequest
	43,  // 124: NuzurProduct.GetLatestProjectVersion:input_type -> GetLatestProjectVersionRequest
	44,  // 125: NuzurProduct.GetLatestProjectVersionForUser:input_type -> GetLatestProjectVersionForUserRequest
	44,  // 126: NuzurProduct.GetLatestProjectVersionLeanForUser:input_type -> GetLatestProjectVersionForUserRequest
	45,  // 127: NuzurProduct.GetLatestProjectVersionUUIDForUser:input_type -> GetLatestProjectVersionUUIDForUserRequest
	47,  // 128: NuzurProduct.GetProjectVersionVersion:input_type -> GetProjectVersionVersionRequest
	49,  // 129: NuzurProduct.GetProjectVersionIdentifier:input_type -> GetProjectVersionIdentifierRequest
	51,  // 130: NuzurProduct.CreateProjectVersion:input_type -> CreateProjectVersionRequest
	52,  // 131: NuzurProduct.CreateProjectVersionFromJSON:input_type -> CreateProjectVersionFromJSONRequest
	54,  // 132: NuzurProduct.CreateDraftProjectVersionForExistingProject:input_type -> CreateDraftProjectVersionForExistingProjectRequest
	53,  // 133: NuzurProduct.UpdateProjectVersion:input_type -> UpdateProjectVersionRequest
	55,  // 134: NuzurProduct.DiscardDraftProjectVersion:input_type -> DiscardDraftProjectVersionRequest
	56,  // 135: NuzurProduct.SendProjectVersionForReview:input_type -> SendProjectVersionForReviewRequest
	57,  // 136: NuzurProduct.WithdrawFromReviewProjectVersion:input_type -> WithdrawFromReviewProjectVersionRequest
	58,  // 137: NuzurProduct.ReviewProjectVersion:input_type -> ReviewProjectVersionRequest
	59,  // 138: NuzurProduct.UploadProjectVersionSnapshot:input_type -> UploadProjectVersionSnapshotRequest
	61,  // 139: NuzurProduct.GetSignedFileURL:input_type -> GetSignedFileURLRequest
	63,  // 140: NuzurProduct.UploadExtensionExecutionFile:input_type -> UploadExtensionExecutionFileRequest
	65,  // 141: NuzurProduct.GetExtensionExecutionFile:input_type -> GetExtensionExecutionFileRequest
	67,  // 142: NuzurProduct.UploadExtensionIcon:input_type -> UploadExtensionIconRequest
	77,  // 143: NuzurProduct.UploadUserAvatar:input_type -> UploadUserAvatarRequest
	71,  // 144: NuzurProduct.UploadRecordFieldFile:input_type -> UploadRecordFieldFileRequest
	73,  // 145: NuzurProduct.GetRecordFieldSignedFileURL:input_type -> GetRecordFieldSignedFileURLRequest
	75,  // 146: NuzurProduct.GetRecordFileContent:input_type -> GetRecordFileContentRequest
	79,  // 147: NuzurProduct.ListExtensions:input_type -> ListExtensionsRequest
	81,  // 148: NuzurProduct.GetExtension:input_type -> GetExtensionRequest
	82,  // 149: NuzurProduct.CreateExtension:input_type -> CreateExtensionRequest
	83,  // 150: NuzurProduct.UpdateExtension:input_type -> UpdateExtensionRequest
	84,  // 151: NuzurProduct.ListExtensionVersions:input_type -> ListExtensionVersionsRequest
	86,  // 152: NuzurProduct.GetExtensionVersion:input_type -> GetExtensionVersionRequest
	87,  // 153: NuzurProduct.CreateExtensionVersion:input_type -> CreateExtensionVersionRequest
	88,  // 154: NuzurProduct.UpdateExtensionVersion:input_type -> UpdateExtensionVersionRequest
	89,  // 155: NuzurProduct.ListExtensionExecutions:input_type -> ListExtensionExecutionsRequest
	91,  // 156: NuzurProduct.GetExtensionExecution:input_type -> GetExtensionExecutionRequest
	92,  // 157: NuzurProduct.CreateExtensionExecution:input_type -> CreateExtensionExecutionRequest
	93,  // 158: NuzurProduct.UpdateExtensionExecution:input_type -> UpdateExtensionExecutionRequest
	180, // 159: NuzurProduct.CheckExtensionExecutionLimit:input_type -> CheckExtensionExecutionLimitRequest
	94,  // 160: NuzurProduct.ListUserChangeRequests:input_type -> ListUserChangeRequestsRequest
	96,  // 161: NuzurProduct.GetChangeRequest:input_type -> GetChangeRequestRequest
	97,  // 162: NuzurProduct.CreateChangeRequest:input_type -> CreateChangeRequestRequest
	98,  // 163: NuzurProduct.UpdateChangeRequest:input_type -> UpdateChangeRequestRequest
	99,  // 164: NuzurProduct.ListChangeRequestsForReview:input_type -> ListChangeRequestsForReviewRequest
	101, // 165: NuzurProduct.GetUserProjectVersionData:input_type -> GetUserProjectVersionDataRequest
	103, // 166: NuzurProduct.SaveUserProjectVersionData:input_type -> SaveUserProjectVersionDataRequest
	105, // 167: NuzurProduct.GetObjectStoreWithSecret:input_type -> GetObjectStoreWithSecretRequest
	106, // 168: NuzurProduct.CreateObjectStoreSecret:input_type -> CreateObjectStoreSecretRequest
	107, // 169: NuzurProduct.UpdateObjectStoreSecret:input_type -> UpdateObjectStoreSecretRequest
	108, // 170: NuzurProduct.DeleteObjectStoreSecret:input_type -> DeleteObjectStoreSecretRequest
	109, // 171: NuzurProduct.GenerateSQLForCR:input_type -> GenerateSQLForCRRequest
	112, // 172: NuzurProduct.ReviewDataChange:input_type -> ReviewDataChangeRequest
	114, // 173: NuzurProduct.GetMembership:input_type -> GetMembershipRequest
	115, // 174: NuzurProduct.CreateMembership:input_type -> CreateMembershipRequest
	116, // 175: NuzurProduct.UpdateMembership:input_type -> UpdateMembershipRequest
	117, // 176: NuzurProduct.MembershipForProject:input_type -> MembershipForProjectRequest
	118, // 177: NuzurProduct.MembershipForTeam:input_type -> MembershipForTeamRequest
	119, // 178: NuzurProduct.CreateCheckoutSession:input_type -> CreateCheckoutSessionRequest
	123, // 179: NuzurProduct.ListMembershipUsers:input_type -> ListMembershipUsersRequest
	125, // 180: NuzurProduct.CancelMembership:input_type -> CancelMembershipRequest
	127, // 181: NuzurProduct.ReactivateMembership:input_type -> ReactivateMembershipRequest
	129, // 182: NuzurProduct.UserHasActiveStripeSubscription:input_type -> UserHasActiveStripeSubscriptionRequest
	131, // 183: NuzurProduct.ListMembershipUserProjects:input_type -> ListMembershipUserProjectsRequest
	133, // 184: NuzurProduct.RemoveMembershipUser:input_type -> RemoveMembershipUserRequest
	135, // 185: NuzurProduct.ListMembershipsForUser:input_type -> ListMembershipsForUserRequest
	121, // 186: NuzurProduct.ChangeMembershipPlan:input_type -> ChangeMembershipPlanRequest
	138, // 187: NuzurProduct.SendPrioritySupportEmail:input_type -> PrioritySupportEmailRequest
	140, // 188: NuzurProduct.HandleWebhook:input_type -> HandleWebhookRequest
	142, // 189: NuzurProduct.IsProActiveForProject:input_type -> IsProActiveForProjectRequest
	144, // 190: NuzurProduct.GetMembershipTierForProject:input_type -> GetMembershipTierForProjectRequest
	146, // 191: NuzurProduct.EvaluateUserPrompt:input_type -> EvaluateUserPromptRequest
	148, // 192: NuzurProduct.HandleUserPrompt:input_type -> HandleUserPromptRequest
	150, // 193: NuzurProduct.GetUserPromptJob:input_type -> GetUserPromptJobRequest
	152, // 194: NuzurProduct.HandleFollowupUserPrompt:input_type -> HandleFollowupUserPromptRequest
	154, // 195: NuzurProduct.FinalizeUserPrompt:input_type -> FinalizeUserPromptRequest
	156, // 196: NuzurProduct.ListAIUsageForUser:input_type -> ListAIUsageForUserRequest
	158, // 197: NuzurProduct.GetAIUsageForUserInPeriod:input_type -> GetAIUsageForUserInPeriodRequest
	160, // 198: NuzurProduct.RegisterLocalAgent:input_type -> RegisterLocalAgentRequest
	162, // 199: NuzurProduct.RevokeLocalAgent:input_type -> RevokeLocalAgentRequest
	164, // 200: NuzurProduct.ListLocalAgents:input_type -> ListLocalAgentsRequest
	166, // 201: NuzurProduct.UpdateLocalAgentConnections:input_type -> UpdateLocalAgentConnectionsRequest
	168, // 202: NuzurProduct.GetLocalAgentConnections:input_type -> GetLocalAgentConnectionsRequest
	176, // 203: NuzurProduct.IssueProvisioningToken:input_type -> IssueProvisioningTokenRequest
	178, // 204: NuzurProduct.ExchangeProvisioningToken:input_type -> ExchangeProvisioningTokenRequest
	170, // 205: NuzurProduct.UpsertDeployment:input_type -> UpsertDeploymentRequest
	172, // 206: NuzurProduct.ListDeployments:input_type -> ListDeploymentsRequest
	174, // 207: NuzurProduct.MarkDeploymentDestroyed:input_type -> MarkDeploymentDestroyedRequest
	182, // 208: NuzurProduct.CreateAutomation:input_type -> CreateAutomationRequest
	184, // 209: NuzurProduct.RotateAutomationSecret:input_type -> RotateAutomationSecretRequest
	186, // 210: NuzurProduct.UpdateAutomation:input_type -> UpdateAutomationRequest
	187, // 211: NuzurProduct.DeleteAutomation:input_type -> DeleteAutomationRequest
	189, // 212: NuzurProduct.GetAutomation:input_type -> GetAutomationRequest
	190, // 213: NuzurProduct.ListAutomationsForProject:input_type -> ListAutomationsForProjectRequest
	192, // 214: NuzurProduct.TestAutomation:input_type -> TestAutomationRequest
	194, // 215: NuzurProduct.ListAutomationEvents:input_type -> ListAutomationEventsRequest
	196, // 216: NuzurProduct.GetAutomationEvent:input_type -> GetAutomationEventRequest
	197, // 217: NuzurProduct.RetryAutomationEvent:input_type -> RetryAutomationEventRequest
	198, // 218: NuzurProduct.MarkChangeRequestApplied:input_type -> MarkChangeRequestAppliedRequest
	202, // 219: NuzurProduct.GetUser:output_type -> nem.User
	202, // 220: NuzurProduct.GetTokenUser:output_type -> nem.User
	202, // 221: NuzurProduct.GetUserByEmail:output_type -> nem.User
	6,   // 222: NuzurProduct.GetTokenUserRoleForProject:output_type -> GetUserRoleForProjectResponse
	8,   // 223: NuzurProduct.GetTokenUserRoleForTeam:output_type -> GetUserRoleForTeamResponse
	202, // 224: NuzurProduct.UpdateTokenUser:output_type -> nem.User
	11,  // 225: NuzurProduct.ListUsersForTeam:output_type -> ListUsersForTeamResponse
	13,  // 226: NuzurProduct.ListUsersForProject:output_type -> ListUsersForProjectResponse
	204, // 227: NuzurProduct.AddUserToProject:output_type -> nem.UserProject
	204, // 228: NuzurProduct.UpdateUserProject:output_type -> nem.UserProject
	204, // 229: NuzurProduct.RemoveUserFromProject:output_type -> nem.UserProject
	203, // 230: NuzurProduct.AddUserToTeam:output_type -> nem.UserTeam
	203, // 231: NuzurProduct.UpdateUserTeam:output_type -> nem.UserTeam
	203, // 232: NuzurProduct.RemoveUserFromTeam:output_type -> nem.UserTeam
	21,  // 233: NuzurProduct.ListTeamsForUser:output_type -> ListTeamsForUserResponse
	23,  // 234: NuzurProduct.ListTeamsForAdminUser:output_type -> ListTeamsForAdminUserResponse
	205, // 235: NuzurProduct.GetTeamForUser:output_type -> nem.Team
	205, // 236: NuzurProduct.CreateTeam:output_type -> nem.Team
	205, // 237: NuzurProduct.UpdateTeam:output_type -> nem.Team
	207, // 238: NuzurProduct.GetConnectionWithSecret:output_type -> nem.Connection
	207, // 239: NuzurProduct.CreateConnectionSecret:output_type -> nem.Connection
	207, // 240: NuzurProduct.UpdateConnectionSecret:output_type -> nem.Connection
	207, // 241: NuzurProduct.DeleteConnectionSecret:output_type -> nem.Connection
	32,  // 242: NuzurProduct.ListProjectsForUser:output_type -> ListProjectsForUserResponse
	208, // 243: NuzurProduct.GetProjectForUser:output_type -> nem.Project
	208, // 244: NuzurProduct.CreateProject:output_type -> nem.Project
	208, // 245: NuzurProduct.UpdateProject:output_type -> nem.Project
	37,  // 246: NuzurProduct.GetProjectName:output_type -> GetProjectNameResponse
	39,  // 247: NuzurProduct.ListProjectVersions:output_type -> ListProjectVersionsResponse
	41,  // 248: NuzurProduct.ListProjectVersionsForUser:output_type -> ListProjectVersionsForUserResponse
	41,  // 249: NuzurProduct.ListProjectVersionsForUserCached:output_type -> ListProjectVersionsForUserResponse
	209, // 250: NuzurProduct.GetProjectVersionForUser:output_type -> nem.ProjectVersion
	209, // 251: NuzurProduct.GetProjectVersionForUserCached:output_type -> nem.ProjectVersion
	209, // 252: NuzurProduct.GetProjectVersionLeanForUser:output_type -> nem.ProjectVersion
	209, // 253: NuzurProduct.GetLatestProjectVersion:output_type -> nem.ProjectVersion
	209, // 254: NuzurProduct.GetLatestProjectVersionForUser:output_type -> nem.ProjectVersion
	209, // 255: NuzurProduct.GetLatestProjectVersionLeanForUser:output_type -> nem.ProjectVersion
	46,  // 256: NuzurProduct.GetLatestProjectVersionUUIDForUser:output_type -> GetLatestProjectVersionUUIDForUserResponse
	48,  // 257: NuzurProduct.GetProjectVersionVersion:output_type -> GetProjectVersionVersionResponse
	50,  // 258: NuzurProduct.GetProjectVersionIdentifier:output_type -> GetProjectVersionIdentifierResponse
	209, // 259: NuzurProduct.CreateProjectVersion:output_type -> nem.ProjectVersion
	209, // 260: NuzurProduct.CreateProjectVersionFromJSON:output_type -> nem.ProjectVersion
	209, // 261: NuzurProduct.CreateDraftProjectVersionForExistingProject:output_type -> nem.ProjectVersion
	209, // 262: NuzurProduct.UpdateProjectVersion:output_type -> nem.ProjectVersion
	209, // 263: NuzurProduct.DiscardDraftProjectVersion:output_type -> nem.ProjectVersion
	209, // 264: NuzurProduct.SendProjectVersionForReview:output_type -> nem.ProjectVersion
	209, // 265: NuzurProduct.WithdrawFromReviewProjectVersion:output_type -> nem.ProjectVersion
	209, // 266: NuzurProduct.ReviewProjectVersion:output_type -> nem.ProjectVersion
	60,  // 267: NuzurProduct.UploadProjectVersionSnapshot:output_type -> UploadProjectVersionSnapshotResponse
	62,  // 268: NuzurProduct.GetSignedFileURL:output_type -> GetSignedFileURLResponse
	64,  // 269: NuzurProduct.UploadExtensionExecutionFile:output_type -> UploadExtensionExecutionFileResponse
	66,  // 270: NuzurProduct.GetExtensionExecutionFile:output_type -> GetExtensionExecutionFileResponse
	68,  // 271: NuzurProduct.UploadExtensionIcon:output_type -> UploadExtensionIconResponse
	78,  // 272: NuzurProduct.UploadUserAvatar:output_type -> UploadUserAvatarResponse
	72,  // 273: NuzurProduct.UploadRecordFieldFile:output_type -> UploadRecordFieldFileResponse
	74,  // 274: NuzurProduct.GetRecordFieldSignedFileURL:output_type -> GetRecordFieldSignedFileURLResponse
	76,  // 275: NuzurProduct.GetRecordFileContent:output_type -> GetRecordFileContentResponse
	80,  // 276: NuzurProduct.ListExtensions:output_type -> ListExtensionsResponse
	211, // 277: NuzurProduct.GetExtension:output_type -> nem.Extension
	211, // 278: NuzurProduct.CreateExtension:output_type -> nem.Extension
	211, // 279: NuzurProduct.UpdateExtension:output_type -> nem.Extension
	85,  // 280: NuzurProduct.ListExtensionVersions:output_type -> ListExtensionVersionsResponse
	212, // 281: NuzurProduct.GetExtensionVersion:output_type -> nem.ExtensionVersion
	212, // 282: NuzurProduct.CreateExtensionVersion:output_type -> nem.ExtensionVersion
	212, // 283: NuzurProduct.UpdateExtensionVersion:output_type -> nem.ExtensionVersion
	90,  // 284: NuzurProduct.ListExtensionExecutions:output_type -> ListExtensionExecutionsResponse
	213, // 285: NuzurProduct.GetExtensionExecution:output_type -> nem.ExtensionExecution
	213, // 286: NuzurProduct.CreateExtensionExecution:output_type -> nem.ExtensionExecution
	213, // 287: NuzurProduct.UpdateExtensionExecution:output_type -> nem.ExtensionExecution
	181, // 288: NuzurProduct.CheckExtensionExecutionLimit:output_type -> CheckExtensionExecutionLimitResponse
	95,  // 289: NuzurProduct.ListUserChangeRequests:output_type -> ListUserChangeRequestsResponse
	216, // 290: NuzurProduct.GetChangeRequest:output_type -> nem.ChangeRequest
	216, // 291: NuzurProduct.CreateChangeRequest:output_type -> nem.ChangeRequest
	216, // 292: NuzurProduct.UpdateChangeRequest:output_type -> nem.ChangeRequest
	100, // 293: NuzurProduct.ListChangeRequestsForReview:output_type -> ListChangeRequestsForReviewResponse
	102, // 294: NuzurProduct.GetUserProjectVersionData:output_type -> GetUserProjectVersionDataResponse
	104, // 295: NuzurProduct.SaveUserProjectVersionData:output_type -> SaveUserProjectVersionDataResponse
	217, // 296: NuzurProduct.GetObjectStoreWithSecret:output_type -> nem.ObjectStore
	217, // 297: NuzurProduct.CreateObjectStoreSecret:output_type -> nem.ObjectStore
	217, // 298: NuzurProduct.UpdateObjectStoreSecret:output_type -> nem.ObjectStore
	217, // 299: NuzurProduct.DeleteObjectStoreSecret:output_type -> nem.ObjectStore
	110, // 300: NuzurProduct.GenerateSQLForCR:output_type -> GenerateSQLForCRResponse
	113, // 301: NuzurProduct.ReviewDataChange:output_type -> ReviewDataChangeResponse
	219, // 302: NuzurProduct.GetMembership:output_type -> nem.Membership
	219, // 303: NuzurProduct.CreateMembership:output_type -> nem.Membership
	219, // 304: NuzurProduct.UpdateMembership:output_type -> nem.Membership
	219, // 305: NuzurProduct.MembershipForProject:output_type -> nem.Membership
	219, // 306: NuzurProduct.MembershipForTeam:output_type -> nem.Membership
	120, // 307: NuzurProduct.CreateCheckoutSession:output_type -> CreateCheckoutSessionResponse
	124, // 308: NuzurProduct.ListMembershipUsers:output_type -> ListMembershipUsersResponse
	126, // 309: NuzurProduct.CancelMembership:output_type -> CancelMembershipResponse
	128, // 310: NuzurProduct.ReactivateMembership:output_type -> ReactivateMembershipResponse
	130, // 311: NuzurProduct.UserHasActiveStripeSubscription:output_type -> UserHasActiveStripeSubscriptionResponse
	132, // 312: NuzurProduct.ListMembershipUserProjects:output_type -> ListMembershipUserProjectsResponse
	134, // 313: NuzurProduct.RemoveMembershipUser:output_type -> RemoveMembershipUserResponse
	137, // 314: NuzurProduct.ListMembershipsForUser:output_type -> ListMembershipsForUserResponse
	122, // 315: NuzurProduct.ChangeMembershipPlan:output_type -> ChangeMembershipPlanResponse
	139, // 316: NuzurProduct.SendPrioritySupportEmail:output_type -> PrioritySupportEmailResponse
	141, // 317: NuzurProduct.HandleWebhook:output_type -> HandleWebhookResponse
	143, // 318: NuzurProduct.IsProActiveForProject:output_type -> IsProActiveForProjectResponse
	145, // 319: NuzurProduct.GetMembershipTierForProject:output_type -> GetMembershipTierForProjectResponse
	147, // 320: NuzurProduct.EvaluateUserPrompt:output_type -> EvaluateUserPromptResponse
	149, // 321: NuzurProduct.HandleUserPrompt:output_type -> HandleUserPromptResponse
	151, // 322: NuzurProduct.GetUserPromptJob:output_type -> GetUserPromptJobResponse
	153, // 323: NuzurProduct.HandleFollowupUserPrompt:output_type -> HandleFollowupUserPromptResponse
	155, // 324: NuzurProduct.FinalizeUserPrompt:output_type -> FinalizeUserPromptResponse
	157, // 325: NuzurProduct.ListAIUsageForUser:output_type -> ListAIUsageForUserResponse
	159, // 326: NuzurProduct.GetAIUsageForUserInPeriod:output_type -> GetAIUsageForUserInPeriodResponse
	161, // 327: NuzurProduct.RegisterLocalAgent:output_type -> RegisterLocalAgentResponse
	163, // 328: NuzurProduct.RevokeLocalAgent:output_type -> RevokeLocalAgentResponse
	165, // 329: NuzurProduct.ListLocalAgents:output_type -> ListLocalAgentsResponse
	167, // 330: NuzurProduct.UpdateLocalAgentConnections:output_type -> UpdateLocalAgentConnectionsResponse
	169, // 331: NuzurProduct.GetLocalAgentConnections:output_type -> GetLocalAgentConnectionsResponse
	177, // 332: NuzurProduct.IssueProvisioningToken:output_type -> IssueProvisioningTokenResponse
	179, // 333: NuzurProduct.ExchangeProvisioningToken:output_type -> ExchangeProvisioningTokenResponse
	171, // 334: NuzurProduct.UpsertDeployment:output_type -> UpsertDeploymentResponse
	173, // 335: NuzurProduct.ListDeployments:output_type -> ListDeploymentsResponse
	175, // 336: NuzurProduct.MarkDeploymentDestroyed:output_type -> MarkDeploymentDestroyedResponse
	183, // 337: NuzurProduct.CreateAutomation:output_type -> CreateAutomationResponse
	185, // 338: NuzurProduct.RotateAutomationSecret:output_type -> RotateAutomationSecretResponse
	225, // 339: NuzurProduct.UpdateAutomation:output_type -> nem.Automation
	188, // 340: NuzurProduct.DeleteAutomation:output_type -> DeleteAutomationResponse
	225, // 341: NuzurProduct.GetAutomation:output_type -> nem.Automation
	191, // 342: NuzurProduct.ListAutomationsForProject:output_type -> ListAutomationsForProjectResponse
	193, // 343: NuzurProduct.TestAutomation:output_type -> TestAutomationResponse
	195, // 344: NuzurProduct.ListAutomationEvents:output_type -> ListAutomationEventsResponse
	227, // 345: NuzurProduct.GetAutomationEvent:output_type -> nem.AutomationEvent
	227, // 346: NuzurProduct.RetryAutomationEvent:output_type -> nem.AutomationEvent
	199, // 347: NuzurProduct.MarkChangeRequestApplied:output_type -> MarkChangeRequestAppliedResponse
	219, // [219:348] is the sub-list for method output_type
	90,  // [90:219] is the sub-list for method input_type
	90,  // [90:90] is the sub-list for extension type_name
	90,  // [90:90] is the sub-list for extension extendee
	0,   // [0:90] is the sub-list for field type_name
}

func init() { file_product_proto_init() }
func file_product_proto_init() {
	if File_product_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_product_proto_rawDesc), len(file_product_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   198,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_product_proto_goTypes,
		DependencyIndexes: file_product_proto_depIdxs,
		EnumInfos:         file_product_proto_enumTypes,
		MessageInfos:      file_product_proto_msgTypes,
	}.Build()
	File_product_proto = out.File
	file_product_proto_goTypes = nil
	file_product_proto_depIdxs = nil
}
