...
// generatedMarker is what go-code-gen stamps into every file it owns. A chart
// file without it was either written by hand or deliberately taken over — the
// generator's own escape hatch, since an unmarked file is left out of the
// manifest and so is never overwritten or cleaned up.
const generatedMarker = "Code generated by nuzur go-code-gen"

// checkHandMaintainedChart refuses to regenerate a chart nobody generated.
//
// The k8s provider force-enables `helm` whatever the project's saved config
// says, and the resolved config is then SAVED — so a single `deploy --provider
// k8s` against a project whose chart is hand-maintained would rewrite that
// chart from templates, commit it, release it, and make helm:true permanent.
// For sfapi that means dropping cert-manager.io/cluster-issuer and the tls
// block from a production gRPC API, which does not degrade to plain HTTP: it
// stops serving gRPC, because ingress-nginx only speaks HTTP/2 over TLS.
//
// Only Chart.yaml and values.yaml are consulted. The marker is per-file, and a
// fully generated chart already contains unmarked files — .helmignore and
// templates/NOTES.txt get no marker because their extensions have no comment
// syntax registered — so "the chart has an unmarked file" would refuse every
// chart we generate.
func checkHandMaintainedChart(chartDir string) error {
	var handWritten []string
	for _, name := range []string{"Chart.yaml", "values.yaml"} {
		body, err := os.ReadFile(filepath.Join(chartDir, name))
		if err != nil {
			// No chart at all is the normal first deploy; an unreadable one is
			// not ours to diagnose here.
			return nil
		}
	...
	return fmt.Errorf(
		"refusing to regenerate the Helm chart at %s: %s carries no generated marker, so this chart is maintained by hand.\n"+
			"  The k8s provider turns `helm` on regardless of the project's saved config, which would overwrite it from templates —\n"+
			"  losing anything the generator cannot express, such as a cert-manager cluster-issuer annotation or a tls block.\n"+
			"  To deploy this chart as it stands, use --release-only (skips generation, commit and CI).\n"+
			"  To hand the chart over to the generator instead, delete it and re-run.",
		chartDir, strings.Join(handWritten, " and "))
}
