...
func k8sValuesYAML(st *deployState) string {
	...
	writeIngressValues(&b, "", strings.TrimSpace(st.s.Domain))
...
// writeIngressValues enables an ingress for a host, or writes nothing when no
// host was given — leaving the chart's own default (disabled) in place.
func writeIngressValues(b *strings.Builder, indent, host string) {
	if host == "" {
		return
	}
	fmt.Fprintf(b, "%singress:\n", indent)
